...
const SENSITIVE_REPLACEMENTS: Array<[RegExp, string]> = [
  // v12.372:**原来每条都用 `\b` 包着中文词 —— 整个净化器对中文是空操作。**
  //
  // JS 的 `\b` 是「`[A-Za-z0-9_]` 与非词字符之间的边界」,而**中文字符不是词字符**,
  // 所以 `\b血腥\b` 在纯中文里永远匹配不上。实测 5 条典型 prompt:**0 条被改动**。
  // 后果:1026 敏感词的「用净化后的 prompt 重试一次」拿的是**逐字节相同的原文**,
  // 那个重试机制从写出来那天起就是死的 —— 白白多打一次上游、白白多等一轮。
  //
  // 改法上避开刚清理过的两个坑(v12.358~v12.366):
  //   · **不用单字**(原表里的裸 `枪` 会命中「水枪/枪法/长枪短炮」);
  //   · **长的备选排前面** —— JS 交替是 leftmost-**first** 不是 longest,
  //     `枪|手枪` 会先命中 `枪` 把「手枪」切成「手+能量装置」。
  // 暴力/血腥
  [/(血腥场面|血腥|鲜血|流血|血迹|血泊)/g, '红色液体'],
  [/(死尸|尸体|遗体)/g, '倒下的身影'],
  [/(砍杀|斩杀|屠杀|杀戮|杀害|杀死)/g, '击败'],
  [/(狂揍|毒打|殴打|暴力)/g, '激烈对抗'],
  [/(冲锋枪|机关枪|机枪|步枪|手枪|猎枪)/g, '能量装置'],
  [/(子弹|炮弹|弹药)/g, '能量光束'],
  [/(核弹|炸弹|炸药|爆炸)/g, '能量迸发'],
  // 裸露/色情
  [/(全裸|赤裸|半裸|裸露|裸体)/g, '身披薄纱'],
  [/(淫秽|淫荡|淫欲|色情)/g, '浪漫'],
  [/(极度性感|性感至极)/g, '优雅'],
  // 政治敏感(地名/人名通常已在 LLM 层过滤,此处兜底删词)
  [/(习近平|毛泽东|普京|特朗普|拜登)/g, ''],
  [/(台独|港独|藏独|疆独)/g, ''],
  // 毒品/自残
  // `大麻` 是「大麻烦」的子串 —— **多字词也不保险**,它可以是更长词的一部分。
  // 这条是本版自己的测试抓出来的(「大麻烦来了」→「神秘物质烦来了」)。
  [/(海洛因|冰毒|大麻(?!烦)|毒品)/g, '神秘物质'],
  [/(自杀|自尽|自残)/g, '情绪崩溃'],
];
