...
const TAG_COLOR: Record<string, string> = {
  character: 'var(--primary)', clothing: 'var(--accent-green)', environment: 'var(--accent)',
  lighting: '#E8943A', time: '#C8AA6E', seed: 'var(--secondary)',
};
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4">
        ...
        <div className="cinema-card !p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="rounded-lg border border-[var(--border)] p-3">
              <div className="flex items-center gap-1.5 mb-2"><ScanFace size={13} className="text-[var(--primary)]" /><span className="text-[11px] font-semibold">角色锁定</span></div>
              {char0 ? (
                <>
            ...
            {/* 环境锁定 */}
            <div className="rounded-lg border border-[var(--border)] p-3">
              <div className="flex items-center gap-1.5 mb-2"><Mountain size={13} className="text-[var(--accent)]" /><span className="text-[11px] font-semibold">环境锁定</span></div>
              {scene0 ? (
                <>
            ...
            <div className="rounded-lg border border-[var(--border)] p-3">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5"><Hash size={13} className="text-[var(--secondary)]" /><span className="text-[11px] font-semibold">种子锁定</span></div>
              ...
              <div className="cinema-mono text-sm text-[var(--primary)] tracking-wider">{s.mainSeed}</div>
        ...
        <div className="cinema-card !p-4">
          ...
          <div className="flex flex-col gap-2">
            {storyboards.slice(0, 8).map((sb: any, i: number) => {
              ...
              return (
                <div key={sb.id || i} className="flex items-center gap-2 py-1.5 border-b border-[var(--border)] last:border-0">
      ...
      <aside className="cinema-card !p-4 h-fit">
          ...
          <div className="flex flex-col gap-1">
            {LINK_MODES.map((m) => (
              <button key={m.id} onClick={() => set({ linkMode: m.id })}
                className={`text-left px-2.5 py-1.5 rounded-md border transition ${s.linkMode === m.id ? 'border-[var(--primary)] bg-[var(--primary-muted)]' : 'border-[var(--border)] hover:border-[var(--border-hover)]'}`}>
        ...
        {/* 连贯性强度 */}
        <div className="mb-4">
          <label className="text-[11px] font-semibold mb-1 flex justify-between">连贯性强度 <span className="cinema-mono text-[var(--primary)]">{s.continuityStrength.toFixed(2)}</span></label>
          <input type="range" min={0} max={1} step={0.05} value={s.continuityStrength}
            onChange={(e) => set({ continuityStrength: Number(e.target.value) })} className="w-full accent-[var(--primary)]" />
        ...
        {([['clothingLock', '服装锁定'], ['lightingLock', '光照锁定']] as const).map(([key, label]) => (
          <div key={key} className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => set({ [key]: !s[key] } as any)}
              className={`cinema-mono text-[10px] px-2 py-0.5 rounded border ${s[key] ? 'border-[var(--accent-green)] text-[var(--accent-green)]' : 'border-[var(--border)] text-[var(--muted)]'}`}>
              {s[key] ? 'ON' : 'OFF'}
            </button>
          </div>
        ))}
        ...
        <div className="cinema-mono text-[9px] opacity-50 mb-2 leading-relaxed border-t border-[var(--border)] pt-2">{describeContinuity(s)}</div>
        ...
        {savedMsg && <p className="cinema-mono text-[10px] mt-1.5 text-center flex items-center justify-center gap-1 text-[var(--accent-green)]"><Check size={11} />{savedMsg}</p>}
      </aside>
    </div>
  );
}
