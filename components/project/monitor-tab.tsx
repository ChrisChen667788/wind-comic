...
export function MonitorTab({ projectId, storyboards = [] }: { projectId: string; storyboards?: any[] }) {
  ...
  function download(format: 'edl' | 'fcpxml') {
    const a = document.createElement('a');
    a.href = `/api/projects/${projectId}/export-edl?format=${format}`;
    a.download = '';
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="cinema-card !p-4">
        <div className="cinema-eyebrow mb-2 flex items-center gap-1.5"><FileDown size={13} className="text-[var(--primary)]" /> 专业出片对接 · DaVinci / Premiere</div>
