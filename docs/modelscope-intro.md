...
<h1 align="center">Wind Comic <sub><sup>v12.455</sup></sub></h1>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tests-6095%2F6095-2ea44f"  alt="6095 tests passing" />
...
<p align="center">
  <sub>Mirror for mainland China · 国内 AtomGit 托管(自动同步自 GitHub):<a href="https://atomgit.com/ChrisChen667788/wind-comic">atomgit.com/ChrisChen667788/wind-comic</a></sub>
</p>
...
<table>
  <tr>
    <td width="190" align="center">
      <a href="https://metaso.cn/minimax-h3/?s=wind-comic" target="_blank" rel="noopener"><img src="https://modelscope.cn/models/haozi667788/wind-comic/resolve/master/assets/sponsors/metaso.png" width="163" alt="秘塔科技 MetaSota"></a>
    </td>
    <td>
      <strong>MiniMax H3 视频生成 API｜秘塔科技</strong> 秘塔科技提供高性价比的 MiniMax H3 视频生成服务：<strong>768P 仅 0.09 元/秒，2K 仅 0.15 元/秒</strong>。支持原生 2K、音画同步，API 兼容 <strong>OpenAI 协议</strong>，同时支持 <strong>ComfyUI</strong>，无需自行部署 GPU。 🎁 通过 <a href="https://metaso.cn/minimax-h3/?s=wind-comic" target="_blank" rel="noopener noreferrer">wind-comic专属链接注册</a>，即可领取赠送额度及专属优惠。<br/><br/>
      <sub><strong>MiniMax H3 video-generation API — MetaSota (秘塔科技).</strong> ¥0.09/s at 768P, ¥0.15/s at 2K · native 2K with synced audio · <strong>OpenAI-compatible</strong> API · <strong>ComfyUI</strong> supported · no GPU to deploy. 🎁 Sign up through the <a href="https://metaso.cn/minimax-h3/?s=wind-comic" target="_blank" rel="noopener noreferrer">wind-comic link</a> for bonus credits and a partner discount.</sub>
    </td>
  </tr>
</table>
...
### 10. **6095 tests, TypeScript strict, no fake "coming soon"s**
...
> Lineup verified **2026-09-22** (Artificial Analysis arena, fetched twice by independent checkers that agreed rank-by-rank; 18 standing claims re-tested + 35 hard numbers independently re-checked. Ledger: [`docs/competitive/claims.json`](https://github.com/ChrisChen667788/wind-comic/blob/main/docs/competitive/claims.json); full analysis: [`docs/COMPETITIVE-GAP-2026-09.md`](https://github.com/ChrisChen667788/wind-comic/blob/main/docs/COMPETITIVE-GAP-2026-09.md)): **the top slot swung back to Google, and the Chinese camp still owns the no-audio boards** — T2V **with** audio: Gemini Omni Flash **1233** > Wan 3.0 1229 > MiniMax H3 Max 1227 > MiniMax H3 1220 > Seedance 2.0 720p 1210. T2V **without** audio: **Wan 3.0 1336** > Gemini 1330 > MiniMax H3 1302. I2V with audio: **MiniMax H3 Max 1195** > MiniMax H3 1181 > Gemini 1177; I2V without audio: Gemini 1369 > Wan 3.0 1361 > Bach 1.0 Pro 1359 > MiniMax H3 1354. New entrant **xAI Grok Imagine Video 1.5** (Jun 16): **7 simultaneous visual reference anchors** (character + scene + prop + style) plus native audio and voice cloning, $0.08 (480p) / $0.14 (720p) / $0.25/s (1080p) — **more reference slots than our current cref+sref pair**. **Kling 3.0** remains the all-round pick for short drama (native 4K / 60fps / 15s / **6 coherent shots** / built-in multilingual dialogue + lip-sync, $0.084–0.112/s); **Veo 3.1** (first shipped 2025-10-15; 4K and vertical added 2026-01-13) can reach 60s+ via **Scene Extension — but that is stitching; the single-generation ceiling is still 8–10s**. It uses its own joint audio-visual generation (**Lyria 3 is a separate Google music model, not Veo's audio component**), from $0.40/s. Note: Veo 3.1's own T2V+audio Elo is ~1091 — the 1237 on the board belongs to Gemini Omni Flash, a different product. **✅ The Sora 2 API shutdown (2026-09-24) is confirmed by OpenAI — and this project has been guarded since v12.173/207**: warn before the date, auto-drop from the model chain after it (falling back to veo/kling), throw only if the chain empties (`tests/v12-452-sora-sunset-runtime.test.ts` runs the branch against a fake clock; `tests/v12-173-sora-sunset.test.ts` keeps the defaults clean). **Not an open action item.**
> **🇨🇳 The domestic camp broke through on both shot length and reference count — and two engines we already ship are behind.** **Seedance 2.5** (7-31) and **Wan 3.0** (8-24 GA) both do **native 30s single takes**; Seedance accepts **50 reference inputs** (token-billed, ¥42–70/M tokens), Wan 3.0 runs **¥0.42/s at 720P** and converts documents straight to video (PPT/Word/PDF → 30s). **Vidu Q3** is the only Chinese model explicitly positioned for short-drama industrialisation (7 reference images with multi-subject locking, 6 cinematic effect packs, lip-sync driving; Turbo 1080p ≈¥0.41/s). **PixVerse C1** (4-08) is the most narrowly short-drama-vertical of all (storyboard-grid output + multi-speaker lip-sync). **⚠️ One finding about our own stack, which matters more than any competitor:** ① **both of the previous round's self-findings are now fixed** — the default video model moved to H3 in v12.402 and the two disagreeing Vidu call paths were unified in v12.403. What replaced them is a billing wall: **H3 is pay-as-you-go only — MiniMax's own pricing page says Token Plan does not cover it** (re-confirmed 2026-09-22, $0.08/s at 768P, $0.13/s at 2K). On a Token Plan key every H3 call is refused, so the pipeline falls back to Hailuo-2.3 (v12.446 made that fallback actually fire — it had been dead for 16 days because it only recognised the English error text), and **reference-video motion transfer (v12.448) cannot run at all**. The health page now says so out loud instead of showing MiniMax all-green off a TTS probe (v12.452). ② **The Sora 2 API retires 2026-09-24** (re-confirmed on OpenAI's deprecations page 2026-09-22, no extension). The guard has existed since v12.173/207 — warn before the date, drop Sora from the model chain after it, throw only if the chain empties — but until v12.452 the two tests "locking" it only grepped the source, so **the branch had never actually executed**. It is now run against a fake clock with mocked upstreams, which also exposed a gap: if the vendor stops serving before our UTC-midnight cutoff, a Sora error used to abort the whole chain instead of falling through to veo/kling.
>
...
| Per-shot regenerate with custom prompt | ⚠️ | ✅ | ✅ region edit (timestamp + marked area, 2.5) | ✅ motion brush | ✅ 对话式迭代编辑(招牌能力) | ✅ video-edit 端点 | **✅ + reference image upload** |
| Pacing / conflict audit | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | **✅ v2 (v12.275): shot score + reversal detection **plus** conflict-curve shape (escalating / flat / front-loaded / no-climax), drag-segment localisation to exact shot ranges, opening-density check, and duration-rhythm analysis — every finding names the shots to fix** · optional hard gate `PACING_GATE=block` (v12.455; default only warns) |
| Editable timeline export (EDL / FCP7 XML) | ❌ | ❌ | ⚠️ Dreamina can push assets into CapCut, not a timeline | ❌ | ❌ | ❌ | **✅ EDL (CMX 3600) + FCP7 XML (Premiere / DaVinci) + built-in multi-track timeline** |
...
| Royalty-free AI BGM per story | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | **✅ ElevenLabs Music → self-hosted ACE-Step / YuE → MiniMax fallback chain (v12.410; needs one of them configured — MiniMax music-2.6 is closed to new accounts)** |
...
2. **能交付进专业剪辑线,支撑平台正在扶持的「真人+AI 协作」。** EDL / FCP7 XML(Premiere、达芬奇可导入)/ **真二进制 AAF**(MS-CFB,面向 Avid,尚未在 Avid 实机验证)导出 —— AI 出的素材可以进人类剪辑师的工程,而不是只能整片直出。**Seedance/即梦、可灵、TapNow、OiiOii 都没有结构化时间线导出**(2026-09-25 核查;即梦的「去剪映编辑」送的是散素材,不是时间线)。但这不是「全部竞品的空白」:Descript 早有,开源的 [openframe](https://github.com/murongg/openframe) 也已能导出 FCPXML/EDL。
...
>
> **📚 完整 18 张 + 每张的实现逻辑与工作流架构说明:[docs/SCREENSHOTS-v12.425.md](https://github.com/ChrisChen667788/wind-comic/blob/main/docs/SCREENSHOTS-v12.425.md)**
...
|---|---|
| ![角色转身图](https://modelscope.cn/models/haozi667788/wind-comic/resolve/master/assets/v12-425/08-character-sheets.jpg) | ![导演台](https://modelscope.cn/models/haozi667788/wind-comic/resolve/master/assets/v12-425/10-director-console.jpg) |
...
| **Music / BGM** | ElevenLabs Music (`ELEVENLABS_API_KEY`) | self-hosted ACE-Step / YuE (`MUSIC_SELFHOST_URL`) → MiniMax music-2.6 (closed to new accounts) |
...
- `MINIMAX_API_KEY` — image-01 / Hailuo-2.3 video / speech-2.8-hd TTS / music-2.6 BGM (closed to new accounts — for BGM set `ELEVENLABS_API_KEY` or `MUSIC_SELFHOST_URL`)
...
2. **Tests gate everything.** Vitest 6095/6095 must stay green. Add tests for new lib/service files.
