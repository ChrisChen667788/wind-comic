...
# 增补 · Batch L:2026-09-03 竞品分析落账(v12.415 落账本身,v12.416 → 起执行)

> 来源:[`docs/COMPETITIVE-GAP-2026-09.md`](./COMPETITIVE-GAP-2026-09.md)
> (8 视角并行联网调研 + 26 条高风险数值独立二次复核 + 上轮 12 条断言逐条证伪,45 个 agent 全部完成)。
>
> **本轮最该记住的一句话:四条我们对外宣称的「独有」被逐一推翻 —— 不是被追上,是当初说过头了。**
> 说过头本身是一种劣势:它让人低估真实竞争强度,也让真正站得住的那条(节奏审计是
> **独立可拦截门禁**,而非生成内联优化)被淹没在夸张里。

## L-P0 · 说过头的话要先收回

| 版本 | 标题 | 量级 |
|---|---|---|
| v12.415 | **收回四条「独有」宣称**:EDL/AAF(Descript 早有)、唯一 MIT 自托管端到端(EvoLinkAI 等多个)、BYO 注册表(PopShort.AI 已有)、节奏审计无对应项(火山剧创有内联校验)。README 双语改口 + GAP-2026-09 + claims 台账刷新 | S |

## L-P1 · 补真缺口

| 版本 | 标题 | 量级 |
|---|---|---|
| v12.416 | **帧内中文文字渲染**:接 Seedream 5.0 Lite($0.04/张,Glyph-Aligned ByT5)——片头/字卡/对白框,libass 后期叠加替代不了 | M |
| v12.417 | **唇形重塑**:激活已接入的 Kling 口型(**5 语言**,复核纠正:非 20+)+ 评估 Sync.so Lipsync-2-Pro(任意语言,含免费层) | M |
| v12.418 | **多参考图路数**:从拼合图近似升到官方分通道(GPT Image 2 **16 路** / Nano Banana Pro 14 图双通道) | M |

## L-P2 · 把已配置的真正用起来

| 版本 | 标题 | 量级 |
|---|---|---|
| v12.419 | **独立 grader 默认生效**:v12.412 配置就位但默认仍自评(如实告警中);指向另一家模型即可拿到 Anthropic Outcomes 实测的 +8–10pp | S |
| v12.420 | **自托管推理包**:v12.411 只给了契约,用户仍要自己把 Wan 2.7 / LTX-2.5 包一层 —— 出一份 compose 即可跑的参考实现 | M |

## L-P3 · 长期

| 版本 | 标题 | 量级 |
|---|---|---|
| v12.421+ | 预构建 Docker 镜像,`docker pull` 一行起(v12.414 已压到一条命令,但仍需本地 build) | M |
| v12.422+ | 30s 原生单镜(等国产端点开放 API,或自托管权重跑通) | L |

## 本轮明确**不做**的

- **不接 Seedance 2.5** —— 质量最强之一,但好莱坞六大停止函纠纷**未和解**,接入有法律风险;
- 不自研生成模型 —— 结论不变但**理由变了**:现在是「开源权重已经够好」,不是「我们做不过」;
- 不做多平台自动投放 —— 越过「制作工具」边界。

## 下一轮(最晚 2026-09-17)必须重验的

- Seedance 版权纠纷是否和解 / 再次中止 → 直接决定 C3;
- MJ **V8.2** 下 `--oref` 的**真机出图验收**(v12.404 只做了代码级修复,无额度未验);
- Wan 2.7 / LTX-2.5 是否被我们真正跑通(现在只有契约,见 C14);
