...
| `character_ip_tokens` `character_ip_grants` (+ cameo import 写 character_library) | 7+1 | ❌ 新建 | v9.0.3d |
...
- **v9.0.3d · 新建 cameo-ip-repo**(IP 经济收尾):`character_ip_tokens`(issue/revoke/recordUse)+ `character_ip_grants`(request/decide/recordUse)+ `importCameoToLibrary`(写 character_library)。注:recordTokenUse 同时改 token + grant 两表(原为两条独立写, 非事务), 迁移保留或可顺手包 `getDbDriver().transaction`。callers: cameo-ip 3 路由 + prompt-ide/assets + usage。
