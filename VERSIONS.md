...
| **v8.1** | 2026-05-25 | `(本次)` | 阶段九增强 · 智能联动规则引擎(对标 CineMatrix「Auto-Update Logic」):新增 `lib/auto-rules`(纯逻辑 + 单测 11)—— 声明式规则(条件 tension/intensity/shotSize/atmosphere × gte/lte/in → 给 ShotSpec 打补丁)+ 5 条预设(高紧张→低调高反差 / 特写→浅景深大光圈 / 强情感→提运动 / 平静→高调低反差 / 霓虹夜→霓虹黑色冷色温)+ `buildRuleContext`(情绪词→tension/intensity, 串 v7.5 emotionScore)+ `evaluateRules`(多规则命中合并)+ `applyRulesToSpec`;摄影台弹窗加「✨ 智能建议机位」一键按情绪/景别套用规则 + 命中清单提示;项目页打开摄影台时透传该镜情绪。tsc 0 / 全量 1841 测试(+11) |
