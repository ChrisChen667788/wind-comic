...
| **v6.7** | 2026-05-25 | `(本次)` | 移除 banana 死配置 + API 健康仪表盘:`lib/provider-health`(响应归一成 正常/额度用尽/配置缺失/不可达,19 单测)+ `GET /api/health/providers`(实时探测 MiniMax/qingyuntop/vectorengine,读余额,不回传 key,60s 缓存)+ `/dashboard/health` 仪表盘 + 侧栏入口;删 banana.service + config/midjourney legacy 引用 |
