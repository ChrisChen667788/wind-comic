...
| **v6.5.1** | 2026-05-24 | `(本次)` | 成员消费扣减 + 真·多用户邀请:`team-credits` 扩 `consume`/`costOf`/`capAllocationToPool` + `lib/team-invite`(token 校验/过期/接受,15 新单测)+ `team_invites` 表 + `POST /api/team/consume`(超额 400)+ `POST/GET /api/team/invite` + `POST /api/team/invite/accept`(须登录不创建账号)+ 团队页邀请面板 + `/dashboard/team/accept` 接受页 |
