...
| **v7.0.1** | 2026-05-25 | `(本次)` | MiniMax 语音兜底打通:核实新 key 支持 TTS 且 `t2a_v2` 无需 GroupId(之前仅模型名错),`MINIMAX_TTS_MODEL=speech-02-hd` + `tts.service` 默认改之 + 健康看板去 GroupId 硬要求 + `classifyMinimax` 加 2056 限流窗口判定(已配置可用);健康看板 minimax-tts → ok、整体 healthy。tsc 0 / 1712 测试 |
