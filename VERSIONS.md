...
| **v7.0** | 2026-05-25 | `(本次)` | DeepSeek 创意主 LLM + MiniMax 全局兜底:编剧/导演创意 LLM → `deepseek-v4-pro`(独立 endpoint),通用仍 `claude-sonnet-4-6`;`callLLM` 重构成尝试链(主→MiniMax `MiniMax-M2.7` 兜底,任何异常/欠费/超时自动路由);config 加 creative/fallback endpoint;MiniMax key 更新(LLM 兜底实测 200);健康看板拆 3 条 LLM 线(通用/创意/兜底)全 ok。tsc 0 / 1711 测试 |
