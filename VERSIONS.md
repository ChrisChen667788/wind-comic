...
| **v7.0.2** | 2026-05-25 | `(本次)` | MiniMax 视频标准版额度用尽自动转 Fast 版:标准/Fast 768P 各有独立日额度,`generateVideo` 在配额错误(2056/usage limit/额度)时自动路由 `generateVideoFast`(独立额度),Fast 也满才落下一引擎;`isMinimaxVideoQuotaError` 纯函数 + 2 单测。tsc 0 / 1714 测试 |
