...
> 多智能体 AI 短剧/漫剧生成流水线。本文件汇总从首个公开版本 (v2.12.0) 到当前 (**v12.443**)
> 的全部版本信息。每条含发布日期 + commit + 关键交付。详细验收数据见 `ROADMAP.md`。
>
> 截至 **v12.443**:**vitest 5789 全绿,tsc 0 错误**(SQLite/Postgres 双驱动)。v12.218 起进入「加固路线图」(对抗尽调 P0→收官,详见 `docs/ROADMAP-hardening-v12.218.md`)。
>
...
| **v12.443.1** | 2026-09-18 | `待填` | **🩹 零回归基线锁错了东西:PNG 文件哈希锁的是 zlib 实现,不是图像**。v12.443 那条「没设姿态的草图逐字节不变」本机绿、**CI 红** —— PNG 的像素数据经 zlib 压缩,不同 Node 版本的 zlib 输出不同(本机 Node 25 / CI Node 22),于是同样的图算出两个文件哈希。改成锁**解码后的像素**(测试里本来就自带极简 PNG 解码器),内容一样就过、画错一个像素就红 —— 反向验证:把头部半径改 1.2 倍,立刻红。教训与 v12.442.1 同源:**断言要对着你真正关心的那个东西**,别对着它的某种编码形式。 |
