...
> 多智能体 AI 短剧/漫剧生成流水线。本文件汇总从首个公开版本 (v2.12.0) 到当前 (**v12.455**)
> 的全部版本信息。每条含发布日期 + commit + 关键交付。详细验收数据见 `ROADMAP.md`。
>
> 截至 **v12.455**:**vitest 6095 全绿,tsc 0 错误**(SQLite/Postgres 双驱动)。v12.218 起进入「加固路线图」(对抗尽调 P0→收官,详见 `docs/ROADMAP-hardening-v12.218.md`)。
>
...
| **v12.394.0** | 2026-08-31 | `47d89b1` | **🚨 每天 04:10,清理任务把正在被引用的成片当成孤儿删掉**。`referencedBasenames()` 从 DB 的 `persistent_url` / `media_urls` 里抽文件名,用的是 `/([A-Za-z0-9._-]+\.(?:mp4\|…))/g`。可 DB 里的成片 URL 是 `serveFilePathUrl()` 用 `encodeURIComponent` 生成的:`/api/serve-file?path=%2FUsers%2F…%2Fcomposed%2Ffinal-1788071173502.mp4&sig=…`。**字符类不含 `%`,匹配从 `2F` 起步** —— 抽出来的是 `2Ffinal-1788071173502.mp4`,而 `sweepDir` 比对的是磁盘上的真名 `final-1788071173502.mp4`,`referenced.has(f)` 于是**恒为 false**。而它是这段清理逻辑**唯一**的保护(其余只有 mtime 阈值)—— 保护恒假 = 所有被引用的成片过了 7 天照删不误。**实测(真实库 + 真实目录)**:composed 目录 16 个成片,**修复前受保护 0 个**;库里有 **124 条**这种 URL 编码形态的资产;引用集从 336 涨到 591。launchd 里 `com.qingfeng.windcomic.cleanup` 每天 04:10 **真的在跑**。owner 那次「30 个项目 534 个素材被清空」,机制就在这里 —— 当时只记下了现象和「删除类代码必须查引用」的教训,没找到这个正则。修法:抽名前先 `decodeURIComponent`,解码串与原串**都**扫一遍(没编码的 URL 解码后不变、只扫一次;多扫一遍原串能兜住解码失败与双重编码)。**这个集合是保护名单,只能变大不能变小** —— 宁可多留,不可多删,测试里专门锁了这条单调性。测试 10 条,含用真实 DB 形态驱动的正反例。 |
| **v12.399.0** | 2026-08-31 | `c789543` | **🧯 同一道防线,两条清理路径都得有**。这个仓库有**两套**删文件的逻辑,都由同一个 cron(每天 04:10)触发、都以「有没有被引用」为唯一保护:`cleanup-media` 的 `sweepDir()`(扫 composed / exports / media,按**文件名**比对)和 `asset-storage` 的 `cleanup()`(扫 storage/assets,按**内容寻址 key** 比对)。v12.394 的事故形态是**保护名单恒空**,那次 owner 丢了 30 个项目 534 个素材;v12.398 给 `sweepDir` 加了「一个都没被引用就停手」的自检 —— **但 asset-storage 那条没有**,而它管着 160 个素材文件。同一个病要犯第二次的话,第二次就是它。本版把判据抽成 `lib/cleanup-guard.ts`,两处共用;asset-storage 侧同样改成「先统计、后删除」(边走边删的话,等发现「一个都没被引用」时前面的已经删了),拒绝时返回 `aborted` 而不是假装「删了 0 个」——「removed: 0」和「拒绝执行」是两回事,调用方得能分辨。实测干跑:149 受保护 / 0 待删,行为不变但多了一道防线。测试 10 条。 |
| **v12.455.0** | 2026-09-26 | `待填` | **🚦 节奏审计真能拦了 + 对外口径按代码校正**。起因是一次竞品定位核查:README 第 253 行和竞品台账把节奏审计写成「可拦截的独立工程门禁」,代码里却是「非阻塞」(`writer-agent` 注释原话),全仓没有任何地方拿审计结果做拦截 —— 以「可证伪」为卖点的台账,自己先和代码打架。① **真拦截**(`lib/pacing-gate.ts`):`PACING_GATE=off|warn|block`,**默认仍是 warn**(保留「先看到画面再决定」的原取舍)。block 档下剧本不达标,就在角色设计之前停下:剧本连同重审报告和门禁结论落库、项目标 failed、发 `PACING_GATE_BLOCKED`。**不复用 waitForGate** —— 那套人工闸门 5 分钟没人响应就自动放行,拿它当门禁等于没拦。放在人工闸门之后、**按当前剧本重审**(闸门里改过的按改后的审;续跑装载的同一份不达标剧本照样拦);请求字段 `pacingGate` 只能比服务端更严、放松不了;要照拍得显式带 `pacingOverride: true`,在剧本资产里记为 override;审计跑不出结果时 block 档按拦下算(fail-closed)。冲突词典只有中文,所以**非中文剧本跳过并明说「这次没有生效」**,拉片复刻同样跳过。② **队列不白重试**:拦截事件带 `terminal:true`,worker 见到直接终态(`failJob(..., {terminal:true})`)。刻意不看 `retryable` —— 那个字段的语义是「能否只重试这一步」,欠费/限流/无 key 早就带 `retryable:false`,拿它判终态会顺手改掉它们的重试行为。`startProduction`(第二条创作入口)同一判据,拦下即抛。③ **对抗复查两轮**(4+2 个视角,每条发现独立核实,两轮都完整跑完):**整季批量的进程内路径会把失败的集写成 completed** —— 给流水线传空回调、返回就标完成,而流水线的失败是「发 error 后正常返回」;一集就是一个项目,`setEpisodeStatus` 还会把流水线如实写下的 failed 覆盖回 completed(v12.433 的诚实状态在这条路径上失效,剧本没出、一条片都没出的集也中招)。现收进 `lib/series-episode-run.ts`,与 worker 同一判据:发过 error → failed(整季页仍可重生)。中文判定两轮都有漏:按字母数算,英文人名多的中文剧本被判成非中文;改按词 1:1 算,AI / CU / SFX 这类短缩写又被高估 —— 最终每个英文词折半个汉字。文档一处说过头(「请求字段绕不过去」,而 `pacingOverride` 本身就是请求字段)已改。一条被驳回:`saveAsset` 内部本就吞异常。④ **对外口径**:README 两处节奏门禁表述加自查更正;「FCPXML」改为实际导出的 FCP7 XML(Premiere / 达芬奇可导入);英文 README 里「EDL/FCPXML/AAF 是全部已查竞品的共同空白」删掉(九月初就已收回、却一直留在正文,Descript 早有,开源 openframe 也有);竞品表:Seedance 2.5 单镜重生改 ✅(区域编辑),新增「可剪辑时间线导出」一行,配乐格与引擎表从停服的 MiniMax music 改成 v12.410 的兜底链;剪映导出按钮点之前就标明只兼容 5.9 及以下。COMPETITIVE-GAP 收回「测试密度全赛道无竞品」(开源 ArcReel 有 822 个测试文件,本仓 631)。台账:C19 按自己的推翻条件记为 falsified(openframe 已有 FCPXML/EDL),C1 更新限定,新增 C31(门禁曾名不副实)/ C32(测试密度不独有)/ C33(四家闭源竞品均无时间线导出)。⑤ **测试** 3 个新文件 41 条,断言都落在「下游到底有没有被调用」上,每个拦截用例配反面;**18 处注入缺陷全部被断言抓住** —— 其中「门禁沿用编剧报告的短剧口径」首轮没抓住,补了用例;另有 3 处首轮因 vitest 进程异常退出显示「抓住」,单独重跑确认是断言失败才算。门禁逻辑收进 `lib/pacing-gate.ts` 的 `gateScriptForProduction` / `assertPacingGate`,两条入口共用;编排器只加一行调用(它有「< 4500 行」的瘦身锁,改前正好卡在线上,顺手合并了一处连续空行)。已知未做:片段重拍的「字节拷贝」不成立(保留段会重编码、48kHz 原片的重拍段音调会跑偏)由另一会话修;门禁只覆盖中文剧本;界面上还没有 block 档开关(靠 env / 请求字段)。 |
| **v12.454.0** | 2026-09-25 | `824fd41a` | **🧭 画布布局存得住 + 开机失败留在界面上**。两件事都是「做了一半」的补齐。① **自由画布第一步:节点位置持久化**。流水线画布拖一下节点,位置只写进内存 store(纯 zustand,无持久化)—— **刷新即回默认布局**,八个节点默认斜着排,想理顺一次就得每次重排。本版:`projects` 加 `canvas_layout` 列,新增 `GET/PATCH /api/projects/[id]/canvas-layout`(读 view / 写 edit —— 改的是项目的布局,不是个人视图,只读协作者看得到改不了),校验按**白名单直接拒**而不是静默丢弃(静默丢弃会让「保存成功」和「其实没存」长得一样);画布进来拉取、拖完防抖 500ms 存,`keepalive` 让「拖完立刻关标签页」也能送达。② **开机失败必须留在界面上**。开机请求非 200(实测 401;v12.448 的归属守卫会产生 403;还有 402 预算、429、断网)或 SSE 推来 error 时,此前**只弹一个几秒就消失的浮层**;浮层一没,顶栏仍写着「创作中」、各节点仍是「等待编剧完成…」——**与「跑得慢」完全无法区分**,用户一直干等。现在:顶栏变「已中断 · 原因」、正在跑的那个节点转 error(**没轮到的保持 pending**,标红会谎报范围)、对话流里留一条不会消失的系统消息;状态码翻成人话(401 让人重登、402 说预算、403 说权限),服务端给了自己的话就用它并截断(报错原文里可能带内部路径)。这是 v12.433「后端不许把失败标成完成」的前端孪生。**真机跑通**(本地测试令牌 + `MOCK_ENGINES=1`,不碰账号、零外部调用):拖动 → PATCH → 落库 → 再读回来逐一对上;401 与 402 两条失败路径都在顶栏如实显示(`data-pipeline-status="error"`,文案分别是「登录已失效,请重新登录后再开机」与服务端原话「本次预估 ¥18.8 将越过硬上限 ¥5」)。**真机顺带挖出一处静默失败**:把节点拖到极远处,接口按范围校验回 400,而界面毫无反馈 —— 拖动照样跟手,刷新才发现没存上;现已把失败原因回报到界面(黄色提示带接口给的原因)。**对抗复查三视角挖出并修掉 5 处**:重新开机走的是 `clearAgentOutputs` 而不是 `resetWorkspace`,不清中断标记 → 新一轮全程顶着上一次的「已中断」;换项目时本地节点没清,而八个节点 id 在所有项目里都一样 → 新项目永久继承上一个项目的布局;保存用的是闭包里的 `nodes`(可能还是拖动前那一帧),改以**回调带回的被拖节点**为准(多选拖动一并处理);组件卸载时把还在防抖窗口里的那次**补发**而不是丢掉(拖完就切页是常见操作);布局保存不再顺手改 `updated_at` —— 项目列表按它排序,拖个节点不该让项目窜到「最近修改」最前(原测试正好把这条错误行为断言成了预期,一并改正)。**测试 44 条**(纯逻辑 + 真库路由往返 + 防抖/补发 + 把 ReactFlow 换成桩件**真渲染**画布验「拖完真的存」「读回来真的用上」「换项目不串」「存不上要出声」)。**变异 39 个注入点全部由断言拦住** —— 首轮漏掉的正是「组件里那一行保存调用被删掉」:17 条纯逻辑测试全绿放行,补了真渲染才抓住(本仓最常见的坏法就是能力写好了没接线)。**已知未做**:`addColumnIfMissing` 是 SQLite 专有实现(`PRAGMA table_info`),Postgres 老部署加不上新列 —— 这是历次加列共有的系统性老问题,不在本版改,另发一版;自由画布的连线编辑与阶段级重跑是后续版本。 |
| **v12.453.0** | 2026-09-23 | `10a37412` | **🧱 创作页角色槽位:窄列里被压成 99px,和一处「渲染中更新别的组件」**。两处都是 v12.447 真机时记下、当时另开的待办。**① 槽位栅格按视口断点排**(`md:grid-cols-3`),而它实际待在一个 **320px 宽的窄列**里 —— 视口 1024 就按「≥768 三列」排,每格被压到 **99px**:角色名输入框只剩 **18px**,整排控件(上传文件 / 用 URL)横向铺到 383px、**溢出到卡片外点不到**。改成按**可用宽度**排(`auto-fit` + 每格最少 200px),放不下就换行 —— 这类问题的根子是「用视口断点排一个窄容器」,只调断点(md→xl)治不了:容器宽度由页面布局决定,不由视口决定。**真机实测**:1024 宽下栅格 320px → 单列、每格 320px、角色名框 **18px → 218px**、溢出 0;1680 宽下栅格 648px → 仍是三列(每格 208px)、溢出 0,宽屏表现与修前一致,无回归。**② `updateSlot` 把 `onChange`(父组件的 setState)写在 `setSlots` 的更新函数里面**:React 可能在渲染期调用更新函数,报「渲染中更新别的组件」;StrictMode 下更新函数被调两次,**父组件因此被通知两次**。更新函数必须是纯的 —— 改成先按当前 state 算出 next,再分别 `setSlots` 与 `onChange`。**测试 4 条**:StrictMode 下改一次名字父组件只被通知一次(修前两次)、改动期间无该条 React 告警、连改两个槽位两次都留得住(换用当前 state 算之后最容易犯的回归)、栅格不再用视口断点分列。**3 个变异全部抓红**。
| **v12.452.0** | 2026-09-23 | `508c8ef5` | **🩺 健康页终于探 MiniMax 视频了 —— 草稿版差点把用不了的 H3 报成绿;Sora 退役闸门第一次被真跑**。**① 健康页 MiniMax 视频探针**:健康页对 MiniMax 原来只探 TTS,TTS 在 Token Plan 上是好的 → 一片绿;而视频默认模型 H3 在 Token Plan 上根本调不动,出片靠 v12.446 的回落改用 Hailuo-2.3,参考视频动作迁移(v12.448)整项用不了 —— 健康页一个字都没有。新增「受限」状态(黄灯,六语文案,动作「换计费方式」):H3 用不了、回落的 Hailuo-2.3 在套餐内 → 「当前套餐用不了 MiniMax-H3(只能按量付费)· 出片自动改用 MiniMax-Hailuo-2.3 · 参考视频动作迁移用不了」。**零成本**:探测请求没有任何可生成的内容(空 content 数组),只让它走到模型/套餐校验;万一哪天上游对这种请求也建了任务(会扣费),探针当场自停、之后不再发。钥匙与地址直接取出片代码用的那份(`API_CONFIG.minimax`),不自己读环境变量 —— 否则地址配成 `…/v1` 时探针打错地址报红、出片却是好的。**草稿版的假绿(真打一次才发现)**:9-18 首测时只带 `model` 的请求就能走到套餐校验,草稿据此把「被参数校验拒」当成「在套餐内」;**9-22 复测上游已改成「格式绑定 → 模型/套餐 → 内容语义」**,只带 model 的请求停在绑定层 —— 真模型与**根本不存在的模型名**回的是同一句 `binding: expr_path=content, missing required parameter`,草稿把用不了的 H3 报成了绿(单测全绿,喂的是 9-18 的原文)。对照实验逐个补必填字段(内容始终为空)后,H3 回「TokenPlan 或 Credit 暂不支持 MiniMax-H3 系列模型」、假模型回「该模型暂不支持 /v2/video_generation」,终于分得开。判「在套餐内」现在**必须有正向信号**(错误说的是内容/提示词为空 —— 只有过了模型与套餐两关才会走到这一步);停在绑定层、模型名不认、读不懂的回复一律不报绿,原文照登。**② Sora 退役闸门真跑**:v12.173/207 的「2026-09-24 起自动把 Sora 从模型链剔除」由两条测试「锁住」—— 实际上它们只在源码里搜字符串,这个分支**从来没被执行过**,而它后天才第一次生效。现用假时钟 + 模拟网络真跑闸门翻转前后:前一秒 Sora 照常先用;零点起一次都不请求 Sora、直接走 veo;链里只剩 Sora 则不发请求直接抛出能照着改的错误。真跑顺带补上一处缺口:闸门按 UTC 零点翻,**上游若提前停服**,Sora 报的非 transient 错误(410 之类)原来会让整条链中止、veo 兜底都不试 —— 停服后上游具体回什么那天之前没法实探,不编报文去认,改为 Sora 系失败一律换下一个模型(非 Sora 模型的语义不变)。本仓 owner 的配置不走 Sora,不受影响。**③ 竞品台账刷新(约定 14 天一轮,已超期 4 天)**:9-19 四视角联网重验 18 条旧断言 + 35 个硬指标独立复核;发版前(9-22)把**变化最快**的部分由**两名独立核对者分别抓取再对账** —— 四张榜逐名次一致,且与 9-19 相比**无变化**:T2V 带音频 Gemini Omni Flash 1233 > Wan 3.0 1229 > MiniMax H3 Max 1227;T2V 不带音频 Wan 3.0 1336 领跑;图生视频带音频 MiniMax H3 Max 1195 第一;另新识别出「图生视频·不带音频」一张榜(Gemini 1369 领先)。状态项也都没变:Sora 2 API 仍定 9-24 停服(无延期)、OpenMontage 60.8k★ 仍无 EDL/AAF 与节奏门禁(收窄后的护城河断言继续成立)、H3 仍只能按量付费、Wan 3.0 仍无开源权重、MJ 默认仍是 V8.2;三天内**无新入场者**。**README 上四处已经过时的说法改掉**:榜首早已从「Wan 3.0 领先」换回 Gemini;两条自查结论(默认还在调 Hailuo-2.3、两条 Vidu 路径不一致)分别在 v12.402 / v12.403 修掉了 —— 换成如实写「H3 是计费墙挡住的」;Sora 那句的测试引用改成本版新加的真跑测试。**台账新立 12 条(C19–C30)**,其中两条本轮当场改正:qwen-image 原判「选中会抛异常」说过头了(它根本不可能被选中,真问题是死配置 + 误导文案),Sora 回落已由本版真跑锁住、只剩停服后的真机验收。**4 条旧断言被我们自己推翻**(配乐缺口 v12.410、Vidu 未钉版本 v12.403、画内中文 v12.416、默认模型 v12.402)。**④ 浏览器真机当场撞出一个早就存在的问题:健康页整页白屏**。接上探针后去真机看,页面直接被错误边界接管(`Cannot read properties of undefined (reading 'map')`)。拿未改动的 v12.448 代码在另一个端口做 A/B —— **同样白屏,不是本版引入**。真因两条叠加:`/api/health/providers` 在安全收口时加了鉴权,而本页同一组 fetch 里**只有拉 providers 那处没带令牌**(另外两处早就带了);更要命的是它**不看响应码**,把 `{message:'Unauthorized'}` 直接 setState,于是 `data.providers.map` 抛错、整页白 —— 而健康页正是出事时才去看的那一页。登录会种 7 天 httpOnly cookie,所以正常登录的人打得开;但 cookie 被隐私设置拦掉、过期、或接口回 500 时就是白屏。修:带上令牌;非 200 一律当加载失败;200 但响应体形状不对也当加载失败(**响应码不是 200 时,哪怕响应体形状是对的也不能渲染 —— 陈旧数据不许冒充实时探测结果**)。**测试 22 条**(探针 12 + Sora 退役真跑 5 + 健康页加载 5),**变异 16 个全部抓红**(探针 8:其中「去掉正向信号要求」首轮漏网,补了一条**实测原文**的负样本 —— v12.446.1 探到的 `model MiniMax-Hailuo-2.3-Fast does not support Text-to-Video mode (2013)`,它也是 2013、长得像「过了校验」—— 才抓住;Sora 5;健康页 3:其中「不看响应码」首轮因形状校验顺带兜住而漏网,补了「503 带正确形状」一条才分得开)。真机存证:健康页正常渲染,MiniMax 视频卡是黄色「受限」,文案与处置建议(换计费方式)都在。
| **v12.451.0** | 2026-09-23 | `926f19fd` | **🎬 演示工程每人一份 —— 第二个人点「导入」,原来会被跳进无权访问的项目,还顺手把第一个人那份重置成出厂**。v12.448 第三轮全仓越权粗扫 16 个路由,核实后唯一查实的一处。演示工程(v10.5.0,「0 key 也能逛完成片工作台」的首次体验入口)用的是**全站一个固定项目号** `qfmj-demo-showcase`:只有第一个导入的人能打开;之后任何人点「导入演示工程」,前端都跳到这个项目 —— 不是他的,打不开;而 `importDemoProject` 里的 `updateProjectById` 与资产 upsert **不看归属**,那一下会把第一个人的演示工程重置成出厂状态(剧本、状态、失效标记全被覆盖)。GET 也是全站一个答案:别人导入过,你的按钮就显示「已导入」。写进去的内容是写死的常量,攻击者控制不了,故定低危;但对多用户部署,首次体验从第二个人起就是坏的。**修法**:每人一份。第一个导入的人沿用固定号(单用户自部署与 9 个 e2e 用例零变化),之后的人各自 `qfmj-demo-<sha256(用户 id) 前 16 位>`;先认「已归他所有的固定号」,再认「他自己那份」,最后才去认领空着的固定号 —— 这样固定号的主人删掉项目后,已有自己那份的人刷新时不会多出第二份;两人同时首次导入、都想认领固定号时,后到的那位插入撞主键 → 重判一次落到自己那份;哈希撞上别人的项目(实际不可能)宁可报错也不写进去。导入只碰**归调用者所有**的那份。GET 改按登录用户答(未登录 = 未导入)。归档脚本的保留名单同步认每人一份的号。**测试 9 条**(真库,A/B/C 三个用户):第一个人拿固定号;第二个人拿到自己的一份且打得开;第二个人导入后第一个人那份的剧本、状态、`updated_at` 与失效标记原封不动;同一人重复导入落在同一份、资产不翻倍;固定号主人删项目后的两种走向;**两人同时首次导入都成功、各归各**(这条在 v12.449 之前的驱动上会撞「cannot start a transaction within a transaction」—— 正是 v12.449 修的那个并发事务问题);哈希撞号报错不写;项目号判定;GET 按人作答。**8 个变异全部由断言抓红**。
| **v12.449.0** | 2026-09-23 | `4b146328` | **🗃️ SQLite 事务必须排队 —— 两个事务一交错,两边全挂**。`SqliteDriver.transaction` 原来是「BEGIN → await fn → COMMIT」,注释假设 fn「只做 DB + 同步计算」—— 但全仓 13 处调用在 fn 里都会 await(传入 tx 的仓储函数、动态 import)。一让出执行权,同进程另一个事务就在**同一条连接**上再 BEGIN,抛 `cannot start a transaction within a transaction`;抛错那一方的 catch 还会 ROLLBACK,**把先开的那个事务一起回滚**,先开的那个随后 COMMIT 又因「没有活动事务」失败 —— 两边全挂。v12.448 用真库测「同一镜并发保存参考视频」当场撞出;导演台站位保存、资产 upsert、注册、邀请接受都是同一写法,v12.451 的「两人同时首次导入演示工程」也独立撞出同一个错。**修法**:事务按到达顺序排队,前一个结束(不论成败)才开下一个;前一个失败不影响后面排队。嵌套(fn 里又调 transaction)同一连接做不到,排队会自己等自己 —— 用 AsyncLocalStorage 认出来**立即报错**,而不是挂死。**排队的代价核过**:逐一检查 13 处事务体,除 `tx.*` 外只有传入 tx 的仓储函数和一个本地动态 import,**没有任何一处在事务里等网络**,排队不会把全站写库拖慢。已知仍在:事务开着时,别处不带 tx 的普通读写也走这条连接、会落进这个事务(better-sqlite3 单连接的固有限制,与本修无关)。**测试 4 条**(真库):10 个并发事务(事务内真让出执行权)全部成功且依次执行 —— 每个都看到前一个已提交的结果;一个失败只回滚它自己、排在后面的照常提交;嵌套立即报清楚的错且队列没被堵死;真实调用方同一镜并发保存导演台站位 6 次全部成功、只留一行。**正反两侧都验过**:换回原驱动,这 4 条全部变红。**变异 5 个**:不排队 / 不认嵌套 / 失败不回滚三条当场抓红;另两条(`then(run, run)` 改成 `then(run)`、队尾不吞异常)**单独改属等价变异** —— 两处中任一保留,「前一个失败不挡后面」都还成立;两处同时去掉即被 3 条测试抓红。
| **v12.448.0** | 2026-09-22 | `68027489` | **🎬 参考视频动作迁移:给一镜挂一段参考视频,出片照着它的动作与运镜来(MiniMax H3)**。每镜可挂一段参考视频(镜头工坊每张镜头卡新增「动作参考」):**本地上传**入库即 ffprobe 校验(2–15 秒、256–5760px、宽高比 0.4–2.5、24–60fps、H.264/H.265、MP4/MOV,按官方契约),超过 15 秒自动截取前 15 秒并告知;**公网链接**过内网拦截(SSRF)后原样交给 MiniMax。出片时 H3 走「参考生视频」:`content[]` 带 `reference_video` + 参考图 —— 官方规定参考素材与首帧**互斥**,所以分镜图降级为第一张参考图保构图,其后是各角色正面、角度图按轮次、场景参考,9 张封顶、请求体预算封顶;官方没有「学动作 / 学运镜」的字段,只能写进提示词(措辞集中在一处)。本地文件 MiniMax 访问不到(localhost),发送时转 base64(内联上限 20MB,给参考图留请求体余量)。**用户当前是 Token Plan,H3 只能按量付费** —— 所以本版的重点是**没 H3 时不丢镜、不悄悄没了**:H3 报套餐不支持(2013)→ 回报「参考视频没用上」并说明要按量付费,去掉参考视频按原流程重出(有角色的回到 S2V-01,角色一致性还在);H3 因别的原因失败(如参考视频被拒)同样回报原因并重出,且**不**记成「H3 不可用」。**H3 可用性进程内记 30 分钟**:确认套餐不支持后直接走 legacy,不再每镜白打一次必败请求(v12.446 起每镜一次),告警只打一次。三条出片路径(整片生成、单镜重生 / 自愈 / 每日重跑、审片重生)共用一个注入口。**结局要看得见**:`refVideo`/`refUsage` 事件前端还没人接,所以同时发 `agentTalk`(创作页对话流)与 `status`(项目页重生状态行),被忽略的另进服务端日志(每日重跑只看日志)。**更正 v12.447**:S2V-01 默认只锁第 1 个角色(`MINIMAX_S2V_MAX_SUBJECTS`),第 2 个角色连正面图都不发 —— v12.447 按「每个角色 1 张」上报,少报了;原测试锁的正是这个错行为,已改。**对抗复查三轮挖出并修掉**:第一轮 ——「整阶段重做视频」路由从没设项目号,导演台站位(v12.440 起)与参考视频在这条路径上都被静默跳过;H3 失败回落 S2V 后角度图丢失没有上报;上传在读完整个请求体后才判大小;出错把内部报错原文回给客户端。第二轮 —— **工作流执行路由从不校验 `input.projectId` 归属(越权写入,早就存在)**:真跑会把结果写进该项目,任何登录用户填别人的项目号就能写,现两个执行路由(普通 / 流式)都要求编辑权限;执行器只把项目号放进工作流输入、没交给编排器,真跑模式下导演台站位与参考视频同样被静默跳过;分块上传(不带 Content-Length)能绕过大小检查把任意大的请求体读进内存 → 改为边读边计、超限即断;上传入库不限并发 → 同时最多 2 个,多出的在读请求体之前就 429;内网拦截把服务端解析出的内网 IP 原样回给客户端(可用来枚举内网)→ 只进日志;去掉参考视频重出时敏感词重试次数没清零,重出少一次机会;内层递归已经兜底失败、外层 catch 再兜一遍 → 重复请求、重复回报;读本地参考视频改异步(20MB 同步读会卡住事件循环),读盘异常转成「用不了 + 原因」而不是把出片打挂。第三轮 —— **一键出片 `/api/create-stream` 同样从不校验 projectId 归属(越权覆写,早就存在)**:项目已存在就 UPDATE 并往里写全套素材,填别人的项目号就能用自己的创意覆写别人的项目;现在已存在的项目要求编辑权限、且在任何计费调用之前就拒,新建(前端先生成 `proj-<时间戳>` 再传上来,那时项目还不存在)照常放行;上传入库全程改异步读写(50MB 同步写会把整个进程卡住几百毫秒);链接那条 JSON 分支也边读边计、16KB 封顶;空请求体 / 格式坏了的 multipart 回 400 而不是 500;Fast 兜底的敏感词重试同样会被外层再兜一遍 → 发第三次请求(早就存在),同法修掉;MiniMax 视频接口不可用(非官方端点)时参考视频的结局也要回报,不然换了引擎用户却不知道参考视频没用上。第三轮顺带对全仓做了一次同类扫描:粗扫出 16 个接收 projectId 却没有显式归属守卫的路由,逐个核实后**只查实 1 处低危**(演示工程是全站单例:第二个人点「导入」会被跳到无权访问的项目,还顺手把第一个人那份重置成出厂 —— 单独发 v12.451),其余都有别处的守卫或读写不落到他人数据。第四轮(只审第三轮的修复,4 个视角各带 2 名默认驳倒的复核者)—— 越权守卫、回落递归两个视角 0 条;查实并修掉 3 处:**部署漏装 ffprobe 时每次上传都回 422「文件损坏」**,每个用户都被告知自己的文件坏了、去反复转码一个好文件 → 进程起不来 / 被超时杀掉算服务器故障回 500 并点名缺的工具,正常跑完但退出码非 0 才算文件的问题(截取时缺 ffmpeg 同理);存储写不进(磁盘满 / 权限)原来也回 422「保存失败」→ 同样改为服务器故障 500;出片发送路径 `refVideoToEngine` 残留一个 `fs.existsSync`(第三轮「全程异步」的同类漏修)→ 改用异步 stat 判存在,读盘报错原文(带本机路径)也不再经事件推给前端,只进服务端日志。**突变验证**:75 个注入点(每点一个缺陷)全部由断言拦住 —— 第二轮的 56 个首跑漏 4 个,各补一条测试:参考模式未给画幅应为 16:9;并发出片时 A 镜净化重试途中 B 镜刚把 H3 记成不可用,重出仍要清零重试次数;27MB/24 秒的原片要先截取再判内联上限(截完 17MB 收下);测试桩熔断改为放行收尾,让失控递归由断言判红而不是挂到超时;第三轮修复另加 11 个,首跑全中(其中「空请求体单独说明」一条原本与「解析失败回 400」等价,补了报错文案断言才分得开)。第四轮修复另加 8 个,全中(发变异时一处锚点因反缩进没命中,改锚重跑后确认)。**测试 102 条**(规则层用 ffmpeg 现场生成真视频测上传与截取,并断言入库全程没有同步读写;缺工具用不存在的路径真 spawn;服务层模拟网络逐一走通每种结局;三条出片路径用假引擎真跑编排器;路由直接调 handler;界面真渲染真点击;存储层对真 SQLite 往返 —— 路由测试把存储层换成了假的,浏览器真机又因预览环境里 `/api/projects/[id]/*` 全部 404 碰不到这条路由,故补)。**真库测试另撞出一个早就存在的问题**:SQLite 驱动的事务不能并发(全仓 13 处),单独发 v12.449。旧测试同步:v12.446 每条重置 H3 记忆;v12.402 的回落窗口断言按新顺序仍成立。浏览器真机:镜头工坊 11 镜都有入口、弹窗打开并如实说明 H3 前提;保存一步因预览环境路由 404 未能在浏览器里走通(同一环境下 `stage`、`assets` 等老路由同样 404),由路由 handler 测试与真库测试覆盖。**已知未做**:真跑要等按量付费 key;每镜只挂一段(官方允许 3 段合计 15 秒);前端专门的「参考视频被忽略」徽标。 |
| **v12.447.0** | 2026-09-19 | `bb79ac84` | **🧑‍🤝‍🧑 多角度角色参考图:把一条全程被剥的链路接通,引擎吃不下的必须说出来**。装配层 `lib/elements-registry` 从 v12.12 起就会读 `refs` 并产出 `reference_image_urls`,可灵 service 也早接了 —— 但**两道白名单**(`lib/locked-characters` 与编排器 `setLockedCharacters`)只放行 name/role/cw/imageUrl/traits,角色的角度图永远到不了出片端:造好没接线。本版:① 创作页角色锁定区加「角度图」入口(有正面图才出现;侧面 / 3-4 侧 / 背面;每角色最多 3 张,与可灵「1 正面 + 3 参考」同口径;与正面图也去重;满 3 张收起入口;上传失败不留 busy);② 两道白名单放行 `refs`(只收 http(s)、不认识的角度标签归 `detail` 而不是把图丢掉、去重、截前 3),编排器的 `LockedCharacter` 类型补上 `refs`,去掉一串 `as any`;③ 整片生成把 refs 交给元素注册表;单镜重生 / 自愈 / 审片重生的主体参考也带上(修前重生只拿到正面图 —— 同一项目两套身份锚点);④ **新增 `reportRefUsage`**:引擎吃不下的张数如实写日志并发 `refUsage` 事件 —— 可灵未开 Elements 每角色 1 张、MiniMax S2V-01 每角色 1 张(H3 能收 9 张,但**只能按量付费**,Token Plan 订阅调不动 —— 同日核官方文档并实探;S2V-01 同日实探过了套餐校验)。上报覆盖整片生成(MiniMax / 可灵)与两条重生路径,只在**真轮到该引擎**时报,且**上报本身出错绝不拖垮出片**(推送回调抛错也照常出片)。取图 / 判定逻辑放在 `lib/`(`angleRefUrls`、`findAngleRefs`、`refDropFor`),编排器只留调用点 —— 首版净增 47 行顶破了 v12.262 的「编排器 < 4500 行」预算,收敛后 4489 行。**两条旧测试随之调整**:v12.440 重生测试的假编排器补上真的 `reportRefUsage`;v12.409「Veo 时长不再写死」的锚点 `engine === 'veo'` 在去注释后的源码里出现两次、命中的是引擎标签小函数,离 `generateExtended` 1711 字符(窗口 1800)—— **之前是侥幸通过**,改锚到真分支 `engine === 'veo' && this.veoService` 并用「写回 duration: 8」的注入验证窗口仍罩住真分支(锚点门禁没拦,因为 ORCH 外包了一层 strip(),已另开待办)。**测试 31 条**:白名单、取图助手、装配层、存库 → 重生往返、上报三条路径真跑(含「没轮到 MiniMax 不报」「上报出错不影响出片」)、`runVideoProducer` 整片生成用假引擎真跑(MiniMax、可灵、两个角色不张冠李戴、无角度图不报)、界面 7 条。**变异两轮全部由断言抓红**:首轮 22 个漏 2 个(A8 编排器建注册表不给 refs —— 原测试手动喂 refs;A12 整片生成不上报 —— 原测试只扫源码有没有这个调用,我给重生路径加同名调用后它就失明了),补整片生成真跑后抓红;重构后第二轮 11 个漏 2 个(新助手不过滤非 http、按名字找错人),补直测 + 真跑后抓红。**浏览器真机**(本地测试令牌,不碰账号):无正面图不出入口、设正面图后出入口;1440 与 1024 宽下控件都在卡片内且点得到;1024 宽下角度下拉框比区块宽 4px,已加 `max-w-full`。真机顺带挖出两个**早就存在**的问题(已另开待办,不在本版改):1024 宽下「用 URL → 抓取」那一行溢出到隔壁卡片底下、按钮点不到;角色锁定区在 setSlots 的更新函数里调父组件 onChange(React 报「渲染中更新别的组件」)。**已知未做**:单镜重生走可灵时连正面图都不传(早就如此,可灵欠费无法真跑);MiniMax 真正吃多图要等 H3。 |
| **v12.446.1** | 2026-09-19 | `24ca2188` | **🩹 「模式不支持」不是「套餐不支持」—— v12.446 自己引入的一处误判**。补审计缺口时实探了 Hailuo-2.3-Fast 的套餐校验,拿到的报错原文是 `model MiniMax-Hailuo-2.3-Fast does not support Text-to-Video mode (2013)` —— 它同时含 `not support` 与 `model`,正中 `isModelUnavailableError` 里 v12.402 留下的那条宽规则。v12.402 时这条规则只影响视频回落;**v12.446 把用量看板也接到同一判定上**,于是看板会把这种**参数/模式错误**记成 `model_unavailable`(「套餐不支持该模型」)—— 看板旧正则不会这么判,这是 v12.446 带进来的回归。将来开了按量付费,H3 若报「某模式不支持」,回落也会误判成套餐问题、打出「当前套餐用不了」的假告警并换成旧模型。修法:删掉那条宽规则 —— 套餐类英文原文都带 `token plan` 字样,已由 v12.446 的「计费方式/套餐 + 不支持」规则覆盖。测试 1 条(用实探原文,判定与看板两侧都断言;并确认真正的套餐英文原文收紧后照样命中);把宽规则加回去 → 该条抓红。 |
| **v12.446.0** | 2026-09-18 | `07ea2656` | **🩹 H3 回落认不出中文报文 —— 套餐里每天 3 条 Hailuo-2.3 白白浪费了 16 天**。v12.402 把默认视频模型翻到 `MiniMax-H3`,并设计了「套餐用不了 H3 → 回落 legacy 的 Hailuo-2.3 并大声告警」;但 `isModelUnavailableError` 只认 `2061` 和英文 `not support`+`model`,而国内站实际返回的是中文 `invalid params, TokenPlan 或 Credit 暂不支持 MiniMax-H3 系列模型 (2013)` —— **回落分支上线以来一次都没被走到过**。认不出 → 抛错 → 可灵欠费、Veo 额度尽 → 整镜落成 Ken Burns 占位片 → 重跑脚本判「欠费」整轮停。每日重跑日志实证:9-03 ~ 9-18 H3 尝试 180 次、报 2013 共 79 次(其余为网络中断)、**回落告警 0 次、真视频 0 条**;9-02 及之前 Hailuo-2.3 每天稳定出 3 条后报 2056,正是套餐页写的「3 条/日」。**不能按 2013 认**(它是 MiniMax 的通用「参数错误」码,Fast 纯文生也报 2013),改按语义:「计费方式/套餐」与「不支持」同时出现才算;「已达到 Token Plan 用量上限」有前者没后者 —— 那是额度,换模型没用,必须不命中。**两份实现收成一份**:用量看板 `lib/api-usage-tracker.ts` 的 `model_unavailable` 原来另写一份英文正则,同样认不出,看板上只是一次无告警的普通失败;现在直接调同一个判定。回落告警原来叫人「去控制台开通 H3」—— 但官方文档写明 H3 只走按量付费,Token Plan 订阅与积分都开不了,照着找会白找一圈,改成说实话并给出 `MINIMAX_VIDEO_MODEL` 的关法。**测试 10 条**:判定用生产日志原文(裸报文与 service 实际抛出的整串);回落链用模拟网络真跑 `generateVideo` —— H3 报套餐不支持 → 第二次请求改用 Hailuo-2.3 打到 v1、首帧不丢、拿回视频地址、留下告警;两个模型都报不支持时直接抛错、不无限递归;另一侧 H3 报额度用尽 → 转 Fast、不走 legacy;无关错误照常抛出;看板与回落逐条同判。**9 个变异全部由断言抓红**(其中一个初版注入本身有语法错误,红的是「测试文件加载失败」而不是断言 —— 已换合法注入重验,确认是「留下告警」那条拦住)。 |
| **v12.445.0** | 2026-09-18 | `2d41fc7b` | **🧍‍♂️ 3D 人偶也吃姿态 —— 顺带修掉「躺着的人被判成大远景」**。v12.443 让几何与草图认了姿态,**3D 人偶没跟上**:坐着的人在提示词里坐着、草图里矮一截,3D 视口里却还站得笔直 —— 又一次「同一语义两套口径」,而用户正是照着 3D 视口摆位的。**① 人偶与草图共用同一张骨架表**(`mannequin3D` 读 `POSE_SKELETONS`):按 `heightFactor` 变矮、按骨架长出四肢、躺倒整体放倒(绕 X 轴 −90°);头不随姿态缩放(坐下只是矮了,头没变小);四肢 z 一律 0 贴在人偶正面平面 —— 单目参考图本来就给不出深度,假装有 z 是无依据的精细。没设姿态的人偶与 v12.440 完全一样(不暗示没有的东西)。**② 浏览器实测撞出一个语义错误**:躺下的人被判成「大远景」。他并没有变远变小,只是躺着 —— **景别说的是「这个人在画面里占多大」,由身量与距离决定**。改成:轮廓(screenTop/Bottom)用姿态压过的高度,景别用站立身量。v12.443 那条「躺下会改变景别」的断言随之迁移(它锁的是个错的行为)。**验证**:新增 13 条测试(人偶高度与几何反解出的头顶逐一对上、举手的肢体点高过头顶、指向横向伸出身宽、躺倒标记与高度、四肢无假 z、同距离下坐跪蹲躺与站立景别相同而距离真变了景别照常变、3D 视口确实读同一张表);10 个变异全部抓红。真机实测:导演台里给两人分别选「举手」「坐着」,3D 机位视角里一个举着手、一个明显矮一截且腿前伸;改「躺倒」后自由视角里人躺在地面上。 |
| **v12.444.0** | 2026-09-18 | `f90d0b67` | **📷 上传一张参考照片,本机认出姿态与朝向 —— 推理全在浏览器里,照片不上传**。v12.441~443 让姿态能选、能进提示词、能进草图,但「摆一个具体姿势」仍要用户从 15 项里自己挑。本版接 MediaPipe Pose Landmarker(Apache-2.0):选一张参考照 → 33 个人体关键点 → 直接回填 `posePreset` 与 `facingDeg`。**① 判据写成纯函数**(`lib/pose-from-photo`):躯干与竖直方向的夹角判躺;髋-膝-踝的折叠角判坐(≈90°)/跪(一腿折死一腿伸)/蹲(双膝 <60° 且髋下沉);腕相对肩与脸的位置判举手/指向/掩面/抱臂/叉腰;朝向按「肩宽/躯干」比例 + 鼻子可见性判正面/侧面(朝画面左右)/背面。这一层不碰模型、不碰 DOM,所以能用**构造出来的关键点**逐姿态验证 —— 否则「认得准不准」只能靠肉眼看一张图。**② 认不出就明说,绝不瞎猜**:关键点缺失/可见度过低 → 返回 null;置信度 ≤0.45(典型是「侧身但看不到脸」,左右各一半概率)→ 界面标「不太确定…请自己核对」。失败面分三种话:设备不支持 wasm / 这份部署没带模型 / 照片里没认出人 —— 三者的解决办法完全不同。**③ 资产自托管**:模型 5.8 MB 随仓库提交(构建期不联网下载 —— 联网下载等于给镜像构建加一个断网就失败的步骤);12 MB 的 wasm 由 `npm run prebuild` 从 `node_modules` 拷、不入库;只带 SIMD 版(non-SIMD 回退版再要 11 MB,而 SIMD 自 2021 年起是各浏览器默认)。MediaPipe 约 600 KB JS 只在点按钮时动态加载,不进项目页首包。**④ 真机实测**(单测证明不了这一步):拿库里一张真人四视图转身表切成正/侧/3-4/背四张,在浏览器里跑真实推理 —— 正面→正面朝镜头、侧面→侧身朝画面左(与原图一致)、3-4→正面(我们的朝向只分四档,合理)、**背面→认不出人**。背面认不出是模型的已知局限,提示词里如实写明「背面照通常认不出,可直接手动选『背对镜头』」,而不是让用户反复换图。**验证**:新增 24 条测试(15 条纯函数逐姿态/逐朝向 + 9 条接线与失败面);14 个变异注入全部抓红(含认不出也硬猜、躺判据反、坐蹲不分、朝向左右反、背对当正面、置信度取高的、低置信度不提示、失败一句话通吃、静态导入把 12 MB 打进首包等)。 |
| **v12.443.1** | 2026-09-18 | `f959f932` | **🩹 零回归基线锁错了东西:PNG 文件哈希锁的是 zlib 实现,不是图像**。v12.443 那条「没设姿态的草图逐字节不变」本机绿、**CI 红** —— PNG 的像素数据经 zlib 压缩,不同 Node 版本的 zlib 输出不同(本机 Node 25 / CI Node 22),于是同样的图算出两个文件哈希。改成锁**解码后的像素**(测试里本来就自带极简 PNG 解码器),内容一样就过、画错一个像素就红 —— 反向验证:把头部半径改 1.2 倍,立刻红。教训与 v12.442.1 同源:**断言要对着你真正关心的那个东西**,别对着它的某种编码形式。 |
| **v12.443.0** | 2026-09-18 | `a1cbe8ac` | **🖼️ 姿态进构图:草图里画出四肢,坐/跪/蹲/躺的人头顶真的更低**。v12.441 让姿态进了提示词,但布局草图里每个人仍是「一根矩形 + 一个圆头」—— 坐着的人和站着的人画出来一模一样。而**草图锁正是拿这张图去约束构图的**:图里站着、提示词里坐着,两个口径打架,模型听谁的都不对(同一语义两套口径,本仓栽过五次)。新增 `lib/pose-skeleton`,只做两件**几何事实**而非美术的事:**① 头顶高度**(`heightFactor`:坐 0.72、跪 0.62、蹲 0.55、躺 0.18 倍站立身高)进 `projectScene` —— 坐下的人在画面里真的更矮,**景别判定跟着变**(实测躺下会从全景变成另一档);脚底不动(人不会浮起来)。**② 四肢折线**:15 个姿态各一套骨架,坐标用「身宽为单位的 x + 脚底 0 头顶 1 的 y」,草图层只做缩放,不需要知道任何姿态细节。躺倒特判为横向躯干(四肢留空 —— 实渲对比过,那个尺度下再画两条斜线反而像从身体里伸出去的东西)。**零回归有硬证据**:没设姿态的草图与 v12.442 **逐字节相同**(基线 SHA256 在改代码之前先取好,写进测试)。**实渲三张对照图逐个看过**,修掉两处:躺倒原本用压缩后的高度算横向长度,缩成了一个小方块;站立手臂藏在躯干里看不出来,外扩到 0.68 身宽。**验证**:新增 10 条测试(骨架表自检、高度分档单调、未设=站立、景别随高度变、举手在头顶线以上有墨而站立那里是空白、指向横向更宽、躺倒横跨 ≥ 站高的 0.7 且在地平线以下、15 个姿态两两不同图、以及那条逐字节零回归);测试里自带一个极简 PNG 解码器直接查像素,不靠肉眼。9 个变异注入全部抓红;其中一条「躺倒用压缩后高度」最初没被抓到 —— 补了「躺着的长度不该明显短于站着的身高」才红。**还被 v12.315 立的分工守卫拦了一次**:画线函数里用了 `Math.hypot` 算步数,而那道守卫禁止草图层出现几何运算;没去放宽守卫,改用光栅化本来就该用的 DDA 步数(`max(|dx|,|dy|)`),零回归哈希不变。 |
| **v12.442.1** | 2026-09-18 | `dfe1a2d0` | **🩹 v12.442 那条新测试在 CI 里红了 —— 是测试本身的问题,不是修复**(同一次推送的 Docker 构建是绿的,证明构建锁竞争确实修掉了)。子进程里用 `import()` 拿 `lib/db` 的 `dbPath`:本机 Node 25 直接给命名导出,**CI 的 Node 20 下 tsx 走 CJS 互操作,命名导出落在 `default` 上** → `m.dbPath` 是 `undefined`,而我那句 `'DBPATH=' + m.dbPath` 把它拼成了字符串 `"undefined"`,于是断言拿到的是一个**看起来像正常返回值的假值**。修法:两种形态都认(`m.dbPath ?? m.default?.dbPath`),并且**取不到就显式失败** —— 打印模块的导出键、退出码 3,而不是继续往下拼字符串。这是老毛病的新变体:失败要长得像失败,不能被拼接成一个合法字符串蒙混过去(同 v12.433「流水线不许把失败标成完成」)。本机没有 Node 20 无法复现 CI 那侧,所以这一版的判据是「两种互操作形态都能过 + 取不到时必红」。 |
| **v12.442.0** | 2026-09-18 | `720a23ed` | **🔒 构建期不许几十个进程抢同一个 sqlite 文件 —— 修掉一个会周期性红掉 Docker 的竞争**。v12.441 推上去后 `build (linux/arm64)` 红了:`Failed to collect page data for /api/auth/login` ← `SqliteError: database is locked`。根因:`next build` 会起多个 worker 并行收集页面数据,**每个 worker 导入 `lib/db` 都会对同一个 `data/qfmj.db` 建表 + 跑迁移**(全是写操作);arm64 镜像跑在 QEMU 下更慢,`busy_timeout = 5000` 等不到锁就抛。**这是竞争不是必现** —— 所以「重跑一次绿了」不能当修好了(v12.440 的 Docker 就是侥幸绿的,代码路径完全一样)。**修法**:构建期给每个进程一个临时目录里的独占库(`NEXT_PHASE=phase-production-build` 或显式 `QFMJ_EPHEMERAL_DB=1`),进程退出即清理;Dockerfile 只在 builder 阶段置位,runner 阶段绝不带 —— 运行时必须用真库。构建期本来就不该读生产数据:页面全是 `force-dynamic`,库只是被 import 带起来的副作用。**证据**:本机跑一次生产构建,`data/qfmj.db` 的 mtime/大小**全程未变**,同时 `/tmp` 下出现 **32 个**构建期临时库目录 —— 那 32 路写竞争正是 arm64 上锁死的东西。**验证**:新增 4 条测试(运行时仍是 `cwd/data/qfmj.db`;三个进程三个独占库且都不在数据目录下;子进程退出后临时目录自己消失;Dockerfile builder 置位/runner 不带且在 `npm run build` 之前);6 个变异逐一注入全部抓红;**顺带被自家门禁抓到一次**:Dockerfile 那条断言原本对 runner 段只有「不包含」,窗口切错(切成空串)会静默通过 —— `fake-green` 门禁当场报出来,补了正向自证(builder 段含构建命令、runner 段含运行入口)。测试刻意不赌「共享时一定锁死」(快盘上可能侥幸不锁),而是锁住那个必要条件:共享模式 N 个进程指向同一文件,构建模式各自一个。**过程中自己踩的坑**:用 `git checkout -- lib/db.ts` 回滚变异时,把本版刚写的改动一起还原了,重写了一遍 —— 这个坑在 2026-09-08 犯过同样一次。 |
| **v12.441.0** | 2026-09-18 | `1bb3d15d` | **🧍 姿态预设进提示词:朝向说「身子朝哪」,姿态说「在干什么」**。v12.440 让朝向进了几何,但「他在做什么」仍只能靠用户在剧本里写 —— 导演台摆完位,人物永远是站着的。本版给每个人物一个**固定词表**的姿态:站立/坐着/跪地/蹲下/躺倒/走动/奔跑/倚靠/抱臂/叉腰/举手/指向/伸手/掩面/低头(15 项),中文给界面、英文进提示词。**① 为什么不给自由文本**:提示词全链路是英文(v12.6.1 定的口径),用户填中文动作会被视频模型当画面文字渲染出来 —— v2.22 那次 CJK 乱码就是这么来的;固定词表顺带保证同一个动作在每一镜的说法一致。英文短语刻意只描述身体,不含情绪(走剧本 emotion)与镜头语言(走 cinema 那套),三者在提示词里各占一段,混着写会互相打架(v12.9.1 在角色外观上栽过同一类)。**② 挂在各自那半句上**:`. Staging: 林晚 … kneeling on one knee; 陆沉 … pointing with one arm extended` —— 不是笼统追在末尾(多角色时提示词引擎分不清是谁的动作)。画外的人不进提示词,他的姿态自然也不进。**③ 脏值一律当未设**:词表外的 id、非字符串、甚至 `toString` 这种原型链上的键都返回 null(用 `hasOwnProperty` 判,不用真值判);`POST /stage` 直接按白名单拒 400,而不是落库后由几何层静默忽略 —— facingDeg 在 v12.440 就栽过这个,存了却没生效、全程零报错。null/空串视同清除。**④ 零回归**:没设姿态的舞台,提示词与中文描述与 v12.440 逐字相同。**验证**:新增 11 条测试(词表自检、提示词/描述、多人各自挂载、画外不进、路由白名单与清除、落库读回、弹窗选择与清除);10 个变异注入,8 个当场抓红,2 个是我写坏的无效变异,另有 1 个真缺口(清除姿态若落到某个默认值,原断言仍通过)—— 已把断言收紧成「与没设过时逐字相同」后抓红;真浏览器实测:在 16:9 真项目里给两人分别选「跪地」「指向」,提示词立刻变成对应英文,保存后重开仍在,服务端 `/stage` 返回的提示词与界面显示完全一致。**下一版**:关节角与布局草图里的四肢线条(让姿态不仅进提示词,也进构图草图)。 |
| **v12.440.0** | 2026-09-18 | `44a090b0` | **🧭 人物朝向真的进了几何与提示词;顺手补上「导演台只对整片生成有效」这个更大的洞**。`StageActor.facingDeg` 从 v12.316 起就在类型里,**全仓没有一处读过它** —— v12.439 发版时为了不误导,3D 人偶刻意不画朝向。**① 朝向进几何**:`facingOf` 按「人→机位」方向分档(正面 ≤30°、3/4 侧 ≤70°、侧面 ≤110°、3/4 背 ≤150°、其余背面);参照刻意不是机位光轴 —— 广角画面边上的人视线偏光轴可超 30°,朝向夹在两者之间时左右判反(对照 `THREE.PerspectiveCamera` 逐点验证)。朝画面哪边按朝向在视线右法向上的分量算。**「望向谁」±25° 内取角度偏差最小者**(人工复核时改的:原来取最近的 —— A 正看着 6 米外的 C、3 米外偏 20° 站着 B,会说成 A 在看 B),角度相同比距离、仍相同按数组顺序,同一份数据结果恒定;且只在对方**也在画内**时进提示词(锥内最近的若是画外的人,会把画内真正被望向的那个挤掉)。**旧数据逐字不变**:没设朝向的舞台,提示词与中文描述的期望字符串取自 v12.439 的代码本身,投影结果连 `facing` 键都不带。**② 界面**:俯视图人物外圈拖动转朝向(吸附 5°)、双击清除、箭头只在设了朝向时画;**键盘可达**(Tab 聚焦人物/机位、方向键移动、Shift 大步、Q/E 转 15°、Delete 清除、焦点圈);拖动中移出俯视图改为捕获指针继续拖(修前半路取消,朝向停在中间角度且无提示),**且只认发起拖动的那根指针**(多指触摸时第二根手指离开会被捕获,之后它的移动在拖人 —— 第二轮审查两票确认);3D 人偶只在设了朝向时画面罩与胸牌。俯视图视野扇形改为**世界坐标**后再映射:俯视图横向 28.3px/m、纵向 21.4px/m,修前直接在像素里按角度画,扇形边上的人颜色(几何判定)与位置(扇形)对不上。**③ 落库校验**:`POST /stage` 逐人校验 x/z 为有限数字、`facingDeg` 为 null 视同清除、其余非数字一律 400 —— 修前传 `facingDeg:"90"` 会原样落库、几何层按「未设」忽略,用户存了朝向下次打开却没了,全程零报错。**④ 比朝向本身更大的洞**:导演台站位此前**只有整片生成读**。单镜重生(两条路由)、自愈补拍、4K 重渲、剪辑师烤字重生四条路径直接拿分镜描述出片,完全不看导演台 —— 而保存成功的提示是「该镜后续出片会带上这份站位」,用户最常见的用法恰恰是「摆完位 → 重生这一镜」。新增唯一注入口 `withStageDirective`(主管线也改走它):没摆过位原样返回、已带站位句不重复追加、读舞台抛错不打挂出片;`regenerateShot` 经 `options.projectId` 拿项目(不调 `setProjectId`,避免连带改动评分回写等行为)。**验证**:新增 58 条测试(几何分档/左右/望向/体检/键盘/拖动/路由校验/四条出片路径真跑);**49 个变异逐一注入全部抓红**(含左右符号反、光轴算法、容差边界、未设朝向也带 facing 键、扇形回到像素算角度、路由不校验、重生不注入、指针不认 id 等);**人工几何复核**:随机 3,323 个场景的左右判定与 three.js 投影 0 错、正侧背分档 0 错、±180/超大值归一化正确;真浏览器实测 16:9 真项目(两人面对面 → 箭头相对、3D 面罩相对、提示词 `in profile, facing screen right toward 李长安`、保存后重开仍在、双击清除后回到旧格式)。**本版没做**:被画外的人遮挡时提示词仍会写出那个人的名字(v12.316 起如此,站在画面边缘的人确实可能挡住一部分,单独权衡);姿态(四肢)控制按调研结论另起版本。 |
| **v12.439.0** | 2026-09-13 | `cadce0cb` | **🎬 3D 导演台 —— 先把几何修对再上 3D;真浏览器又挖出项目页从 v10.6.0 起就拿不到画幅**。用户选定「先做 3D 导演台」(多角度角色参考图调研后搁置:引擎侧参考链路是断的,等可灵充值后验证 `KLING_ELEMENTS` 再做)。**① 几何先修,三处都已数值验证**:投影按角度线性 `rel/half`,真实镜头是直线透视 `tan(rel)/tan(half)`(18mm 下错 4.3% 画幅);**完全不看画幅**,永远按 36×24 算 —— 真库 32 个项目 27 个是 9:16,35mm 下真实水平视角 32.3° 被算成 54.4°,画外的人被写进提示词;纵向用水平距离而非沿光轴深度(偏 20° 差约 6%,设计评审与三份独立设计全漏了这条)。改为长边固定 36mm 的等效底片,`tests/v12-439-stage-3d-parity` 直接对照 `THREE.PerspectiveCamera.project()`(5 画幅×5 焦段×3 朝向×2 机高×4 偏移 = 600 点,误差 < 1e-3),不自己写参考投影器(那是拿同一公式比同一公式)。**② 画幅注入口只有一处**:`withProjectAspect`(`getStageScene` 内部也走它),编排器注入提示词、`/stage` GET 与 POST dryRun 体检、舞台草图四条路同源;画幅**不存进舞台数据**(项目改画幅,舞台跟着变)。**③ 草图尺寸**:新增 `frameSize`(面积恒定,宽高比取自同一个 `sensorDims`),16:9/9:16/1:1 三档与修前逐像素相同。导演台**从来不传** `aspectRatio`,修前竖屏项目的舞台草图一律出 960×540 横图 —— v12.317 那条「画幅比跟随请求」断言只查了源码里出现 `'9:16'`,一直绿;已迁移为真跑路由读 PNG 头(9:16 项目 → 540×960,请求体传 16:9 也不改)。**④ 3D 视口** `components/project/stage3d-viewport.tsx`:three 0.186 + @react-three/fiber 9.7 + drei 10.7.8;「机位视角」就是出片那台相机(竖向 fov、−yaw、画布宽高比 = 画幅),「自由视角」可在地面拖人/拖机位(射线求交地面,范围与俯视图同一片场);按需渲染不占 GPU;`next/dynamic` 按需加载,弹窗不静态导入(否则 three 进项目页首包)—— 生产构建核对过:3D 相关 chunk 948KB,不在项目页 `entryJSFiles` 里;Mac 上 WebGL2 经 ANGLE 走 Metal。r3f 9.7 要求 react < 19.3,npm 曾把 `^19.2.8` 解析到 19.3.0 报 ERESOLVE,react/react-dom 收紧为 `~19.2.8`。**⑤ 真浏览器实测抓到 5 个单测全绿时没暴露的问题**:(a) **项目详情接口从未返回 `aspect`** —— 项目页 `isVertical`、安全区叠层、素材竖图样式自 v10.6.0 竖屏优先起对所有竖屏项目都是死的,导演台只是最新受害者;接口补字段(空值按库默认 16:9)。(b) r3f v9 在 async `configure()` 里建渲染器,`new WebGLRenderer` 抛错成了未处理的 Promise 拒绝,**ErrorBoundary 接不住**(`onCreated` 里 throw 同理),强制 getContext 失败实测只剩一块黑框;改为 `gl` 工厂捕获、通知退回平面预览并附一句说明。(c) 探测结果只在为真时切 3D,StrictMode 二次执行时出现「只剩平面选项却显示 3D 黑框」;改为两个状态一起定。(d) 竖屏平面预览白底铺满宽框、渲出的 540×960 草图用 `w-full` 撑满整屏;盒子按画幅收窄、草图限高。(e) 弹窗自画的关闭按钮与 `DialogContent` 自带的叠在一起(v12.318 起)。**验证**:新增 38 条测试(几何对拍 9 + 接线/渲染 29);23 个变异逐一注入全部抓红(含注入口漏接、POST 用裸场景、视锥 tanH/tanV 对调、lens 相机用水平 fov、payload 存画幅、弹窗静态导入 three、渲染器失败直接抛等);真浏览器在复制出的库上实测 9:16 项目(视角 32°、画框 540:960、渲出草图 540×960)与 16:9 项目(54°、960:540、项目页横屏画框不变);机位转 10° 后 3D 机位视角与平面预览中人物位置逐像素一致(x≈531、头顶≈287、脚底≈397),与公式 screenX = −0.609 吻合。**没做的**:动作/姿态精确操控(L0 引擎 API、L1 Apple Vision 常驻进程另起版本);人物朝向 `facingDeg` 几何层未使用,3D 人偶不画朝向,免得暗示它会进提示词。 |
| **v12.438.0** | 2026-09-13 | `cdfb2432` | **🧹 删掉 `skills/` 里一块从没被接上的「集成」—— 运行时目录只留运行时的东西**。v12.437 发版时登记的遗留:`skills/skills-implementation.ts`(258 行,自称「从 Vidu 平台提取的 Skills 集成」)。**① 先复核再删**:全仓 `git grep` 零 import(静态 / `import()` / `require` 都查了,含 tests 与 scripts);它被编译**只因为** tsconfig 的 `**/*.ts` 通配,不是有人用。四个导出函数全是占位:`generateComicContent` 恒返回「Generated content will appear here...」、`generateComicImage` 恒返回 `/generated/image.png` —— **任何人真去接,拿到的是看起来成功的假结果**。**② 连带删掉只描述它的文档**:`skills/base/`(3 份)、`advanced/`(1 份)、`examples/`(1 份)的接口定义与该文件逐字段相同,示例调用的 `generateContent` / `applyEffect` 在全仓不存在(`generateImage` 倒是有同名方法,但签名是 `(prompt, opts)`,照文档传对象参数是错的);`INTEGRATION_REPORT.md` 是当年的「集成完成报告」,写着「可以直接在项目中使用」并附本机绝对路径。**这件事在 v12.437 之后变得不只是整洁问题**:`skills/` 现在是运行时目录,Dockerfile 整目录 COPY 进镜像,旁边摆着一份「怎么调用 skills」的假接口文档,和真正的 `SKILL.md` 规范说明互相打架。`skills/README.md` 删去「早期的能力说明文档」一节,首句改为指向真加载器 `lib/skills/registry.ts`;ROADMAP §6 技术债对应行划掉。**③ 留了一个口子,如实写在这**:`skills/screenwriter/SKILL.md` 的 Cross-references 里有一行指向已删的 `skills/base/content-generation.md`。本版按约定不动 screenwriter,这行现在是悬空引用 —— 它原本指向的「generic generation primitives」本来也从未实现过。**验证**:`npm run skills:check` 删前删后都是 7 个技能 0 问题;`tsc --noEmit` 关增量缓存冷跑 0 错误。
| **v12.437.0** | 2026-09-13 | `e0fc54e5` | **📚 导演技能库:把方法论从代码搬成数据 —— 顺手修掉一个埋了 9 个大版本的「造好没接线」**。对标 LibTV:100+ 个专业视频 Skill 由导演们封装真实项目方法,用户能发布自己的。修前「题材镜头包」写死在 TS 常量里,加一个题材要改代码发一版。**① 格式沿用仓里已有的约定,不另起一套**:`skills/<id>/SKILL.md`,就是 Anthropic SKILL.md 规范,也是 Agent Plugins v1.0.0(2026-08-06,Amazon/Cursor/Microsoft/OpenAI/Vercel)包裹的格式 —— 它不另造技能格式,就是包着 SKILL.md。**写入前没先看目标目录,是个失误**:`skills/` 原本就存在,`screenwriter/SKILL.md` 早就是这个规范。好在新建的都是新名字没覆盖任何东西;而且确认了**运行时此前没有任何代码读这个目录**,所以这是它的第一个加载器,不是第二套。wind-comic 专属字段一律放规范允许的扩展点 `metadata.wind-comic`;没有它的(如 screenwriter)是合法外部技能,只是不进流水线。**② 9 个版本的空转**:v12.193 起题材包命中后执行 `(orchestrator as any).bgmStyleHint = pack.bgmStyleHint`,注释写着「BGM prompt 侧读取(软注入)」——**全仓零读取**,编辑 agent 的上下文类型里连这个字段都没有。于是每个悬疑/甜宠/古装项目都弹「题材镜头包已注入」,**BGM 风格那一半从来没生效**。配乐提示词有**两条**构造路径(单段 `musicPrompt` 与多幕的三次 `moodPromptForAct`,后者完全不经过前者),只接一条长片照样丢 —— 收成 `withBgmStyleHint` 一个出口,5 个调用点逐个锁住。**③ 迁移会悄悄改行为**:`detectShotPack` 取第一个命中,原顺序悬疑→甜宠→古装;从目录读取默认字母序,「宫廷悬疑」会**从悬疑变成古装**且无任何报错。用显式 `priority` 排序,并从 git 取迁移前的原始常量逐字比对。**④ 坏技能不能搞垮流水线**:加载器从不抛出,问题收进 `problems`,由 `npm run skills:check` 如实报出(退出码 1)。**测试抓到一个真 bug**:缺 `cameraDefault` 的题材包**报了问题却照样进流水线**,带着空串被选中执行 `setCameraDefault('')` —— 构造条件漏了题材包必填三件套。CLI 探针没抓到,是因为那个探针同时还缺 name 和 description,**多个错误叠在一起把这一个藏住了**。**⑤ 本版最危险的一处在部署层**:迁移前题材包是 TS 常量、编译进 `.next`,哪都能跑;迁移后依赖运行时读 `skills/`。**Docker 运行时镜像只拷 .next/public/package.json/node_modules/scripts —— 没有 skills/**,加载器按设计「目录不存在不报错」返回空:**v12.436 镜像里好好的三个题材包,在 v12.437 镜像里会全部悄悄消失,本地测试全绿**。与 v12.421 那行 drizzle 是同一类拷贝清单问题。已补 COPY 并锁测试。**⑥ 新增三套导演技法**(对话戏覆盖 / 追逐动作 / 内心戏,可叠加,上限 3),命中后正文进导演 system prompt。方法论自己写 —— 轴线、正反打、视线匹配是公共手艺,受版权保护的书籍原文不抄。**明确不接的边界**:`app/api/projects/[id]/music` 是用户手动重生配乐的独立接口,题材是流水线运行时从创意检测的、没落库,这条不带题材风格词。另发现 `skills/skills-implementation.ts`(258 行)全仓零引用,已单独登记,不塞进本版。**测试 38 条,二十处突变逐条验红**(含「Dockerfile 漏拷 skills/」「多幕只接两幕」「迁移悄悄改优先级」)。 |
| **v12.436.0** | 2026-09-13 | `85d226b6` | **🎞️ 列表页改海报墙 —— 顺手挖出三个比「好不好看」更实在的问题**。对标用户给的 Castloop 截图:一屏扫几十个故事,卡片只有封面、标题、一句梗概。实测密度:项目 3→5 列、案例 3→4 列(4 条恰好铺满一行,不再 3+1 留大片空白)、素材 4→5 列(首屏 12→20 张;1052 个资产原本要滚 263 行)。**① 竖屏项目被横框裁掉 72%,而且徽章撞版三轮的根子在这**:库里 32 个项目 27 个是 9:16,列表写死 `h-[160px]` 横框,按实测框比 1.992 一张竖图只看得见 28%。更要紧的是封面容器**没有 `position: relative`**,里面的绝对定位徽章实际以整张卡片(带 transform 动画的 Link)为基准 —— **v12.431「N 镜」徽章撞 SCORE、撞页脚改了三轮,根子在这里**。墙面统一 9:16;**填充方式在图片加载后按真实像素比决定,不信库里的 `aspect` 字段**(流水线可能给竖屏项目出了横图):比 3:4 窄的铺满,更宽的 contain 并用同一张图模糊铺底,**一个像素不裁**。候选串回退(v12.426)改由 React 状态持有下标 —— 修前直接改 DOM 的 `img.src`,模糊底层跟不上前景换图。**② 案例库两个按钮是空壳**:「复制提示词 / 用这个创作」都取 `c.prompt || c.description || c.title`,但 **`cases` 表根本没有 prompt 列**,永远落到标题 ——「复制提示词」复制到「月华藏境」四个字;「用这个创作」把这四个字带进创作页,**开机门槛 10 字,按钮是灰的**。副标题还写着「点击一键复用创意」。不替案例编剧情,只用真有的标题 + 题材拼一句开得了机的种子创意;开机门槛收进 `lib/idea-gate`,创作页与案例库读同一个数 —— 否则门槛一改,入口就悄悄变回断头路。同时去掉作者行和播放/点赞数 —— 那是种子数据(1129 赞 / 4112 播放),在自己的案例库里摆假热度没有意义。**③ 素材库标题:说画面里有什么,不说运镜**。1095 个资产里 488 个叫「镜头 N / 视频 N」。分镜 309 个**全部**有描述,视频 179 个**一个都没有** —— 视频按 (projectId, shotNumber) 借同镜分镜的描述。初版取描述首句,**真机截图里首屏 5 张卡叫「Dolly-in on 85mm lens」「Static on 50mm lens」**—— 那是运镜指令。实测 294 条里 59 条如此。流水线的描述有固定格式,按格式解析:英文 `<运镜> on <焦距>, <景别>, <角度>: <画面内容>` 取冒号之后;中文剧本式首句即地点,以景别开头的跳过那一句。**只在冒号前确是镜头规格时才切** —— 真实剧本描述里常有对白冒号,一律切会把「Shirley公寓大厅」换成半句台词。改后 466 个机器名残留 0。**④ 梗概也得清洗**:剧本导入项目的 synopsis 是结构化倒出(「主要角色:Lip…。剧情概要:[1-1卡内基音乐厅门口,白天]…」),description 又以「第 1 章 <标题>」逐字复述标题。剥掉前缀后 11 条 0 条残留;**方括号里有时就是整段场景正文,初版整段删掉会删成空串**,改成只去编号保留内容。**⑤ 收尾自审抓到一处本版要修的那类毛病**:题材签被放进「有视频才渲染」的分支,又从信息区删掉了原来的题材文字 —— 无视频的案例会**连题材一起丢掉**(现存 4 条都有视频,所以看不出来)。已移到画面容器顶层。**测试 37 条,二十八处突变逐条验红**。三条第一版没红或红错:「冒号一律切」没红 —— 断言用的句子根本没冒号;「模糊铺底」红错 —— 锁的是 `blur-xl` 类名不是有没有铺底;视频借描述那条是我没算截断上限。 |
| **v12.435.0** | 2026-09-13 | `def26912` | **💰 把抽卡的代价摆到按下去之前**。估算逻辑 v12.172 就在 `lib/budget-estimate` 里了,但**只被 `assertBudget` 用于服务端拦截** —— 超预算才报错,没超就一声不吭。于是「重试镜头」是个普通按钮,点完钱就没了。对标调研里 Runway Director Mode 在每个镜位旁写着「本镜约消耗 N 积分」,而那几篇讲 3D 导演台/动作捕捉的文章共同卖点就一句「别再抽卡烧钱」——**抽卡贵不是模型问题,是代价在按下去之后才出现的交互问题**。**① 唯一出口 `lib/action-cost`**:估价直接调护栏用的 `estimatePipelineCostCny`,测试锁死「按钮上的数 === 护栏里的数」—— 两处不同用户就对不上账。展开的算式必须**加得回总价**(「8 镜 × 5 秒 × ¥0.30/秒,另含出图与配音约 ¥4.4」= ¥16.4),用户会拿它自己验。**② 估不准就得改口**:`estimatePipelineCostCny` 刻意宁高勿低、未知引擎按最贵档 —— 那是护栏口径,**拿它当报价展示会系统性吓唬用户**。所以引擎已知写「约 ¥1.2」,未指定写「最多 ¥3.6 —— 按最贵档估」。**一个数字只有说得清它怎么来的,才配写在按钮上**,和 v12.432「0 是一个结论」是同一条规矩,换到了钱上。**③ 重生是追加不是首次**:确认话写「这一步预计**再**花 ¥X」,并锁死确认必须排在真正花钱的请求**之前**。**④ 收尾自审抓到本版要修的那类毛病**:初版建了 5 种花费,**3 种零调用**;其中 `storyboard` 用 ¥0.45/张 —— 那是护栏里「含 1.5 倍重生均摊」的数,用户明确点一次重出分镜只花一张 ¥0.30,却会被标成 confident 的「约 ¥0.45」,**一个声称准确、实际偏高 50% 的数字**。没接线的备用件会在接线那天把错数一起带进界面,删了,并加断言「每种花费都得真有按钮在用」。**⑤ 顺手补上一版漏的同类谎**:v12.434 修了列表**正文**的空态,漏了**标题栏** ——「角色库 · 共 0 个」在读不到时照样写 0。抽 `countText(n, loadError)`,读挂给破折号,真空态的 0 照样是 0(断言两侧都锁)。**真机验收**:开机按钮正上方显示「本次 约 ¥16.4」,悬停出算式;把角色库/素材库接口打成 500,标题栏改口为「共 — 个」。第一次验收全返回空 —— 查出是 token 过期 + 首次引导浮层挡住输入框,是脚本问题不是代码问题。**测试 15 条,十六处突变逐条验红**;其中「开机旁估价被拿掉」第一版没红 —— 只断言了组件在且排在按钮前,把门改成 `{false && (` 照样绿,锁的是写法不是行为,已改成锁门条件本身。 |
| **v12.434.0** | 2026-09-11 | `217cc256` | **🕳️ 列表页空态不许冒充错误态**。五个列表页各写了一遍同样的取数,而且都错在同一处:`const data = await res.json(); setX(Array.isArray(data) ? data : [])` 外面套一个 `catch { setX([]) }` —— **两条路都通向同一个 `[]`**,而渲染层只认 `length === 0`。于是页面对一个库里有 137 条角色的账号说「暂无角色 · 点击「保存角色」添加你的第一个角色资产」,等于告诉他东西没了,而他能做的只有反复刷新。**① 根因比 catch 吞错更隐蔽:没查 `res.ok`**。401 返回的是 `{ message: 'Unauthorized' }`,`Array.isArray` 为假,**连异常都不是**,静静变成空数组 —— catch 根本没参与。所以抽的是 `lib/load-list`:它只回答两件事,**拿到了什么** 或 **为什么没拿到**,绝不把失败折叠成空列表。401/403 说登录、5xx 说服务端、404 说接口不在、网络错原样带出 —— 处置方式不同就不能糊成一句「加载失败」。**② 案例库最狠**:它连空态都没有 —— 失败时标题下面**一片空白**,没有加载中、没有出错、也没有「暂无案例」,三态一次补齐。**③ 失败态五处长一个样**(`LoadErrorState`),并且明写一句「**这不代表你的角色库没了,只是这次没读到**」——用户最容易误判的就是这一点。**④ 反向验证救了这一版**:失败侧 8/8 一次通过(4 页 × 500/401),但**正常路径上角色库和素材库报了失败** —— 查出这两页的列表 GET **根本不带 Authorization**,纯靠 cookie,而同一页的 DELETE 是带 Bearer 的。修前这个 401 被吞成「暂无角色」所以没人发现;现在如实报错反而把它暴露了。补上头之后正常/真空 8/8 也通过。**只验失败侧就会把这条当成「修好了」发出去。****⑤ 变异验红三次打脸**:`indexOf('LoadErrorState')` 命中的是**顶部那行 import**,分支挪到空态后面照样绿(这轮第三次栽在 indexOf 命中第一处);风格收藏那处改回 `setItems([])` 也照样绿,因为我只断言了「文件里有 loadError 这个词」。已全部改成锁**三元的门**(`) : loadError ? (` 必须排在 `.length === 0 ? (` 前面 —— 排后面就是永远走不到的死代码)和锁**取数那一段真的把失败落到状态上**。**⑥ 一条门禁被我自己收窄了**:整文件禁 `.catch(() => {})` 过宽,角色库里那处「复制成功后台累加使用次数」是正当的即发即忘(主操作已成功且有反馈),不该误伤 —— 断言收到取数函数内部。**测试 24 条,十五处突变逐条验红**。 |
| **v12.433.0** | 2026-09-11 | `6e9925c` | **🚨 流水线不许把失败标成完成**。上一版记了账没修的那条,是全仓最严重的一处,**真库里有活样本**。**① 病灶**:`create-pipeline` 收尾第 944 行**无条件**写 `status: 'completed'`。视频生成整段包在 `try/catch` 里,catch 只 `send('status', '视频生成出错，继续下一步...')` —— 那是一条**会滚走的进度消息,不是失败信号**。于是「8 镜一条视频都没出」和「顺利完片」在库里写的是同一个词。实测 `proj-1786416520904`:状态 completed、**0 条 video、8 行 media_urls 为空的 storyboard**,它自己的 timeline 质检里写着「❌ 0 个有效视频片段, 成片无法合成」、healthScore=20 —— 而 `grep -rn` 全仓确认**没有任何 UI 读这句话**。10 个 completed 里 1 个是这样。**② 还有第三层谎,复检没报、我自己查出来的**:`failed` 这个状态**早就在用** —— `pipeline-worker` 的 `markProjectFailedIfTerminal` 从 v12.21.0 起,create 任务耗尽重试就会写它。但列表页的词表里根本没有 failed,`statusConfig[p.status] || statusConfig.draft` **把失败的项目贴成了「草稿」**;项目详情页更直接,`completed ? '已完成' : '制作中'`,失败的片子写着「制作中」,而它永远不会再动一下。**这个状态已经被藏了两个大版本。****③ 判据单独成文件**(`lib/pipeline-outcome`),因为两条路径要用同一份:SSE 直通自己写状态,队列路径靠 worker 的 `fatalError → failJob` 判失败。判据散成两份就一定漂 —— 这个仓库为「同一语义两份实现」栽过太多次。失败时改发 `error`(worker 正是靠它判失败)并 `return`,不再发 `complete`,**两条路径同时闭合**。**④ 刻意只判「全军覆没」**:出了 3/8 镜也不圆满,但那由 v12.427~v12.431 那条线负责(示意片标记/导出告警/列表徽章),用户看得见;把「部分」也拉进 failed 会让大量正常项目变红,**反而把真失败淹掉**。降级出的示意片**算产出** —— 次品不是没有。**⑤ 兜底不再拿「草稿」冒充**:一个不认识的状态,唯一诚实的说法是「不认识」。拿 draft 兜底等于替库里的值编故事,而且**它掩盖的恰恰是新增的状态**,这次就是这么把 failed 藏住的。词表收进 `lib/project-status`,两个页面只挑颜色。**⑥ 已经写坏的行不会自己变回来**:`npm run repair:status` 默认只预览、`--apply` 改判、`--restore` 全还原(账本记原状态)。判据**从 lib/pipeline-outcome 引**,不许脚本自己写一套 —— 否则会出现「脚本说没问题、用户看着还是坏的」。本机已 `--apply`,1 条改判。**⑦ 真机验收**:拦截 `/api/projects` 响应注入状态(不碰库),列表页「生成失败」筛选计数 01、红色徽章渲染正确;塞一个没见过的状态,显示「状态未知(weird-new-stat…)」而**不是**草稿。第一版**文字撑破了卡片** —— 库里可能塞任意长度的脏值,已在词表和徽章两层各加一道截断,重验通过。**测试 24 条,十七处突变逐条验红**。其中一条第一版红了是**我的窗口开太宽**:切了 700 字符,越过失败分支切进后面的正常路径,那里本来就有 `send('complete')`,于是「失败分支里不许发 complete」这条永远红 —— 改成切到该分支自己的 `return;` 为止。 |
| **v12.432.0** | 2026-09-10 | `c89d1fa` | **🔇 失败不许长得像成功**。**① 先推翻自己上一版的规则**:v12.425 修 `onError → display:none` 时用的是「藏图一律有害」,这条是错的 —— 全仓 9 处里 **6 处藏得对**(`ModeCard`/`CreationWizard`/`create` 风格卡/`template-library-picker` 底下压着 `<span aria-hidden>{icon}</span>`,`video-node` 会建一个「点击播放」兜底),**藏掉那张装饰图恰恰露出身份,什么都没丢**。真正该判死的是「藏完什么都不剩」:`character-lock-section` 藏的是 `closest('button')`,**整个可选项从「从角色库带出」里蒸发** —— 实测本机 46 个有立绘的角色 **41 个会消失**(底层文件被那次定时清理删了),用户看到一个只剩 5 人的角色库,还以为自己没存过。已改走 `MediaThumb`:图没了,名字还在,照样选得到 —— 并**真渲染验收**:喂 12 个角色进选择器、把 12 张缩略图逐一打成加载失败,12 个选项一个不少、12 条「缩略图失效」如实露出、全局零个 `display: none`,点第 4 个照样带得出来。**② 交付物里的静默丢图**:`lib/script-export` 两处(分镜缩略 + 角色附页)图挂了整格消失,出册看不出这镜没图。改成 `.thumb-wrap` 里垫一格「图未出」。**这里差点埋一个反向的谎** —— 绝对定位的兄弟节点会画在**非定位元素**之上,`.thumb` 少一行 `position: relative`,占位就盖在**每一张正常出图**上。用 puppeteer + `elementFromPoint` 实测遮挡关系(3 镜 + 2 角色,一张能加载、四张挂掉):图正常时最上层是 `img`、图挂了才是占位,**5 格全对**。**占位自己还差点也消失了**:外框是 flex 项、高度靠兄弟文字撑,构造一个描述/台词全空的镜头实测 `wrapH=0 / missH=2px` —— 占位在那种镜头里等于没有;补 `min-height: 74px` 后实测 74px 可见。风格画廊那处属中间地带(藏完只剩一块渐变,分不清「风格就长这样」还是「预览没出」),也垫了一句话。**③ 白名单门禁**:每一处藏图都要声明它露出的是什么,新增一处会直接打红,逼你当场表态。**④ 后台对抗复检又挖出 5 处更狠的 —— 失败长得像成功**:`editor-node` 和 `node-shell` 都在 `await` **之前**就把按钮点亮成「已保存 ✓ / 已确认」并 disable,随后 `.catch(() => {})` 外面再套 `try {} catch {}` **双层吞错**;内存 store 是 plain zustand 无 persist,刷新即丢,**唯一真写 SQLite 的就是那个被吞掉的请求**。于是网络断了、登录过期了,用户看到的全是绿勾,刷新后确认全没了。两处是**同一语义的两份实现**,合并成 `lib/confirm-assets-client`(权限失败和网络不通分开说),并加门禁禁止第三份;草稿态没有项目实体时标「已确认(未入库)」,不拿一个和真入库长得一样的绿勾糊过去。`dashboard` 统计卡初值全 0 + `.catch(() => {})`,接口一挂就稳稳显示「0 个项目 / 0 次生成 / 0 个案例」,旁边「系统在线」的绿点照闪 —— **和这个账号真的什么都没有完全无法区分**;**0 是一个结论,只有真读到才配写出来**,现在没读到一律破折号 + 「没读到,不是 0」。「重试镜头 N」是 `fetch(...).catch(() => {})`,而 toast 点完即关、那条路由回的 SSE 流从来不读 —— 成功失败都无感,用户只看到「按钮消失了,然后什么也没发生」;现在发起说一声、读完流再下结论,四种结局各有各的话。分享页标题 `分镜 ({videoRows.length})` 用的是 **DB 行数**,渲染时却 `if (!url) return null` 让没出片的镜头整格消失,**客户看到「分镜 (10)」只数得出 4 个播放器**;当前库 185 条视频行全有 URL,属隐性缺陷,但 `regenerate-shot` 失败会写入 `['']` 正好踩中 —— 已改成「已出片 N/M」+ 缺的留一格。顺带治了旁边那盏灯:「● 系统在线」是**写死的** —— 接口全挂时它照样闪着绿,**一个永远说在线的在线指示灯不是指示灯,是装饰**,现在跟统计接口同一个状态(挂了显示「接口没响应」)。**真机验收**:掐掉图片请求,155 张风格卡里在视口内的 8 张全部露出「预览未出」且卡片仍可套用;把 `/api/metrics` 打成 500,三张卡全是破折号 + 「没读到,不是 0」、零个 0、指示灯改口。**⑤ 补跑上一轮因限额中断的复检角度**(「复检没跑完≠干净」),挖出拉片表这一条并经对抗验证成立:`buildPullSheetFromScript` 用 `typeof s?.shotNumber === 'number'` 一刀切,**镜号是字符串的镜头整条被扔掉且不留痕迹** —— 拉片表/剧本册/PDF/JSON 四种导出全都少几镜,用户拿到一份「看起来完整的残表」。已抽 `toShotNumber` 归一(数字 + 纯数字串),**刻意不用裸 `Number(v)`**:`Number([])===0`、`Number('')===0`、`Number(true)===1`,三样都会凭空造出第 0/0/1 镜(v12.425 的 normalizeReviewScore 就栽在这)。同时给 `PullSheet` 加上 `droppedShots`,并让 CSV/Markdown/PDF 三种载体各自把「这份少了什么」印出来(CSV 追加在数据行之后,不破坏「首行是表头」)。**同一轮里有两条被对抗验证推翻,照记**:一是「导出空 CSV/PDF」—— `pull-sheet-table` 在 `shots.length === 0` 时先 return「暂无镜头数据」,**三个导出按钮压根不渲染**,报告里的复现路径是拿脚本直接打 API 绕开了前端;二是「export?type=script 返回空 txt」—— 全仓 grep 后**这条路由没有任何 UI 调用方**,真正的「导出剧本 TXT」在前端用 `new Blob` 直接生成。两条的代码缺陷都真实存在,但用户到不了,**归为纵深防御而不是本版要治的「看起来成功其实失败」**;空表那一支的说明仍然留着(JSON 接口照样诚实),只是不据此声称修了用户可见的问题。顺带记一笔:推翻 #1 暴露的「`shots.length === 0` 时显示『暂无镜头数据』」本身就是**空态冒充**,它和另外八处列表页同族,留给下一版。同一轮复检还挖出**列表页「空态冒充错误态」一族共 7 处**(角色库/素材库/案例库/项目列表/风格收藏/团队/渲染快照),其中**一处就在我这版刚改过的文件里** —— `app/dashboard/page.tsx` 的 `api.generations()` 仍是 `.catch(() => {})`,于是同一张页面上会出现「统计说没读到」和「动态说你还没创作过」并排的荒唐场面。**改了主路径忘旁路**是这一族最常见的复发方式,本版顺手治了(抽 `emptyStateLabel` 共用 `MetricState`,口径统一)。**剩下 6 处留作下一版**,不塞进来充数。同一轮复检还挖出**本版不修但必须记账的一条**,而且它是全仓最严重的一处:`lib/create-pipeline.ts` 的视频生成 catch 只 `send('status', '视频生成出错，继续下一步...')`,随后第 944 行**无条件** `status: 'completed'`。真库实例 `proj-1786416520904`:**状态「已完成」、0 条视频、8 张空分镜**,它自己的 timeline 质检里写着「❌ 0 个有效视频片段, 成片无法合成」、healthScore=20 —— 而**全仓没有任何 UI 读这句话**。10 个 completed 项目里 1 个是这样。**没顺手修是刻意的**:要动项目状态语义(库里只有 active/archived/completed,得引入 failed)、列表徽章映射、以及 SSE 直通与 pipeline-worker 两条路径,半吊子改比不改更糟。连同列表页那 5 处一起留给 v12.433。**⑥ 门禁自己犯的三个错,一并记下**:git 的 `lib/**/*.ts` **不匹配顶层文件**,49 个文件里一个 `lib/*.ts` 都没有,少扫一整层却照样绿(靠「扫得到东西」那条兜底断言抓出来);`git ls-files` 漏掉**尚未跟踪的新文件**,新建的模块永远不在扫描范围内(老坑重演);「不存在」类断言被**我自己写的那段讲历史的注释**命中而永远绿 —— 去注释后才成立。另有两条是仓库既有门禁抓我的:`fake-green-gate` 判出一条**只对切片做 `not.*` 的断言**(窗口切歪了照样绿),已补正向自证;以及在线指示灯那条第一版**没验红** —— 断言写的是 `not.toContain('rounded-full bg-emerald-400 animate-pulse')`,**锁的是类名顺序不是行为**,把灯改回写死照样绿(类名换个顺序那串字面量就不出现了),已改成「这盏灯自己的颜色必须由状态算出来」。**测试 49 条,三十一处突变逐条验红**(含「把门禁的 glob 缩回漏扫顶层」这条,证明门禁自己的覆盖面也锁住了)。 |
| **v12.431.0** | 2026-09-10 | `e3505ba` | **📋 列表页也要看得出「这部片还有几处不是真出图的」**。v12.427 让导演台知道、v12.429/430 让导出与发布知道,但**列表页一片祥和** —— 得逐个点进项目才发现哪部还没真出完。实测本机 30 个项目里 4 个含示意内容(绿皮书 **16 处**、赤马斩龙 9、宿命之柱 9、门槛 2),卡片上一点痕迹都没有。**① 设计上最容易出事的是 SQL 预筛**。判据是 JS(三种形态 + 要读 JSON 字段),SQL 表达不了;但列表要一次算几十个项目,把全部资产捞进内存不可持续(30 个项目已有 1052 条)。所以 SQL **只收窄行数**(1052 → 75,7%),判断仍走 `isPlaceholderAsset` —— **判据只能有一处**。**预筛漏一种形态 = 少算 = 漏报**,而漏报正是这一族 bug 里最难发现的形态(v12.430 刚被坑过)。所以预筛模式和判据放在同一模块,并由测试用一张「已知形态表」把两者绑住:每种形态都要**同时**满足「JS 判为真」和「预筛能命中」;真库另证超集成立(全量扫描 36 = 预筛后 36,零漏)。**② 两次查询而不是 N+1**:按 user_id 一次捞全部疑似行再归组,不在 map 里逐项目查 —— 项目一多就拖垮列表页。**③ 卡片徽章撞版两次才摆对**:第一版给 `absolute bottom-3 right-3`,而那是相对**整张卡片**定位的,实拍压住 SCORE(显示成「SC⚠9处示意」);第二版排进页脚流式布局,又压住同样绝对定位的「N 镜」。最后让两个徽章**共用一个容器**才干净 —— 两个绝对定位元素抢同一处,单靠调偏移是治不好的。**测试 19 条,十处突变逐条验红**;其中两条第一版没验红,都是 indexOf 命中第一处的老坑:断言 `toContain('placeholderCount > 0')` 被**容器条件**满足(把徽章自己的条件改成 `{true && …}` 照样绿),以及「找条件后面某个 absolute」会命中**更后面**的元素(把容器的 absolute 去掉照样绿)。已分别改成锁徽章自身条件、锁「紧接着那个 div 自己带 absolute」。另有一条第一版按字符数截窗口(900)差 13 字符假失败,已改成语义判据。 |
| **v12.430.0** | 2026-09-10 | `fe70dbe` | **🔗 「不是真产物」不能有两套判据 —— 上一版的交付告警在视频上是哑的**。v12.429 让四条导出路径知道了示意图,但视频侧一直走另一套(`data.isAnimatic`,引擎全挂时回落的 Ken Burns 占位片),两套互不相认。**后果是言之凿凿的漏报**:实测构造一部四镜全占位的成片,`countPlaceholders` 返回 **0**、导出说明为空 —— 交付时一个字都不提醒。**说「没有问题」比什么都不说更糟。** 真库更难看:**宿命之柱 12 条视频里 9 条是占位片**,而它是被判为「有真作品」保留下来的项目之一。**① 判据收进唯一出处**,两条判据都留、各有实证:`data.isAnimatic===true`(**26 条**)+ 路径含 `qf-animatic-<时间戳>`(**另 2 条没有标记,只能靠它认**),实测不重合,28 = 26 + 2,少哪条都会漏。**刻意不用宽正则 `/animatic-\d+\.mp4/`** —— **Ken Burns 是一种合法运镜手法**,不是只有降级才用,宽正则会把用户自己上传的 `animatic-1.mp4` 判成占位片,**那是反过来的谎**;实测库里带 animatic 的路径全带 `qf-` 前缀,收窄不丢召回。复核另证:全仓只有 `hybrid-orchestrator` 两处写 `isAnimatic=true`,都在「所有引擎失败」分支,合法运镜走不到那里(26 true / 159 正常,零交叉)。**② 措辞没合并成一个词,是想清楚后的决定**:视频侧回落用的是**真的分镜画面**,假的是那段运镜 —— 叫「示意图」是错的,那不是一张假图,是一段假运镜。于是同一族两个词:图像**示意图** / 视频**示意片**,导出说明按类型说准,图文混合时两个都点名。**③ 收敛四处散落判据**:film-health、项目页、补渲路由、export-audit。其中两处**必须保留「一个视频都没有」那一条** —— 「要补渲的镜」比「占位片」更宽,并掉会让这些镜从补渲名单里静默消失;另一处(把降级标记带过 API 边界的)不属于重复判据,豁免并写明理由。**④ 修掉自己第一版的两个错**:一是那处替换**没加断言**,锚点对不上实际代码(现在是 `provenanceOf(data)` 不是 `data?.provenance`)于是静默没生效;二是视频判据用了直接强转,**对象态认得出、JSON 字符串态认不出** —— 而**导出路径审计的正是原始库行**,恰好把最该认出来的那条路径漏掉。已抽出字符串态兼容的 `readDataField`,两条判据共用。**测试 16 条,十一处突变逐条验红**;其中一处第一版没验红 —— 「保留没视频的镜」只给项目页写了断言,补渲路由删掉照样绿,已补。 |
| **v12.429.0** | 2026-09-09 | `6490615` | **📦 别把示意图当成片交出去**。v12.427 让导演台知道了「这片里有几张不是真出图」,但实测**四条导出路径 + 六条发布路径全部对它无感** —— 用户把片子交给客户、甚至自动发到抖音/小红书时,系统一句话都不会提醒。这是整条链路上唯一一处**会把有问题的成品交到别人手里**的地方。原则同 v12.427:**如实告知,不挡路**(用户完全可能就是要导一版草稿去对需求,拦住等于替他做决定)。新增 `lib/export-audit.ts` 作唯一出处 —— 四条路径的产物形态完全不同(mp4 二进制 / AAF 二进制 / EDL 文本 / 剪映 JSON),各自实现必然漂成四套口径。**① 两种载体**:二进制附件走响应头(`X-QFMJ-Placeholder-Count/Shots`,**值只放 ASCII** —— 中文进 HTTP 头各家处理不一,会截断或乱码;镜号超 50 个截断并标明);JSON 产物直接带人读的那句话。**② 告警必须写进产物本身**:`.edl` 会被下载、几小时后才在 DaVinci 里打开,那时响应头早没了 —— EDL 用 CMX3600 的 `*` 注释行,FCPXML 用标准 `<comments>` 字段。**③ 发布预检查内容,不只查技术指标**:`publish-preflight` 此前只 ffprobe 分辨率/时长/音轨,新增 `contentAudit` —— 发布是把片子送到外部平台,**发出去就收不回来**,这是最后一道关口。**④ 顺带修掉一个会让整套设计失效的洞**:`isPlaceholderAsset` 原先只认对象形态的 `data`,而导出路径走 `listAssetsByType` 拿到的 `data` 是 **JSON 字符串** —— 实测同一条资产,字符串判 false、对象判 true。现在计数还对是**侥幸**(靠 URL 兜底碰巧命中),等资产被持久化成正常链接就不灵了。判据已同时认两种形态。实测:绿皮书之约报出 8 张(第 1–8 镜),演示工程 0 张**完全静默**(总在响的告警等于没有告警)。**测试 13 条,十三处突变逐条验红。** 其中两条第一版没验红,都是我自己的断言太松:一条只断言文件里有 `auditAssetsForExport` 字样 —— 把 builder 的告警实参删掉字样依然在,已改为锚到**调用点**;另一条用 `toContain('contentAudit')`,而它是**子串匹配**,把字段改名成 `_contentAudit` 照样放行,已改为带词边界的正则。**这是我这一轮第四次踩「锁写法不锁行为」。** |
| **v12.428.0** | 2026-09-09 | `8a5947f` | **🔌 把两条「已接但从没跑过」的通路验掉,顺带发现健康页在替网关打包票**。① **自托管视频端点**:v12.411 的通用适配器一直记着「✅ 已接 · 待用户配」,但**那条通路从没跑过任何流量** —— 「已接」只有代码在背书。按文档里那份最小 HTTP 契约起桩,让**真适配器**打过去,同步(POST→`{url}`)与异步(POST→`{task_id}` 再轮询)两条路都走通,请求体字段与文档一致;顺带锁住 **localhost 不被 SSRF 防护挡掉**(自托管最常见就是部署在本机,挡了等于这功能对多数人不存在)。② **MJ `--cref`→`--oref` 真机验收:做不了,但把「为什么」查实了**。竞品文档 C6 挂着「真机验收未做」,这轮有 key 就去做 —— 提交得到「模型 mj_imagine 无可用渠道(distributor)」;**关键对照:一条不带任何特殊参数的 prompt 报同样的错**,所以是渠道级不是参数级;`/v1/models` 返回 629 个模型、MJ 相关 0 个,与报错吻合。**没有花掉一分钱**(请求在生成前就被拒)。③ **由此挖出真 bug:健康页替网关打包票**。`optionalProvider` 对已配置的 key 直接返回 null,注释写着「已配置的会单独探测」——而 MJ 从来没有那个「单独探测」。于是网关报 `ok`、**MJ 连一行都不出现在健康页上**,用户只有真去生成才知道它是死的。新增 `probeMidjourney`:判据取网关模型清单而**不是提交任务** —— 健康检查绝不能是个会扣费的动作(通道可用时提交会真出图)。现在如实显示 `down · 网关无 MJ 渠道(模型清单 629 项,无一 MJ)`。**修的过程里我自己又犯了一次「造好没接线」**:`probeMidjourney` 是 async,却被塞进下面那个**不 await** 的 optionals 数组 —— Promise 是 truthy、过得了 `filter(Boolean)`、再被序列化成 `{}`,于是「加了探测」和「没加」在接口输出上一模一样。已改为真 await,并把这条写进断言。**测试 8 条,六处突变逐条验红。** 另有一条第一版假失败:异步轮询在首次查询前先等一个 5s 间隔,而 vitest 默认超时正好也是 5s —— 看着像代码坏了,其实是测试超时设窄了(已放宽并记下原因;顺带记一笔:对本机秒出的模型这 5s 是纯等待,将来可考虑「先立即查一次再进间隔」)。 |
| **v12.427.0** | 2026-09-09 | `355a5d8` | **🏷 让整条链路知道「这镜没真出图」**。上一版修了封面,但病根还在生成端:所有图像引擎失败时 `hybrid-orchestrator` 返回一张渐变图,**只打一条 console.warn 就当成功返回** —— 这张假图一路流下去(落库、抄成封面、进分镜页、进成片),没有一环知道它是假的。**措辞定为「示意图」**:不用「降级产物」(难听,读起来像在评价用户的作品);**也没用「试拍」** —— 本产品里 `试拍 1 镜` 已指「真出一镜给你先看 vibe」,借这个词会把「真出了一镜」和「一镜都没出成」混成一件事。新增 `lib/placeholder-provenance.ts` 作唯一出处:生成、识别、写入规则、措辞常量全在这一处。**① 合并三份重复实现**:`mockSvg` 在 `services/hybrid-orchestrator.ts`、`services/demo-orchestrator.ts`、`app/api/generations/route.ts` 各有一份(前两份 diff 逐字相同)。第三份是**测试抓出来的** —— 我最初只 grep 了 `services/` 和 `lib/`,漏了 `app/`。7+3 个调用点全部收编。**② 标记打在写入咽喉处,不在调用点**:复核推翻了我「所有资产都经 saveAsset 落库」的假设 —— `regenerate-shot` / `heal-shots` / `regenerate-shot-4k` / `regenerate-asset-image` 四条路由都绕过它直接写。逐点打标必然漏,所以推导放进 `createAsset` / `upsertAsset`。三条规则:调用方显式写了就照用;**带了新内容就按新 URL 重判、不继承旧标记**(否则一镜真重生成功后会永远背着示意图标签);只改 data 没带内容才继承旧标记 —— 因为 `data` 是**整体覆盖写**,分镜要写两次,不继承第一次打的标记会被静默擦掉。**③ 服务端必须在原始行上判**:`normalizeAssetRow` 会把 `mediaUrls[0]` 换成 `persistentUrl`(理由正当,外链会 404),副作用是**历史 mock 的证据在出接口那一刻就被抹掉** —— 实测绿皮书之约库里 8 张 mock 分镜,接口出去后客户端一张也认不出来。改为服务端判完带 `isPlaceholder` 出去,客户端不再从失真数据里倒推。**④ 如实告知但不挡路**:示意图**不**把环节翻成「未就绪」(那会连累导出草稿),而是作为**另一个维度**报数。导演台横幅从无条件的「全链路就绪 · 可导出成片」改为「全链路就绪 · 含 N 张示意图,重生即可替换」,环节卡片挂出「N 张示意图」。实测绿皮书之约:8/8 全部识别,横幅与卡片如实显示。**测试 25 条,十九处突变逐条验红**。其中一条第一版没验红 —— 断言「源码窗口里有 PLACEHOLDER_LABEL 字样」,把判断条件改成 `false`(有示意图也照说可导出)断言依然绿。**这是我这一轮第三次踩「锁写法不锁行为」**,遂把横幅文案抽成 `pipelineHint()` 纯函数,改成锁行为。另有一条是我自己的夹具写错(成片环节认的是 `video` 不是 `final_video`),被测试当场抓出。 |
| **v12.426.0** | 2026-09-07 | `1e98d6c` | **🖼 项目卡片全是渐变占位图 —— 不是库里有垃圾,是封面被冻结成了一次失败的快照**。owner 看 README 的「我的项目」截图时发现卡片大半是占位图,问是不是还有大量测试项目。查下来两件事都有,但主因是第二件:**① 封面从来不重算**。`create-pipeline.ts:942` 在流水线末尾抄一份 `finalStoryboards[0]?.imageUrl` 进 `cover_urls`,抄完就冻住。于是三种假封面被永久写死:**mock 兜底图**(所有图像引擎失败时 `hybrid-orchestrator:1394` 返回 `mockSvg(…,'Shot 1')` —— 只打一条 warn,返回值却长得像一次正常出图,**又一个「失败长得像成功」**)、**mock 引擎产物服务** `/api/mock-assets/*.svg`、以及**带 `Expires=` 的引擎签名直链**(赤马斩龙那条对应 2026-06-17,已过期 82 天)。实测 30 个项目 12 个是前两种、6 个是第三种;其中月挂不下来 / 宿命之柱 / AI觉醒 各有 11~12 张真分镜**还活着** —— 用户后来重生过,但没人回头重算封面。**② 还有一处「造好没接线」**:封面候选面板能让用户定版(落 `chosen-cover` 资产),但只有 publish-package 读它,项目卡片压根不看,选了也白选。修法不做数据迁移,改**读时解析**:新增 `lib/project-cover.ts` 作唯一出处,列表接口一次查询带出「用户定版 / 冻结封面 / 本片第 1 镜 / 本片视频」,排成**有序候选串**返回,卡片前一张挂了就换下一张,全挂才退占位图。mock 两种形态一律剔除;已过期的签名链降到最后(过期时刻明写在 query 里,读出来比较即可,不靠猜);通用风格样张(`/styles/*`)排在本片自有画面之后。分镜候选按 `shot_number ASC, updated_at DESC` 取 —— 同一镜库里常有两行,空的那行 `updated_at` 更早,真画面在后来「重生」的行里。**效果实测:封面能加载的项目 11 → 14,且 14 个全部首选即命中,零回退请求**(修前有 6 个要先打一发必 404 的过期链)。**③ 顺带清库**:新增 `scripts/archive-placeholder-projects.mjs`(默认只预览,`--apply` 才动,`--restore` 一键全恢复,带账本)。判据两条任一命中即下架:封面候选全挂(即列表里就是一张占位卡)、或 ID 是夹具形态(`ad-` / `ev-ad-` / `dd-verify-` / `sketchlock-` / `kling-full-`)。30 个项目下架 21 个,余 9 个全部显示真画面。**用「下架」不用「删除」**:前者只改 `projects.status` 一列、无级联、不碰磁盘、可一键恢复;而经复核确认 **DELETE 是一次延迟的磁盘删除** —— `deleteProjectCascade` 清掉 `project_assets` 后,那些文件就失去了 cleanup 的唯一保护(引用名单),下一轮 cron 按 mtime 照删。也已核实下架**不会**让素材更易被清理:`referencedBasenames()` 查的是全部 `project_assets`,不按项目状态过滤。**测试 17 条,十四处突变逐条验红**。其中两条第一版没验红、当场加固:一条断言 `ORDER BY shot_number` 被**视频那条子查询**满足了(indexOf 命中第一处,分镜排序改坏照样绿),改成锚到 storyboard 子查询本身;另一条只在源码里找 `coverIdx` 字样 —— 把回退下标改成常量 999(回退彻底失效)断言仍绿,遂把推进逻辑抽成 `nextCoverIndex()` 纯函数,改成锁行为。**我自己又踩了反复警告过的两个坑。**另外 mock 检测第一版只认 `data:image/svg`,漏了 `/api/mock-assets/`,绿皮书之约的卡片因此实拍出一块纯绿色矩形 —— 同样是锁写法不锁行为,已按语义补全。README 里 2026-08-18 拍的「我的项目」截图就地更新(抓图脚本已收编该步)。 |
| **v12.425.0** | 2026-09-06 | `ba9d89b` | **🖼 所有素材都显示不全,必须点全屏才看得到完整图**(用户实测反馈)。复现并量化:角色立绘原生 **896×1152 竖构图**,被塞进 **355×200 的横框**配 `object-cover` —— **裁掉 56%**;场景 1344×768 塞进 542×180,**裁掉 42%**。**病根不是某一处写错,是框的比例被写死而素材比例是变的**。抽全库 99 张实测:character 有 0.78 也有 1.33;storyboard 1.75 和 1.33 五五开;storyboard-sketch 全是 0.56 的竖图 —— **同一类型内部比例就是混的,任何写死的框配 cover 都必然裁**。所以 contain 是唯一正确解,框比例只决定留边多少。新增 `lib/media-frame.ts` 作唯一出处(框比例按类型的主导原生比例取,填充恒为 contain + 黑底遮幅),改到项目页角色/场景/分镜/视频、角色库卡片与转身图、拉片、镜头检查器、候选图、参考图架、评论附件、图生视频输入预览等 **24 处**;`ImageLightbox` 的缩略图默认从 cover 翻成 contain —— **缩略图和全屏看到的是同一张完整画面**。身份小圆片(≤96px)、时间线缩略条、风格样张、营销落地页保持 cover 不动:那里裁切是刻意的。**顺手挖出并修掉的三个真 bug**:① 10 处 `onError → style.display='none'`,图挂了卡片变一片空白 —— 本机角色库 44 张立绘底层文件被那次定时清理删了 41 张,页面看着就是「空角色库」;新增 `MediaThumb` 如实显示「素材已失效」。② 顶栏显示 **「评分 undefined/100」** —— `review ? … : '—'` 只判了对象在不在,没判分数在不在;抽出 `normalizeReviewScore` 并测,第一版用 `Number(x)` 又被 `Number([])===0` 钻了空子(测试抓到),改白名单。③ **`useInView` 的 margin 收缩上边造成永久死区**:`'-30% 0px'` 上下各砍 30%,元素明明在屏幕上却不算进入视野;页面本身不滚时死区是永久的 —— 分镜页一致性仪表实测显示 **AVG 0 / PASS 0**,而同屏逐镜分是 89~92,大屏用户看到的是个假的 0 分;`TextReveal` 更狠,死区里的词是 `opacity:0`,**正文直接隐形**。改成只收缩下边。**测试 18 条,七处突变逐条验红**(含把原 bug 种回去验门禁抓得住);新增两道防回潮门禁:「写死像素高 + cover」承载动态素材、以及「收缩上边的 useInView margin」。**真机抓图 18 张**(`scripts/capture-v12-425.mjs`):素材库逐类(角色/场景/分镜/视频/配乐/剧本)、角色资产三层次、导演台/一键成片/创作工坊、分镜规格/拉片五栏/时间线/节奏曲线,每张下面配实现逻辑 + 工作流架构说明。**撤回一条旧结论**:v12.416 记的「点项目页 tab 和滚 body 都不改变视图」是错的 —— 标签栏在折叠线以下,坐标点击打空,先 `scrollIntoView` 再 `el.click()` 就能切,本轮 12 张靠它拍成。抓图脚本自身也修了三处:clip 用视口坐标(默认按文档坐标裁会把 sticky 页头拍进画面中间)、滚动容器改成**运行时试出来**(这页 html/body 都不滚,靠 class 猜会静默失效)、近重复判据从「PNG 字节数」换成「可见文本」—— 字节数从来不是相似度,前后三版分别造成误杀「剧本」当「配乐」、以及同一个 no-op 上一轮抓住下一轮放过,**一道时灵时不灵的门禁比没有还坏**。 |
| **v12.424.0** | 2026-09-05 | `5840fff` | **🖼 参考图被拼进了 prompt 文本,从来没真正到过引擎**。Batch L 剩的最后一项是「多参考图路数」——本以为是接新 provider,查代码后发现问题在更前面:`kontext` provider 发参考图的方式是 `prompt + ' [Reference images: https://… , https://…]'`,**把 URL 拼进 prompt 文本**。而文生图模型读到 prompt 里的一串 URL **取不到图** —— 它既不会去下载,也不知道那是图片地址。于是「角色参考」在这条 provider 上**从来没生效过**,而且不报错:图照出,只是没参考。**又一个「失败长得像成功」**。已按官方字段表(2026-09-05 核 docs.bfl.ai)改为 `input_image` / `input_image_2..N` 编号字段。**第二个问题:全局上限取了最弱那家**。`MAX_REF_IMAGES = 4`,注释写着「minimax-multi 与 gateway 都是 4」——于是 4 成了全局天花板。但各 provider 在注册表里**各自声明了** `maxRefImages`(MJ 2 / minimax 4 / gemini 3 / mock 8),分派层也确实按它选路(`refCount > maxRefImages → 排除`)——**按引擎选路的机制本来就有**,却被收敛函数提前砍到 4 给废掉了:flux-2-pro 官方支持 **8 张**,永远等不到第 5 张,而这同样不报错。现在全局常量只作「任何引擎都不可能超过」的兜底闸(4→8),真正的裁剪交给新增的 `capRefsFor(refs, providerMax)` —— **谁能吃多少,由谁自己说了算**;minimax 仍按自己的 4 张裁(多发会被上游**静默截断**,而静默截断没人会发现),各 provider 的声明改用同一个常量,免得声明与实现两处漂。测试 7 条,六处突变实测全拦。顺带修一条被样本量绑住的旧断言:v12.393 那条用的是常量 `MAX_REF_IMAGES` 而非字面量 4(写法是对的),但**样本量写死 6** —— 上限从 4 提到 8 后 6 < 8 就截不到了,已改成由常量推导。 |
| **v12.423.0** | 2026-09-05 | `cf1e656` | **⚡ arm64 走 QEMU 模拟,实测 43 分钟**。v12.421 用 `platforms: linux/amd64,linux/arm64` 一次出双架构,arm64 在模拟层上跑 `npm ci` + `next build` —— 最近一次完整构建 **43 分钟**(另一次 13 分钟是命中缓存)。当时如实记着「还没改,先让这条链绿起来」,这一版改。改成按架构分 job、各自跑在**原生**机器上(`ubuntu-latest` / `ubuntu-24.04-arm`,后者对公开仓库免费、2025-08 GA),两边并行,总时长约等于慢的那一边。**但结构性改动带出三个「不报错但出错」的坑,逐个都得防**:① **矩阵 job 不能用 job-level `outputs` 传 digest** —— 每个矩阵实例都写同一组 output,**后完成的覆盖先完成的**,永远只拿得到一个架构的 digest。这是 Actions 的既有行为、不是配置写错,**而且不报错**:你会合出一个只有单架构的 manifest。第一版我就是这么写的,改用 artifact 传递才对。② **两个 job 不能各自打同一个 tag** —— 后推的覆盖先推的,tag 最终只指向一个架构,同样不报错,另一半架构的人拉下来才发现跑不了。故按 digest 推、tag 只在合成时落一次。③ **合成成功 ≠ 合对了** —— 加了一步回头 `imagetools inspect`,逐个确认两种架构真在 manifest 里;digest 不齐时直接拒绝合成,而不是产出一个单架构镜像。另外:缓存按架构分 scope(否则两架构互相污染对方的层);冒烟改成**每个架构在自己的原生机器上跑**(arm64 镜像在 arm64 机器上验才作数,不经模拟层),两边都验中文字幕字体 —— `font-noto-cjk` 在 arm64 上是不同的包文件。顺带清掉 `paths` 里的 `drizzle/**`(v12.421 已确认该目录根本不存在)。⚠️ `ubuntu-24.04-arm` 在**私有仓库会直接让 workflow 失败**,已写进注释:将来若转私有,得回退 QEMU 或用付费 runner。测试 9 条,八处突变实测全拦。**本机没有 docker、更没有 arm64 runner,这一版的真正验证只能来自 CI** —— 故测试锁的是那三个坑的防护是否在位,真实构建结果以 CI 为准。顺带改掉 v12.421 两条**锁写法而非锁行为**的断言:一条要求 workflow 里必须有 `setup-qemu-action`、一条锁死 `IMAGE=` 这个变量名 —— 两者锁的都是「当时的实现方式」,而不是「双架构」和「镜像名必须小写」这两个真正的要求。换成原生 runner 后它们就红了,而目标一点没变。这已经是本轮第几次同型:**断言该锁那件事本身,不是锁它当时长什么样**。**实测结果:43 分钟 → 15 分钟**(amd64 9 分钟 / arm64 14 分钟,两边并行)。而首跑时**我自己写的那道校验误报了一次**:它 `grep '"architecture":"arm64"'`,而 `imagetools inspect --raw` 输出的是**带空格的 pretty JSON** —— manifest 明明合对了(两个架构都在),校验却报红。已用真实 manifest 实证:jq 版识别出 `amd64 arm64`、grep 版不命中。改用 jq 解析并排除 buildx 的 attestation 条目。**这比「少了个架构」更糟** —— 一个会误报的门禁只会训练人忽略门禁,而它守的正是「别把单架构镜像发出去」这种拉的人才发现的问题。 |
| **v12.422.0** | 2026-09-04 | `7cac5dd` | **⏱ 剧本写 30s 的镜,成片是 8s —— 而没有任何一处说过它短了**。roadmap 上这一项写着「等国产端点开放 API,或自托管权重跑通」,所以第一步是**核实前提到底成没成立**:六路并行调研 + **24 条独立二次复核**。结论:**条件 A 成立**(Seedance 2.5 与 Wan 3.0 都做到了 30s,且经复核确认是**一次调用直出的连续片段**、不是 Scene Extension 那种拼接);**条件 B 不成立**(开源权重仍到不了 30s:LTX-2.5 Fast 上限 20s、Wan 2.2 约 15s,而 **Wan 3.0 的权重从未发布** —— GitHub 仓库挂着 Apache-2.0 却只有一个 README,**这更正了我在 v12.411 的假设**)。**但真正的收获是查自己**:① 主管线**三处**写死 8 秒 —— v12.409 修了主 switch 那处,却漏了旁边 plugin-chain 那处和再生成路径那处,**同一个错在同一个函数里并排躺着三份**;后果是注册表里做得了更长的引擎**从来没被要求过超过 8 秒**,整套 `maxDurationSec` 过滤形同虚设。② 链被时长过滤清空时,`tried` 是**空数组**,上游据此拼出的原因是空串,最终报「no providers」—— 那句话把人往「没配 key / 引擎都挂了」引,而真因是个**静态可知**的事实:没有引擎做得了这个时长。然后静默走 fallback 出一个短片。③ **两处虚报的上限**:Veo 写 10 而单次枚举只有 4/6/8(60s+ 是拼接,不是单次 —— 与我们在 v12.401 README 上犯过的混淆是同一个,这次是代码版);LTX 写 20 而我们钉的是 Pro 变体、枚举只有 6/8/10。**这两处以前无所谓**(请求都写死 8s,虚报永不被行使),而本版把请求改成按剧本要时长之后,虚报立刻变成**选中一个做不到的引擎、然后交回一个短片**。修法:三处时长统一取自剧本;空链给出说人话的诊断(要多久 / 最长能给多久 / 各引擎上限);两处上限按**实际钉住的端点**修正,并让「用哪个端点」与「声称能做多长」必须一起改。新增 **Wan 3.0** provider(30s 原生单次,官方字段表 2026-09-04 核)—— **选它而不是质量更强的 Seedance 2.5**,是因为 v12.415 明确写过「不接 Seedance 2.5:好莱坞六大停止函纠纷未和解」;本轮查到 2026-08 字节与 MPA 签了备忘录,但**它明确不处理训练数据侵权责任**,风险降低而非消除 —— 那是关于产品法律风险的判断,**不该由我单方面推翻一条写下来的决定**,如实上报由 owner 定夺。⚠️ Wan 1080P 约 **¥1.2/输出秒(一条 30s ≈ ¥36)**,是注册表里最贵的:费率已进成本表 —— 不进的话会落到 ¥0.3 默认兜底、**低估 4 倍**,v12.413 的预算闸就形同虚设;优先级排在常规引擎之后,只在剧本真要长镜时才轮到它。测试 15 条,八处突变实测全拦。 |
| **v12.421.0** | 2026-09-04 | `10e15f0` | **📦 「一条命令」还得先本地构建;而那个镜像会把中文字幕烧成方框**。两件事,后面那件更难看。① v12.414 把上手压到一条命令,但那条命令要 `npm ci` + `next build` —— 几分钟起步、还得本机有构建环境;而竞品复核 C8 的证伪条件写得很明白:「可直接 `docker pull` 并 `docker run` 一行拉起的**预构建镜像**(无需本地 build)」。② **镜像里只装了 `ttf-dejavu` —— 它没有任何中日韩字形**。而 `lib/subtitle-burn.ts` 在 Linux 下给的字体名是 `Noto Sans CJK SC`/`Noto Sans JP`/`Noto Sans KR`,解析不到时 libass 回退 DejaVu,于是**每一个汉字都烧成方框** —— 这是一个中文短剧产品的官方镜像。而它不会报错:镜像构建成功、容器起得来、片子也出得来,只是字看不了,**又一个「失败长得像成功」**。修法:装 `font-noto-cjk`,并加**构建期自证** —— `fc-match` 找不到时会静默回退,所以检查它解析到的确实是 Noto,解析不到就让**镜像构建失败**。**这道自证是因为我做不到实测才加的**:本机没有 docker,我无法验证这个包到底提供哪些 family 名;与其猜,不如让构建本身来证明。发布走 **GHCR + 内置 `GITHUB_TOKEN`** —— 不需要 owner 录任何密钥(Docker Hub 要 `DOCKERHUB_TOKEN`,那是一次人工介入,且密钥一旦要人转交就多一个泄露面)。多架构 amd64 + arm64(只出一个总有一半人跑不了或悄悄走 qemu 慢十倍)。**构建成功 ≠ 镜像能用**:发布后真拉起来打一次首页,并在运行期再验一次字体(构建期过了不代表没被后续层弄丢)。demo compose 改为默认拉镜像、`build:` 退为兜底;两份 README 给出不需要 clone 的一行命令。测试 8 条,七处突变实测全拦 —— 其中两处最初没打中:**断言经由我自己写的注释命中**(YAML 的 `#` 与 Dockerfile 的 `#` 各一次),剥注释后才真正拦下。同一个坑本轮已在四种注释形态里各栽一次。顺带修掉一条**会因为机器忙就红**的测试:v12.240 那两条各自跑一遍 `runConsumerGate`(全仓扫描),本机高负载下单次几十秒,双双撞上 vitest 默认 5s 超时 —— 报出来是「Test timed out」,而门禁 CLI 本身 0 违规。已提到 `beforeAll` 只扫一次并给足超时:测试内耗时 42s → 12s。一条会因负载变红的测试迟早被当噪音忽略,而它守的是「有人绕过唯一入口」。**而那道构建期自证第一次跑就中了**:Docker workflow 首跑即报「字体「Noto Sans JP」解析不到 Noto(回退到了 DejaVu Sans)」—— `font-noto-cjk` 给的是**泛 CJK 家族**(Noto Sans CJK SC/TC/JP/KR),没有独立的 `Noto Sans JP` / `Noto Sans KR`,而 subtitle-burn 请求的正是后者。**我本机没有 docker、验不了这个包提供哪些 family 名,是那道自证替我验的**。修法是在镜像内加 fontconfig 别名映射(`docker/font-aliases.conf`),而**不改代码** —— `Noto Sans JP` 是真实存在的独立字体,本机装了它的用户请求它是对的;问题只出在这个镜像里,就在这个镜像里解决。**修完字体又露出第二层**:`COPY --from=builder /app/drizzle ./drizzle` —— 而**这个项目根本没用 drizzle**(无依赖、无 config、仓库里也没有该目录;建表在 `lib/db.ts`)。buildkit 报 `"/app/drizzle": not found`,也就是说**这个镜像从来就构建不出来** —— 连带 README 的 Docker 说明、v12.414 的零 key 试用(`docker compose up --build`)**一直都是不成立的**。一个从没被执行过的构建,和没有构建是一回事;是本版第一次真把它跑起来才发现的。已删死引用,并加测试锁住「Dockerfile 里 COPY 的源路径必须真实存在」(构建期产物 .next/node_modules 除外)。**第三层**:镜像终于构建并推送成功,却倒在冒烟那步 —— 我手写的 `${REGISTRY}/${IMAGE_NAME}` 用了 `github.repository`,而它带大写(`ChrisChen667788/…`),**docker 仓库名必须全小写**;`metadata-action` 会自动转、我手写那行没转,于是「构建成功、发布成功,只有验证那步倒了」——而那步正是用来证明镜像可用的。已加 `tr` 转换 + 测试锁住「所有镜像引用全小写」。**这三层都是同一件事的不同面**:这个 Dockerfile 从来没被真正执行过,所以三个错并排躺着谁也没发现;一跑起来就一层层露出来。另记一笔已知代价:arm64 走 QEMU 交叉编译,实测约 **40 分钟**;改用原生 arm64 runner 分 job + 合 manifest 是结构性改动,留给后续版本 —— 先让这条链绿起来。 |
| **v12.420.0** | 2026-09-03 | `3bfc438` | **🧱 v12.411「接了自托管端点」,而它在最自然的地址上根本调不通**。两个病,一个比一个难看:① **只给了契约没给能跑的东西**(竞品复核记为 C14)—— 想用 Wan 2.7 / LTX-2.5,用户还得自己写一个 HTTP 服务把推理脚本包成那份契约;接口有了,离真能用还差一层,而那一层门槛不低。② **更难看的**:`lib/ssrf-guard` 默认拒绝 localhost / 内网地址(这是对的),而自托管端点最自然的地址就是 `http://localhost:8188/...` —— **我在 `.env.example` 里写的示例正是它,它被自己的守卫拒掉了**;v12.410 的自托管音乐同病。也就是说这两条「已接入」的路,**一次都跑不通过**。新增 `selfhost/video/server.mjs`(零依赖,只用 Node 内置模块)+ docker-compose + README:收请求 → 起你指定的推理命令 → 把产出的 mp4 交回去。**它明说自己不含模型权重与推理环境** —— 权重几十 GB、显存要求实打实,声称「一键跑起 Wan 2.7」会是谎话;它消灭的只是「还得自己写一个 HTTP 服务」这一步。已用 ffmpeg 造假推理**真跑通端到端**(提交→轮询→取片→Wind Comic 侧 service 拿到 url)。SSRF 那处**不用** `SSRF_ALLOW_PRIVATE=1` —— 那是全局开关且生产禁用,为一条端点撤掉整道防线是拿真实安全边界换功能。改成**极窄例外**:origin 必须**逐字出现在**`SELFHOST_VIDEO_URL`/`MUSIC_SELFHOST_URL`/`LIPSYNC_API_URL`/`COMFYUI_URL` 这几个「部署者自己填」的 env 里。判据落在 env 上,任何从请求内容里冒出来的 origin 都不可能命中;且精确到 origin —— **同主机不同端口仍拒**、云元数据地址仍拒(均已实测)。适配器本身几处刻意设计:命令走参数数组**不进 shell**(prompt 是外部输入,拼进 shell 就是命令注入)、异步(视频推理动辄几分钟,同步会让上游 HTTP 超时先到而任务还在跑、白烧算力)、并发默认 1(显存是硬约束)、超时 SIGKILL(挂死的推理会一直占显存)、**退出码 0 不等于出片**(必须确认产物存在且非空 —— 「成功了但没有产物」是最难查的一类失败)。测试 9 条,六处突变实测全拦。 |
| **v12.419.0** | 2026-09-03 | `fa1a36d` | **📣 v12.412 报了一条永远为真的告警,而这一版让独立评分真正默认生效**。先修上一版自己的 bug:v12.412 说「Vision Audit 和写分镜 prompt 的是同一个模型」,于是拿 grader 与 `API_CONFIG.openai.model` 比 —— **但被评的内容不是 `model` 写的**:剧本/分镜/visualPrompt 由 `writer-agent` 调 `callLLM(..., useCreativeModel=true)` 出,走的是 `creativeModel`;而 grader 用的就是 `model` 本身,所以那个判据**永远返回「自评中」**。本机实测 `OPENAI_CREATIVE_MODEL=claude-fable-5` / `OPENAI_MODEL=claude-opus-5` —— **本来就是两个模型**。也就是说 v12.412 之后每跑一次审计都打印一句不成立的告警。**一条永远为真的告警不是「稳妥」,它是噪音** —— 每次都响的告警,到真出问题那次也没人看;与 v12.418 那道误报的近重复检测、v12.400「会误报的门禁只会训练人忽略门禁」同一条教训,只不过这次误报的是我自己上一版加的东西。已把比较基准改为**剧本作者**,并把 `authorModel` 带进返回值(否则无从判断比的是谁)。再让它**默认生效**:通用档若已异于作者就直接用;真是同一个模型时,**自动改用仓库里早就有的 `LLM_FALLBACK_*` 链**(独立域名 + 独立 key + 不同模型)—— 评分是轻量只读的活儿,拿它当判官不需要新增任何配置。三条保守都留着:fallback 没配齐不硬凑(老实报自评比假装独立好)、fallback 与作者同名不算独立(换个名字不等于换个判官)、显式 `GRADER_*` 优先于自动挑选。**顺带修了一个真潜在 bug**:`fallbackBaseURL` / `fallbackApiKey` 此前在**模块加载时求值**,进程起来后再设 `LLM_FALLBACK_API_KEY`(管理界面改配置、或测试注入)一律不生效,而表现是「配了但没用上」、**没有任何报错** —— 写这一版时就是被它绊住的。已改成 getter,与同文件模型 ID 一致(那些刻意写成 getter 就是为了「免重启生效」)。测试 7 条 + v12.412 那 6 条同步更新,六处突变实测全拦。 |
| **v12.418.0** | 2026-09-03 | `c84aaef` | **🗣 ja/ko/ru 的成片完全没有口型,而解开它的引擎其实早就在仓库里**。`lib/language-detect.ts` 把这三种语言的 `lipsync` 判为 `'none'` —— v12.179/196 的理由是「口型模型只有 zh/en 音素表,拿 en viseme 驱动会严重错位,**错口型比无口型更伤观感**」。那个判断今天依然对,**但它受限于当时的引擎**:而 `services/lipsync-providers.ts` 里的 **Sync.so 是 language-agnostic 的**(对齐波形而非音素)——这三种语言本可以有口型,只是**永远走不到它**,因为语言表在更早的一步就把它们拦下了。新增 `lib/lipsync-language-gate.ts` 按「**语言 × 当前装了什么引擎**」判定,并同时接进**两处**消费方(主管线 editor-agent + 能力提示 engine-capability-notes)——只改一处就是旁路没跟上。刻意保守:**只有语种无关引擎才算数** —— 可灵是 5 语种(本轮复核纠正:不是坊间传的 20+,那是 HeyGen 的数字被张冠李戴);自托管 `wav2lip-http` 背后可能是任意引擎,我们无从知道它对日语行不行,拿「可能行」换「错口型」是拿观感赌运气。顺带修 Sync.so 两处硬伤:① 模型**写死在 `lipsync-1.9.0-beta`**(一个 beta 版,随时可能下线,而下线后的报错长得像「参数不对」)——已钉 `lipsync-2` + 官方枚举校验;② `sync_mode` 写死 `cut_off`,音画不等长时**把画面截掉**,短剧里那是「一镜播到一半没了」—— 改默认 `bounce`。**这一版最该记住的是差点犯的错**:我照着调研结论「Kling 口型已接但零调用」去新建 Sync.so provider,grep 后发现两件事 ——① 口型在主管线里**真的被调用**,那条结论是错的;② Sync.so **早就存在**,我加的是**第三份实现**。已撤掉,并把 claims 里的 C16 如实订正。**别人的结论要先自己核一遍,尤其当它让你去「补一个缺失的东西」时。** 顺带重写了 v12.179 那条被写法绑住的断言:它锁 `lsLang === 'none'` 这一行,而判定改成「语言 × 引擎」后行为更准了、它却红了 —— 与 v12.122 / v9.4.4 同型,已改成直接过生产判定函数。测试 9 条,六处突变实测全拦。 |
| **v12.417.0** | 2026-09-03 | `d36f628` | **🟥 本地 5294 全绿,CI 却红 —— 而且没有任何一条测试失败**。v12.407 / v12.411 的假时钟辅助函数写成「先 `p.then()` 出一个 `wrapped`,推完时钟才 `return wrapped`」——在推时钟的那段时间里,它是一个**已拒绝但还没人处理**的 promise。本机 Node 25 不报,**CI 的 Node 20 报成 unhandled rejection**,于是 vitest 输出「5293 passed / **0 failed** / **1 error**」→ 退出码 1:**最难判的那种红,没有一条测试失败,但整个 run 是失败的**。**修法不是「把那两处改对」** —— 改对只解决这一次。真正的问题是**本地与 CI 的判定标准不一样**,这正是 v12.325 那条教训(「本地全绿从来推不出 CI 会绿,除非本地跑的就是 CI 跑的」)在另一个维度上的重演:那次差的是门禁步骤,这次差的是 **Node 运行时标志**。所以 `npm test` 显式带上 `--unhandled-rejections=strict`,让两边用同一套标准;两处辅助函数改为**立刻用回调接住**结果再重抛。**已实测证明有效**:同一份旧写法在 strict 下退出码 1、新写法 0(本机没有 Node 20,用 strict 标志在本地复现出同一行为才敢下结论)。测试 3 条。 |
| **v12.416.0** | 2026-09-03 | `0279a6d` | **✍️ libass 字幕 ≠ 帧内文字**。本轮竞品复核把这条列为**最实的一个缺口**:我们有 libass 中文字幕烧录,但那是**后期叠加**的一层字;而短剧/漫剧真正需要的是**生成层的帧内文字** —— 片头字卡、对白框、招牌、书信,这些字得长在画面里、跟着透视和光线走,叠一层替代不了。而**通用图像模型画汉字基本是乱码 —— 它不报错**,只画出一堆像汉字的鬼画符:又一个「失败长得像成功」,出图成功了、只是字不对,而且没人会发现。竞品侧早有成熟方案(Seedream 5.0 的 Glyph-Aligned ByT5、Qwen-Image 2.0),而我们的 `seedream` 档一直只是普通图像兜底,**从没因为「这镜要写字」被选中过**。新增 `lib/text-in-frame.ts`:识别形态(字卡/对白框/招牌/信件)→ 交给有字形对齐能力的引擎 → 把**要写哪几个字**用引号明确告诉它(免得被当成场景描述)。**两条刻意的克制**:① 没有确切文字就不算需要帧内文字 —— 否则退化成「让模型自由发挥写点像字的东西」,那正是乱码的来源;② **没有擅长写字的引擎时不硬画**,明确回落 libass 叠字并说明原因 —— 硬画产出的是「看起来对、其实是乱码」的图,比叠一层字幕更糟。已接进主管线图像路由(选中的引擎真顶到 primary)。测试 9 条,五处突变实测全拦。 |
| **v12.415.0** | 2026-09-03 | `26a2183` | **🙊 四条我们对外宣称的「独有」,本轮被逐一推翻 —— 不是被追上,是当初说过头了**。本轮竞品分析(**8 视角并行联网调研 + 26 条高风险数值独立二次检索复核 + 上轮 12 条可证伪断言逐条证伪,45 个 agent 全部完成**)最重的产出不是发现了谁更强,而是发现**我们自己把话说满了**:① 「EDL/AAF 导出竞品独有」—— **Descript 早有** Timeline Export(Premiere XML · FCPXML · Reaper EDL · **Pro Tools/Logic AAF**,官方帮助文档有据);② 「唯一 MIT 开源可自托管的端到端 AI 短剧平台」—— **不成立**,`EvoLinkAI/ai-short-drama`(MIT + Docker + 小说→成片全链)同样满足,另有 LocalMiniDrama / Toonflow / huobao-drama 等多个活跃项目;③ 「BYO 多 provider 注册表独有」—— **PopShort.AI 已公开集成**跨厂商模型;④ 「节奏/反转审计竞品无对应项」—— **火山剧创**公开声称多 Agent 校验优化冲突/反转/高潮。四条已在**两份 README 全部收回**,并改写成仍然站得住的**更窄**版本:火山剧创是生成内联优化、Descript 有导出但无节奏审计也不开源 —— **仍无一家同时具备「节奏审计固化为可拦截的独立工程门禁 + EDL/AAF 导出 + 开源自托管商用」**(注意「独立可拦截门禁」这几个字是新加的,少了它就不成立)。另落 `docs/COMPETITIVE-GAP-2026-09.md` + claims 台账刷新(16 条,含 6 条本轮判决回填:**C2 被推翻**,C1/C3/C6/C8 收窄,C7 被逐处 grep 证实)+ ROADMAP **Batch L**(v12.416→422)。v12.401 那道「历史轮次指路牌不许失效」的测试自动生效,三份旧文档已指向本轮。**说过头本身是一种劣势**:它让人低估真实竞争强度,也让真正站得住的那条被淹没在夸张里。 |
| **v12.414.0** | 2026-09-03 | `4e2e7de` | **🚪 差距不在功能,在第一分钟**。竞品复核**连着两轮**把「上手门槛」判为最大战略劣势,而这一轮是逐条核过的:Dockerfile 有(三阶段构建),但**没有预构建镜像、没有 demo 工程**,Quick Start 仍是「git clone → npm install → 手工编辑 `.env.local` 填至少 3 个付费 key → npm run dev」;零代码平台那边是浏览器打开就能出片。好消息是底子早就有:`MOCK_ENGINES=1` + mock 三件套(确定性 SVG 分镜 / ffmpeg 纯色短片 / 正弦 WAV,零外部调用、零成本,且走**真实 provider 路径**)——缺的只是「一条命令把它跑起来」。新增 `docker-compose.demo.yml`:`docker compose -f docker-compose.demo.yml up` 打开 3100 端口即用,**不需要任何 API key**;配套 `scripts/seed-demo.mjs` 塞一个示例工程,免得新用户第一眼是个空工作台。两份 README 的上手小节都把它放在最前。**这一版真正的难点是那条诚实**:demo 产物必须写明是演示占位、**不代表真实生成质量** —— 拿占位片冒充成片正是这个项目一直在消灭的事(v12.394 那次 Ken Burns 占位片被当成成片、导致续跑永久跳过,教训就在那里)。另加两道保护:真引擎模式下**拒绝**塞演示素材(假素材不该污染真实项目列表),以及演示工程塞不进去也不阻断启动。测试 6 条,四处突变实测全拦 —— 其中一次突变最初被我做错了(该声明在文件里有两处,只改一处剩下的仍满足断言),重做后确认「诚实声明被删即红」。 |
| **v12.413.0** | 2026-09-03 | `b4c4165` | **⏸ 一次「一键成片」内部连着几十次付费调用,中途没有任何刹车点**。`lib/budget-enforce.ts` 管的是**按用户按月**的金额上限 —— 那道闸是对的,但粒度太粗:等月上限拦住时,这一单已经花掉了。新增 `lib/task-budget.ts` 补**单次任务内**的闸(借鉴 Devin 的 ACU),并接进主管线的出图与出片两处 + 逐镜里程碑汇报。两条硬约束:① **超限暂停,不硬失败** —— 硬失败会把前面已经花钱生成的东西一起丢掉,那是双输;暂停后已生成内容仍然有效,批准即从该步接着跑。② **暂停必须产出人能回应的东西** —— 这条是从 v12.401 学来的:一道通向不了人的告警,和没有告警是一回事。所以暂停时必须带出「已花多少 / 卡在哪一步 / **再放行多少能继续**」;没有这三个数,人面对一个按钮也不知道该不该点。追加设硬上限(初始预算的 10 倍)—— 一直批下去也该有个头。未设 `TASK_BUDGET_CNY` 时 limit=0 即不拦,行为与此前完全一致(零回归)。测试 8 条,四处突变实测全拦。 |
| **v12.412.0** | 2026-09-03 | `e0c6422` | **⚖️ 同一个模型既生成又打分**。`lib/vision-audit.ts` 用 `API_CONFIG.openai.*` 调视觉评分 —— **和写分镜 prompt 的是同一个模型、同一套配置**,它在给自己的产出打分。先说清一件好事:提示词那一层是干净的 ——`buildAuditPrompt` 只喂「剧本要求 + 画面」,不含生成时的 prompt,没有措辞层面的暗示泄漏。但**模型层面的自我合理化**仍在:同一个模型对「我理解的『雨夜街头』」有一致先验,更容易认为自己画对了。新增 `lib/grader-config.ts` 提供独立评分上下文,默认回落主配置(零回归)。**这一版真正的重点是那条克制,而不是那个开关**:`independent` 是**算出来的**(模型/端点/凭据确实不同才为 true),不是一个配置项 —— 把 `GRADER_MODEL` 设成与生成同一个模型时它仍为 false,并大声告警说明「当前是自评」以及怎么才能真正独立。加个开关很容易;**不许在没真正独立时宣称有独立 grader**才是难的那半,而那正是这个项目一直在消灭的假绿。测试 5 条,四处突变实测全拦。 |
| **v12.411.0** | 2026-09-03 | `cfdeb21` | **🔓 一个主打「MIT 开源、可自托管」的项目,注册表里全是闭源商业 API**。本轮竞品复核**直接推翻了上一轮的 C2**(「生成层已红海,开源侧进不了第一梯队」)——截至 2026-09 这条不成立了:**Wan 2.7**(阿里,2026-04,**Apache 2.0**)1080p/15s + 原生音频 + 声音克隆 + 指令编辑,14B 模型 24GB 显存(RTX 4090 可跑);**LTX-2.5**(2026-08,宽松商业许可,年营收 <$1000 万免费商用)单次 diffusion 同步出音视频、FP8 量化后 16GB 显存即可,**Artificial Analysis 榜 I2V 第 3 / T2V 第 4,高于闭源的 Sora 2 Pro**。于是那个很难看的事实浮出来:**用户可以自托管这个应用,却必须为每一帧画面向别人付费、并承担别人的停服风险** —— 而停服不是假设:MiniMax Music 停过(410,无预告,见 v12.410);Seedance 2.0 海外 API 因好莱坞六大版权停止函被中止(2026-03,纠纷至今未和解)。新增 `services/selfhost-video.service.ts` + provider 注册项,**优先级排在所有闭源之前**(自托管零边际成本,配了就该优先用);未配置时 `available()` 为 false,整条链行为与此前完全一致(零回归)。**刻意做成通用适配器而非「Wan 2.7 service」**:这两个模型迭代极快(Wan 2.1→2.7 不到一年),绑死某一版下次升级就要重写 —— 那正是 v12.402/403/404 连着三版在还的债。只约定最小 HTTP 契约,同步(直接给 url)与异步(给 task_id 再轮询)**都支持**,因为自建服务两种写法都常见,只认一种会把一半用户挡在门外。轮询时瞬时非 200 不判死(自建服务重启/冷启很常见),明确 failed 立刻抛。测试 7 条,五处突变实测全拦。两处连带修正:① 测试第一次跑时被 `safeFetch` 的**真实 DNS 解析**拦下(报「DNS 解析失败」而非我造的失败)—— 这反证 SSRF 防线确实在路径上,已改为 mock 该层并留一条源码级断言单独守它;② v12.203 那条 music 路由测试因 v12.410 的注册表**按 key 门控**而红 —— 旧路由不检查 key 就直接 new,新设计跳过未配置的 provider(而不是拿空 key 去调、换回一个看不懂的 401),已按新设计更新。 |
| **v12.410.0** | 2026-09-03 | `2f45a1b` | **🎵 BGM 不是「还没做」,是「做过、现在坏了、没人发现」**。全仓 **7 处** BGM 调用点**全部直连** `minimaxService.generateMusic()`(music-2.6),没有任何兜底 —— 而 MiniMax Music API 已对新用户停服(**410 + 2153,无预告**),于是写在 README 上的「按剧生成 BGM」实际是**断服状态**:用户一点就 502。这是 v12.402 那条教训的直接续集:同一家供应商停 Music API 时是无预告的,而我们把一整项能力压在它一家身上。**单点依赖 + 无兜底 = 供应商替我们决定功能生死。** 新增 `lib/music-providers.ts` 唯一入口 + 优先级兜底链(与 video-providers 同构):① **ElevenLabs Music**($0.15/min,官方 API、授权语料商用最干净)作主力;② **自托管**(ACE-Step 1.5 / YuE,Apache-2.0 / MIT)—— **唯一不会被供应商停服掐死的一条**,也最契合本项目「开源可自托管」的定位;③ MiniMax 留在**链尾**(它就是当下停服的那家,排前面等于每次先白等一轮;但老账号可能仍可用、恢复了会自动被用上)。全挂时的错误必须说清**每一家分别为什么失败** —— 只说「作曲失败」的话,用户无从判断该充值、该换 key、还是该等恢复。7 处调用点(路由 1 + editor-agent 5 + 兜底 1)**全部改走注册表** ——「只改主路径不改旁路」正是这个项目最常犯的病。BGM 已纳入 v12.409 的接线门禁(现 4 项)。**顺带被消费方门禁当场拦下一次**:我给两个新 provider 写了裸 `fetch` —— 而 `MUSIC_SELFHOST_URL` 是用户可配的 URL,正是 SSRF 规则要防的形态(v12.235 病史:裸 fetch 默认 redirect:follow,攻击者可 302 到 169.254.169.254 绕过整道防线),已改走逐跳重验的 `safeFetch`。测试 8 条,四处突变实测全拦;另记一坑:ES 模块有缓存,`_resetMusicProviders()` 后再 import 内置注册不会重新执行副作用 —— 第一次写这条测试就栽在这儿(先跑的过、后跑的拿到空表),已改为一次性快照。 |
| **v12.409.0** | 2026-09-03 | `a100185` | **🔌 我在讲「造好没接线」的那一版里,自己又犯了一次**。v12.407 给 VeoService 加了 `generateExtended()`,v12.408 加了弱镜修复分流,并在说明里亲手写下「**造好不接线正是这一版要治的病本身**」——然后本轮竞品复核的 agent 逐处 grep 后指出:`generateExtended()` 全仓零命中、`editImage()` 全仓零命中、`repairs` 无人消费。我自己核了一遍,**它是对的**;而 `editImage` 唯一的搜索命中,正是我在 v12.408 里写的那句「它零调用方」的注释。本版把两条真正接通:① `hybrid-orchestrator` 的 Veo 分支此前**写死 `duration: 8`**,而同一 switch 里别的引擎都用 `shot?.duration` —— 剧本写 15s 的镜走 Veo 只出 8s **且不报错**;现按剧本时长走 Scene Extension,段数受 `VEO_MAX_SEGMENTS`(默认 3 段 ≈24s)约束,因为续接是多次生成、成本随段数线性上涨。② `regenerate-storyboard` 端点接受审计驱动的修复上下文,按 `chooseRepairStrategy` 分流:局部偏差走 Kontext 只改问题处,**失败自动回落整张重生**(修不成不能变成这一镜没了);不给修复上下文时行为与历史完全一致,零回归。**更要紧的是把这件事固化**:新增 `scripts/wired-capability-gate.mjs` —— 维护「能力 → 必须存在的消费方」表,在生产方以外找不到调用点就红。它剥注释后再找,免得被自己写的说明骗过(那正是本版要防的自欺)。已实测:撤掉接线即报红、退出码 1。preflight 8→**10 步**,CI 同步(`gate:parity` 已核)。这类病不会让任何测试变红 —— 函数有单测、类型也对、CI 全绿,只是**产品里没有一条路会走到它**;靠人记不住,靠下一轮 agent 抓也太晚。测试 6 条,五处突变实测全拦。 |
| **v12.408.0** | 2026-09-03 | `2f7b534` | **🩹 Kontext 局部重绘早就造好了,一次都没被调用过**。`services/fal-flux.service.ts` 有 `editImage()`(走 `fal-ai/flux-kontext/max`),但**全仓零调用方** —— 竞品复核那条 C7「已接入 provider 的特色能力大量闲置」说的就是这个:接入成本已经付过,能力一直躺着。而现状是 Vision Audit 判低分 → `rebirth-plan` 排弱镜 → **一律整张重生**。整张重生有两个代价:① 贵;② **会把这一镜里本来已经对的部分**(角色长相、光线、构图)一起重新掷骰子 —— 常见结果是「修好了动作、人却变样了」,于是又触发下一轮重生。新增 `lib/repair-strategy.ts` 按「错得有多整体」分流:`fail`(<50 分)或最弱维度是 `sceneMatch` → 整体跑题,局部改无从改起,走整张重生;最弱维度是 action/mood/composition 的 warn 档 → 走 Kontext 局部重绘,**保住已经对的部分**。重绘指令刻意只描述要改处并显式要求其余保持原样 ——否则 Kontext 也会顺手改动别处,那就退化成「换个方式整张重生」。**一条刻意的保守**:没有维度数据时一律整张重生,不猜 —— 猜错的代价(改半天没改对还多花一次钱)比直接重生更差。已接进 `oneclick-film` 的闭环裁决并把统计写进消息(**造好不接线正是这一版要治的病本身**)。顺带修掉一条被文案绑住的旧断言:v9.4.4 那条锁的是 `message` 匹配 `/自动重拍/` —— 现在弱镜分两种修法,「重拍」二字已不准确,文案改准后它就红了,而行为一字未变。已改成锁行为(每个弱镜都要有明确修法)。测试 8 条,五处突变实测全拦。 |
| **v12.407.0** | 2026-09-03 | `79d19a4` | **🎬 Veo Scene Extension:把成片接长,而不是让单次生成变长**。这一版的由来是**我自己写错的一句话** —— v12.401 竞品复核时我把「Veo 3.1 单次 60s+」写进了 README,被独立二次检索当场推翻:那 60s+ 是 Scene Extension **续接**出来的,单次生成上限仍是 8–10s。所以新增的 `lib/veo-scene-extension.ts` 里,命名/日志/返回值一律说「段数」而不说「时长上限」——**代码本身要挡住下一个人重犯同一个错**。三条硬约束落进实现:① 续接入参是**前一次任务的 task_id**(不是视频 URL)—— 此前这个 id 轮询完就丢了,所以续接根本无从谈起,现已留存;② 网关不一定暴露续接端点,那不是生成失败,`isExtendUnsupported` 与内容/额度失败严格区分;③ **续接失败绝不能把已经生成、已经计费的首段一起丢掉** —— 这个项目的老毛病就是「一处失败拖垮整体」,现在接不上就把首段还回去 + 说明原因,由调用方决定接受短片还是重试。**写测试当场逼出两个真 bug**:① 我自己的参数顺序写反了(`generateVideo(imageUrl, prompt)` 却按 `(prompt, imageUrl)` 调);② **`normalizeStatus` 不认 `'succeeded'`** —— 它只收 succeed/success/completed/finished,网关一旦返回 `succeeded`(MiniMax V2、Vidu 都用这个词),已生成、已计费的任务会白轮 60 次到超时,而报出来的是「超时」,人会去查生成侧、查不到真因。与 v12.122 那个 Kling `Fail`(无 -ed)**同型**:状态字面量差一个后缀,表现却像是生成失败。测试 7 条,五处突变实测全拦;另修一处测试本身的坑 —— 一次性推进 60s 假时钟会把 `fetchWithTimeout` 内部 30s 的 abort 定时器一起触发,表现成「测试超时」而真因是时钟推得太猛,已改按 5s 步进。 |
| **v12.406.0** | 2026-09-03 | `09c945c` | **🎞 把已接入的用满:Kling 3.0 一次调用出 6 个连贯镜头**。`kling-v3` 早就在 provider 注册表里,我们一直只用它的单镜生成 —— 竞品对比表第三列「我们用了多少」在这一行是 **0**。多镜的价值不只是省调用次数:**同一次生成内的镜间一致性由模型自己保证**,比我们事后用角色 DNA + vision retry 去拼要稳。官方字段表(2026-09-03 核):`multi_shot: true` + `shot_type: 'customize'` + `multi_prompt: [{index, prompt, duration}]`,约束是硬的 —— ≤6 镜(index 0–5)、单镜 **≥3 秒**、合计 **≤15 秒**、prompt ≤2500 字;越界不是「效果差一点」而是**整次调用被拒**,而一次多镜被拒 = 6 个镜头一起没了,比单镜失败贵得多,所以规整放在本地做。**最要紧的一条设计**:放不进本次调用的镜头**原样回传**(`plan.overflow`),绝不截断丢弃 —— 静默少几个镜头,成片看起来仍然「成功」,只是故事缺了一段,而没有任何地方会报错;这个项目吃过「清理任务把还在被引用的素材当孤儿删掉」的亏。另加一道提前拒绝:**非 v3 模型直接抛错**,因为在别的模型上这些字段会被上游静默忽略,产出一个「看起来正常但只有一镜」的成片 —— 又是「失败长得像成功」。测试 9 条,六处突变实测全拦(含字段名写错、溢出被丢)。 |
| **v12.405.0** | 2026-09-03 | `ae95458` | **🎬 只有 `/api/create` 这条路上的用户,一直在拿 Kling v1 标准档出片**。同一个供应商有**两份 service**:`kling.service.ts`(528 行,kling-v3 + Elements 多主体 + 首尾帧 + 4K + 运镜 + prompt 裁断 + 横竖屏,provider 注册表用它,17 处调用)与 `keling.service.ts`(123 行,写死 `model_name: 'kling-v1'` + `mode: 'std'`,不传 aspect_ratio、不裁 prompt、以上能力一项没有)。而**主管线 `agent-orchestrator`(服务 `/api/create`)用的是后者**,两处调用点。README 竞品表写着「Kling 3.0」、仓库别处配的是 v3 —— 只有从 `/api/create` 进来的用户拿到 v1 标准档。**「主路径修好了、旁路没跟上」的又一次,而且旧的那条恰恰是用户撞到的那条。**删之前先做了两件事,顺序不能反:**幸存者反而缺两项加固** —— 它自带一份 `fetchWithTimeout`(v12.304 要消灭的正是「各自定义」),且**完全没接 poll-policy**(v12.329),任何一次 5xx 都会把**已在生成、已计费**的任务直接判死。所以先给幸存者补齐这两项,再把 v12-304 / v12-329 两条测试的断言对象迁到它身上,最后才删文件 —— 反过来做就等于把这两项保障静默丢掉(而测试还是绿的)。顺带修掉 v12-329 里一条**锁变量名**的断言:它写死 `classifyPollStatus(response.status)`,而 Kling 轮询有两个端点(image2video 失败要回落 text2video)、失败的是 `response2` —— 行为完全正确、断言却红,和 v12.122 同一个毛病,已改为锁行为。测试 6 条,四处突变实测全拦(含「重建第二份实现」也会立刻红)。 |
| **v12.404.0** | 2026-09-03 | `adc1ba6` | **🎭 角色锁脸可能早已在 MJ 路径上静默失效,而产物看不出来**。`midjourney.service.ts:81` 一直在发 `--cref <url> --cw <n>`,而**全仓没有任何一处声明过 MJ 版本** —— 走网关默认。官方在 V7 用 **Omni Reference**(`--oref` + `--ow`,1–1000 默认 100)取代了 V6 的 Character Reference,且 Character Reference 文档**直接叫 V7 用户改用 Omni Reference**。两件事叠在一起:**若网关默认是 V7,我们发的 `--cref` 就是个无效参数** —— 而 MJ 不会因为多了个不认识的参数报错,它照样出图,只是角色不锁了。于是产物依然好看,只是不是同一个人。**失败长得像成功**,这是最难发现的那一类。修法不是「换个参数名」,而是**不猜、声明**:新增 `lib/midjourney-params.ts`,① 每次请求显式带 `--v`(让版本成为我们决定的事,而不是网关默认值决定的事);② 参数按版本切(V7 `--oref/--ow`,V6.x `--cref/--cw`);③ 权重按各自区间夹住(越界时 MJ 同样不报错、只是不按你想的来);④ `MJ_VERSION` 只接受数字形态,乱填不会被拼进 prompt。**⚠️ 本轮无 MJ 额度,未做真机出图验证** —— 所以只断言请求形态,不声称「一致性已修复」:那需要出图比对,列为下一轮真机验收项。测试 7 条,五处突变实测全拦;其中一条测试第一次跑就被 `cref?: string;  // --cref …` 这样的**尾随注释**绊住 —— 剥注释只处理整行 `//` 是不够的,已补。 |
| **v12.403.0** | 2026-09-03 | `0a41638` | **📉 这条 Vidu 路径从来就跑不通,而决策日志还在替它撒谎**。v12.401 复核时为核实「我们接的 Vidu 是哪一版」去读源码,发现它**一个 `model` 字段都不传**;顺着官方字段表逐项对下来,错的是**六处**:创建端点(`{base}/v1/video/generate` vs `https://api.vidu.com/ent/v2/img2video`)、鉴权(`Bearer` vs **`Token`**)、图片入参(`image_url` 字符串 vs `images` 数组)、**必填 `model` 整个漏掉**、查询端点(`/v1/video/query/{id}` vs `/ent/v2/tasks/{id}/creations`)、状态字段(`status`+`completed` vs **`state`**+`success`,取片是 `creations[].url`)。六处全错意味着它**每次都失败、每次静默回落到 Kling** —— 所以谁也没发现。而 `app/api/u2v/route.ts:76` 还把结果标成 `model: 'Vidu-Q3-Pro-15s'`:**决策日志里记着一件没发生过的事**。这个项目有「逐镜可审计决策日志」这项能力,一条撒谎的记录比没有记录更糟 —— 它让复盘从错误的前提开始。已按官方字段表重写整个 service(2026-09-02 核文档),默认钉 `viduq3-pro`(Q3 才有短剧特效包与多主体锁定,正是我们要它的理由),`VIDU_MODEL` 只接受官方 id、乱填回落默认而不是把一个必然 400 的请求送出去。标签改为派生自**实际发出的模型**。另修两处连带的:① `getVidu()` 返回 `any`,把契约整个抹掉 —— 所以给它传了个官方根本不存在的 `style` 字段,tsc 一声不吭,已补真实类型;② provider 注册表把 `maxDurationSec` 写死 8,而 Q3 官方支持 1–16s,等于把长镜能力挡在门外,已改 16。测试 7 条,六处突变实测全拦。 |
| **v12.402.0** | 2026-09-03 | `6367a55` | **🔌 我们把 MiniMax H3 当竞品写进了对照表,自己却还在调它的上一代**。`services/minimax.service.ts:240` 默认 `MiniMax-Hailuo-2.3`、`.env.local` 没覆盖它 —— 即实际跑的就是这个默认值,而 2.3/2.3-Fast **已被官方降为 legacy**;同一家供应商停 Music API 时是**无预告**的(410 + 2153),legacy 端点没有任何宽限承诺。**但这不是改个字符串的事**:H3 在 Video Generation V2,而 V2 与 v1 有四处不同 ——请求体(扁平 `prompt`+`first_frame_image` vs `content` 多模态数组 + 必填 `resolution`/`duration`/`ratio`)、轮询路径(query string vs path 参数)、状态字面量(`Success`/`Fail` vs `succeeded`/`failed`/`cancelled`)、取片方式(`file_id` 再换一次 vs 直接给 `content.url`)。写成 `if (isV2)` 就是本项目反复栽的那个形态:同一语义两份实现。故新增 `lib/minimax-video-api.ts` 收敛全部版本差异,service 只保留**一条请求流、一条轮询流**。默认翻到官方推荐的 H3,并按官方字段表逐项发送(2026-09-02 核对文档)。套餐若未开通 H3(历史上 I2V-01 就报过 2061 "plan not support model"),**回落到 legacy 但大声告警** —— 不静默替换:静默替换会让人以为自己在用 H3 而实际不是;且与「额度用尽」严格区分(额度用尽换模型没用,那要走 Fast 的独立额度)。**测试当场逼出两处写死的端点**:`generateVideoFast` 与 `generateVideoS2V` 各自硬编 `/v1/video_generation` ——它们的模型确实是 v1 族,但写死意味着哪天换成 H3 变体就会静默打错端点,而错端点返回的错误长得像「模型不存在」,人会去查模型名、查不到真因;已统一派生自 `videoCreatePath(model)`。**顺带重写了 v12.122 那条测试**:它①测的是测试文件里自己抄的一份正则(生产的改坏了它照样绿),②数的是某文件里正则出现几次(期望 4)—— 本次重构行为一字未变、数字变 3 就红了。改成直接拿生产函数过状态字面量 + 逐处检查而不写死数量。测试 12 条,四处突变实测全拦(含一次窗口开太宽导致漏判、已收紧)。 |
| **v12.401.0** | 2026-09-02 | `28ae7a8` | **🔭 竞品分析:两处初稿事实被复核推翻,以及一个「设置了但通向不了人」的自动化**。本轮按历史几轮的方法论重做全景调研(六个视角 × 交叉复核),产出 `docs/COMPETITIVE-GAP-2026-08.md` 与结论台账 `docs/competitive/claims.json`(12 条可证伪断言)。**两处我已经写进 README 的数字被独立二次检索推翻**:① Veo 3.1 首发是 **2025-10-15**(2026-01-13 只是功能更新),它的 60s+ 是 **Scene Extension 拼接、单次生成上限仍 8–10s**,而 **Lyria 3 是 Google 另一个独立音乐模型、根本不是 Veo 的音频组件**(Veo 自带联合音视频生成);榜上 1237 的 Elo 属于 Gemini Omni Flash,Veo 3.1 自身约 1091。② 我写了「Runway Gen-4.5 无原生音频是硬伤」—— **它自 2025-12-11 就有原生音频**,它掉出前十是被超越,不是缺音频。**这两条正是「各项要求不低于历史轮次」里那条『关键数值须独立二次检索复核』存在的理由 —— 本轮它当场兑现了两次。**自查还翻出两条更要紧的:**我们默认在用的 `MiniMax-Hailuo-2.3` 已被官方降为 legacy**(H3/V2 才是推荐路径),**接入的 Vidu 还停在 2.0 级别**(Q3 才有短剧特效包与 7 参考图多主体锁定)—— 都是「早就接了但版本落后」,已列为 v12.402 / v12.403。国产侧另一条代差:Seedance 2.5 与 Wan 3.0 已做到**原生 30s 单镜头**,而拼接路线单次仍是 8–10s。**而这一版真正要记住的,是自动化本身差点也成了假绿**:第一版 launchd 任务把结果重定向进 `~/Library/Logs/…log` 就结束了 —— **那个日志没有人会去打开**,超期时 `exit 1` 落进一个没人读的文件,机制存在、能跑、退出码也对,唯独**没有任何一条路径把结论送到人眼前**;它还**无条件**每周往工作区掉一个 TASK 文件,而每周都出现的东西第三周就成噪音,到真超期那次反而没人看 —— 和「一个会误报的门禁只会训练人忽略门禁」是同一条。改为 `--if-due --notify`:超期才动作,动作必通向人(macOS 通知 + 任务书落进 `git status`)。顺带把「历史轮次指路牌」也从『记得改』变成『不改就红』:新增一轮而旧文档没跟上,测试立刻红。测试 8 条,四处突变实测全部拦下。 |
| **v12.400.0** | 2026-08-31 | `34277ec` | **🔭 同一个病这一轮犯了至少八次 —— 做一把能提前看见它的尺子**。v12.375 主管线按角色名选音色而 recompose 硬编码;v12.381 localize 过滤多语稿而另两处直接取 `[0]`;v12.383 立了唯一入口却只手工接了 3 个消费方(实际 11 个);v12.385 项目级 regenerate-shot 落盘 + 标降级而顶层两样都没跟上;v12.387 shot-audio 读手动音色覆盖而 recompose 不读;v12.388 付费门禁只认直接调用漏掉编排器;v12.393 refCount 被算了三遍三种口径;v12.399 自检只给了两条清理路径中的一条。**每一次都是「主路径修好了、旁路没跟上」,每一次都等到出事才发现。** `consumer-gate` 能拦「绕过唯一入口」,却拦不了「还没有唯一入口、两处各写各的」—— 那正是这八条的共同形态。新增 `scripts/twin-path-scan.mjs`:它不下结论,只把结构性风险摆出来 —— 某个下游能力被多处调用、而各调用点传的参数集合不一样。那不一定是 bug(不同入口本来就可能需要不同参数),但它是「其中一处改了、别处忘了」的**必要条件**。**它当场找出了第九次**:`dispatchTTSGenerate` 有 **8 处调用、7 种传参形态**,而 `shot-audio` 与 `voice-retake` 都传 `speed`/`pitch`(按情绪推导的韵律,v2.9 起、v12.274 逐档配过),**只有 recompose 的配音重生一个都不传** —— 同一句台词走重合成出来就是**平读**:该急的地方不急,该沉的地方不沉。已修:情绪从剧本一路带到 clip,再推导韵律传给 TTS(实测愤怒 speed 1.12 / pitch +2、悲伤 0.9 / −2、中性 1.0 / 0)。扫描本身**只报告不阻断、不进 preflight** —— 它的产出是一份供人过目的清单,不是能自动判对错的规则;**一个会误报的门禁只会训练人忽略门禁**。测试 9 条。 |
| **v12.398.0** | 2026-08-31 | `aba5488` | **🛡 保护名单认不出内容寻址的引用;以及「一个都没被引用」时应该停手**。接着 v12.394 往下查:那版修好了 URL 编码路径,但引用集**仍然认不出 `?key=` 形态**。storage 是内容寻址的 —— 磁盘上叫 `<sha256>.mp4`,DB 里存的是 `?key=<sha256>`,**URL 里没有扩展名**,而抽名正则要求后缀,一个都抓不到。实测:**613 条**引用是 key 形态,`data/storage/assets` 里 160 个文件按当时的引用集**受保护数是 0**。它今天不在清理列表里(只扫 composed / exports / media)所以还没出事 —— 但哪天有人把 storage 加进来,160 个素材会一次删光。补上 key 形态后 **0 → 149**。**更要紧的是第二件事:加一道自检。** v12.394 那个 bug 的形态是「保护名单恒空」——每一个被引用的成片都被判成孤儿,那次 owner 丢了 30 个项目 534 个素材。而一个非空目录里「一个文件都没被引用」,在正常运行的系统里几乎不可能 —— **远比「引用集算错了」更不可能**。所以现在:非空目录 + 受保护数为 0 → **拒绝清理**并报错说清原因(还点名 v12.394,让人知道这不是理论风险)。与其相信自己的正则,不如在这一刻停手:少清一次磁盘,总好过再删一次素材。**如果这道自检早就在,v12.394 那个 bug 根本不会造成损失。** 配套改动:遍历改成只**统计**、删除挪到自检之后(边走边删的话,等发现「一个都没被引用」时前面的已经删了);dryRun 走同一条自检(否则演练说「会删 100 个」而真跑却拒绝,两边对不上更让人困惑)。测试 9 条 —— 写的时候自证断言当场拦下我一次:窗口往前切了 300 字符,切到的是遍历循环而不是自检体。 |
| **v12.397.0** | 2026-08-31 | `6888721` | **🎨 四个图像 provider 里,有一个从来没被选中过**。owner 的重跑日志里场景图三个 provider 全挂(`Minimax multi-ref needs at least 1 ref` \| `flux-2-pro 401 Token quota exhausted` \| `MJ submit failed`),而第四个 —— `minimax-single`(纯 T2I)—— **一次都没出现在 tried 里**。它的 `maxRefImages: 0`,而 `selectProviders` 判「`refCount > maxRefImages` → 排除」:只要传了任何参考图(哪怕只是 styleBible 的风格锚),它就永远出不了场。结果是「**宁可一张图都没有,也不肯出一张没有风格锚的图**」。而对场景图来说参考图是**加分项**不是必需品:有当然更好,没有也总比整个项目卡住强。本版在主轮全失败后加一轮「丢掉参考图重试」,两条边界缺一不可:**①** 只在主轮全失败之后才走 —— 不抢正常路径、不改变正常情况下的选路顺序;**②** 必须如实标注 `refsIgnored` **并一路透传到人眼前** —— 只把 URL 存下来、不说它是怎么来的,就是又一次静默降级。标注贯穿四层:registry 打标 → 类型里有字段(不是靠 any 硬塞)→ API 写进资产 `data.refsIgnored` 并回给调用方 → 重跑脚本在日志里打「这张是**无参考图**出的(没有风格锚)」。降级轮里的失败也记进 `tried` 并带 `(no-ref)` 后缀,否则事后分不清是主轮还是降级轮挂的。测试 9 条,其中 4 条是**真行为测试**(注册两个假 provider —— 一个「要 ref 但必然失败」、一个「不支持 ref 但能出图」—— 验证降级轮确实被触发、且主轮成功时不触发)。 |
| **v12.396.0** | 2026-08-31 | `8a8b124` | **🧪「数断言」判不出测试的真实保护力**。一次静态扫描告诉我「全仓 106 个谓词里,**12 个只有负例断言**」。逐个实证之后 —— **只有 1 个是真的,误报率 11/12**:`isAncient` / `isHorror` 看着也只有 `toBe(false)`,但它们有**间接**正例覆盖(`detectGenreKind` 与 `lock()` 的「题材锁定」字段);`isDissolveTransition` 的正例在同文件更靠下、`parseEditIntent` 用的是 `toEqual` 而非 `toBe` —— 我的正则一个都没看见。**唯独 `isSad` 是真的**:它落在 `detectedMoods` 这条支线上,而 `lock()` 提取的是「题材锁定」字段、不含「情绪基调」,于是没有任何间接覆盖 —— 把它改成 `() => false`,全仓测试**照样全绿**,而它唯一的消费方 `prompt-templates` 会因此永远不给 LLM 加「悲情基调」:owner 写悲剧题材时,脚本 prompt 里少掉这一层情绪指示。已补 5 条单元正例 + 2 条**全链路**用例(单元正例只能证明「正则认得悲伤」,证不了「它真的进了给 LLM 的 prompt」——中间那一环恰恰是没人验的那条);反向验证:再把 `isSad` 改坏,6 条测试同时变红。**结论是方法论层面的:一个断言有没有用,只有把实现改坏、看它红不红才知道。** 所以留下 `scripts/mutation-probe.mjs`(变异探针,按需跑:`node scripts/mutation-probe.mjs <函数> <源文件> <测试文件>`),它**刻意不进 preflight** —— 要改源文件并跑测试,太重,而且改源文件的东西不该在每次发版时自动跑;改动前备份、`finally` 里无条件还原。测试 8 条,含「探针跑完必须把源文件原样还原」(探针改了源码却不还原,比它要查的问题更糟)。 |
| **v12.395.0** | 2026-08-31 | `332c86a` | **♻️ 视频有配额判定,图像没有 —— 同样的失败每天重试三次**。owner 的重跑日志里,同样 4 个场景图在 09:00 / 14:00 / 20:00 各白跑一轮:`❌ 场景 万人演唱会现场 图像生成失败: … image(flux-2-pro) 401: {"error":{"message":"Token quota exhausted"}} …`。报文说得很清楚(**401 + Token quota exhausted** —— 青云top 的口径,既不是 402 也不含 insufficient,`quota-vocab` 的注释里专门记过这个坑),v12.376 的分类器也确实把它判成 `PROVIDER_ARREARS` / `retryable: false`。**问题是脚本对图像失败压根不调那个判定** —— v12.377 只给视频接了 `shouldStopForQuota`,图像侧一直没接,于是额度耗尽后剩下的角色/场景/分镜被一个个全试一遍,每个都必然失败,一天三轮 = 同一批图白跑 12 次。修法把判定放进三步**共用**的 `log()`、循环头短路 —— 一处接线覆盖三步。**关键边界:只停图像、不碰视频**,两套额度是分开的,v12.367 就是反过来栽过一次(视频尽了却把图像也停了,卡住 53 张图);退出码 3 也仍只由视频配额决定,因为它会让调用方关掉**后续项目**的视频步骤。拿不到证据时不停 —— 图像侧宁可多试,别把一次网络抖动当成额度耗尽。测试 8 条。 |
| **v12.393.0** | 2026-08-31 | `f201bc5` | **📏 守卫守的是一个和被守方口径不同的数**。v12.371 给 provider 加了 `minRefImages` 下界,本意是让「0 参考图」的请求不要白试一次 MiniMax multi-ref。可 8/30 定时任务的日志里那句报错照旧出现:`❌ 场景 万人演唱会现场 图像生成失败: Minimax multi-ref needs at least 1 ref \| …`。原因是**同一个量被算了三遍、三种口径**:`plugin-chain-router` 算 refCount 用 `.filter(u => !!u)`(只要非空);`minimax-multi` 内部用 `.filter(startsWith('http'))` + `Set` 去重 + 上限 4;`gateway` 内部用 `.filter(startsWith('http'))` + 上限 4 但**不去重**。于是传一个 `data:` URI 或 `/api/serve-file?…` 本地引用图:router 算 1 → `minRefImages: 1` 满足 → 放行 → provider 内部过滤后是 0 → 抛错。**守卫看的数和 provider 看的数从来不是一个数,那道下界等于没设。** 新增 `lib/image-providers/refs.ts` 作唯一入口(口径以 provider 的实际需求为准:只有 http(s) 能发给引擎、重复的没有意义、超上限的会被丢),三处全部改走它。顺带修掉 gateway **不去重**这一处:传三张相同的图会真的发三份给网关,既费带宽又可能被上游当成三张不同的参考图。**实测同一组输入下的分歧**:一个 data: URI → 旧算 1 / 实际 0;一个本地 serve-file → 旧算 1 / 实际 0;三张相同的图 → 旧算 3 / 实际 1;六张 → 旧算 6 / 实际 4。测试 9 条,其中一条把**旧口径**也写进测试当对照 —— 光证明新口径对没有意义,得证明它和旧的确实不同、且不同在哪。 |
| **v12.392.0** | 2026-08-31 | `3b1f3f6` | **⏳ 昨晚 20:00 的定时重跑,跑的是旧代码**。日志里出现「⛔ 拿不到失败报文,保守处理」—— 那是 v12.377 自己加的兜底,于是当轮视频重跑被白白停掉。我一路查下去:先怀疑编排器有第二处 Ken Burns 回落(**确有**,line 3779 是主管线那处,但定时任务走的不是它)、再查 `regenerateShot` 有几个 return 点(只有一个,就是我改的)、再实测端点 —— 返回里没有 `engineFailures`,却有一个我不认识的 `degradedReason`。最后才想起去看进程:**dev server 起于 8/29 10:38,而 v12.377 的编排器改动是 15:17 提交的,中间差了近五小时,进程从没重启过**。我改完那一刻手工实测是好的(HMR 重编译了那条路由),当晚定时任务跑同一条路径拿到的却是 `undefined`。**这类问题最难查的地方在于:不是代码错了,而是「你以为在跑的代码」和「实际在跑的代码」不是一个东西** —— 日志、行为、响应字段全都指向别处。长期运行的 dev server + 定时任务的组合,能让这种脱节静默持续好几天。新增 `GET /api/build-info`(只回进程启动时刻 / uptime / pid,无鉴权且不碰任何项目数据,已按 `paid-gate: ok —` 写明豁免理由),`rerun-cron.sh` 在跑之前拿它与 `git log -1 --format=%cI` 比对,**旧了就醒目报警**并在结尾再报一次(长日志里开头那行会被淹掉);**只报警不中止** —— 停掉等于当天什么都不做,而图像步骤仍有价值。当场实测:报出「dev server 比代码旧 **41 小时**」。**第一版设计是错的,记下来**:我最先写的是「模块加载时定格 version,与磁盘 package.json 比对」—— 它测不出任何东西,因为那个 route 是**新建的**,Next 刚编译它,定格值永远等于当前值。**在 HMR 模型下新模块永远是新的 —— 一个刚出生的哨兵报不出别人有多老**;能反映「进程有多老」的只有进程级事实。测试 9 条,含「不再拿模块内定格版本当判据」的反向锁。 |
| **v12.391.0** | 2026-08-30 | `60cbb35` | **🧾 一次 rebase 把 CI 弄红了,而红的原因暴露了我自己写的一条坏测试**。v12.390 推送时远端有一个 CI 自动提交(刷新 star 曲线),rebase 后三个提交的哈希全被重写 —— 而 VERSIONS.md 里记的还是 rebase **之前**的(`03826c8 → 35d6a55`、`9ba10c9 → b713d32`)。v12.290 建的「变更日志可溯源」门禁当场判红,**判得对**:表里记的哈希在 main 历史中不可达,正是它存在的理由。跑一次 `sync-version-hashes.mjs` 修正 2 条即可。**但连带红的另一条才是真问题**:v12.390 我写的测试里有 `execFileSync('npm', ['run', 'preflight'])` —— 它想验的只是「preflight 里有几步标了 CI 会跑」,却把**整条 preflight** 拉进来跑。于是 preflight 中**任何一步**失败(这次是无关的 version-hash)都会连累它。一个单元测试不该依赖一条会因无关原因失败的重命令:它让失败信号指向错误的地方 —— 我第一眼看到的是「v12-390 测试红」,而真因在 VERSIONS.md 的哈希。已改成直接解析 `preflight.mjs` 的 STEPS 数组算数,不再执行它。**教训归档**:这一轮我为了把版本拆干净做了三次 rebase,每次都要记得回填哈希;而「测试里跑整条命令」这种耦合,平时不出事、一出事就把排查引向错处 —— 和这一轮修的那些「错误信息盖掉真因」是同一类。测试仍 8 条(改的是实现方式,不是覆盖面)。 |
| **v12.390.0** | 2026-08-30 | `d84bdd7` | **🫥 否定式断言天然容易假绿 —— 把它做成第三类检测**。`expect(win).not.toContain('x')` 在窗口切歪时**必然通过**:indexOf 返回 -1、右界排在左界之前、目标文件读错 —— 任何一种都会切出空串或错段,而否定断言在空串上永远成立。这一轮我反复栽在这个形态上:**v12.340** 锚点落在注释上、窗口切到 line 19–61 而要验的代码在 67(真假绿);**v12.384** 我自己写的测试因请求体缺字段 400 早退,`not.toHaveBeenCalled()`「通过」了却什么都没验;**v12.385** `toContain` 命中批量路径,单镜路径从没被验过。手工加「窗口自证」加了好几次,本版把它固化进 `fake-green-gate`:**同一个 it 里,若某个来自 slice/substring 的变量只被 `not.*` 断言过、没有任何正向断言,判为假绿**。全仓扫出 **9 处**(含我前一版刚写的 v12-389),已逐条补上自证 —— 其中 v12-286、v12-309、v12-310、v12-328 四处的窗口右界都用了 `indexOf(...)` 而没验过它 > 左界,那正是「切出空串」的高危形态。判定刻意做成**否定侧严格、正向侧放宽**:正向侧认根标识符,所以 `expect(win.length).toBeGreaterThan(0)` 也算自证 —— 判得太死会逼人补一条没意义的断言去哄门禁,那本身就是新的假绿(这条边界是修 v12-316 时当场遇到的:我加的长度断言被判不合格,该改的是门禁而不是再写一条哄它的断言)。顺带把 `fake-green-gate` 也纳入 CI —— v12.326 建它时注释只写了「CI 尚未纳入」,从没说过为什么不能进,而它是纯 TS AST 静态分析,读的就是仓库里的测试文件。现在 **preflight 9 步中 8 步在 CI 上跑**,仅「README 媒体体积预算」留在本地。测试 8 条(含三种自证形态的分辨、以及「前两类检测没被这次改动弄坏」的回归)。 |
| **v12.389.0** | 2026-08-30 | `35d6a55` | **🪞 preflight 自称「与 CI Security job 同步」,而它一半的检查 CI 根本不跑**。实况:本地 preflight 8 步、CI security job 只有 4 步。v12.378 的锚点门禁、v12.382 的付费端点门禁都是新建后**只挂在本地**的 —— 而 preflight 跑完打印的是「✅ preflight: 8/8 通过(与 CI Security job 同步)」。那句话给人的印象是「这 8 条 CI 都会替我把关」,实际只有 4 条:真忘了跑 preflight 就推,CI 照样绿,门禁等于不存在。**这是关于门禁本身的假绿**,最不该有 —— 它让人对整套防线的覆盖范围产生系统性误判。本版三件事:**①** 把两道纯静态门禁放进 CI(它们只读源码、不依赖本地环境,没有任何理由留在本地);**②** 结语改成**由代码算出**的如实报数「N 步中 M 步 CI 也会跑」,而不是手写一句断言;**③** 新增**元门禁** `scripts/preflight-parity.mjs`:preflight 里没标 `(local only)` 的步骤,其脚本必须真的出现在 `ci.yml` 里;标了的则相反 —— 出现在 CI 里就说明**标签在骗人**。它自己也进 CI,防的是**下一次**漂移。判定按脚本文件名而非整行命令(CI 写 `node scripts/x.mjs`、preflight 写 `npm run gate:x` 调的是同一个东西);匹配不上就报,不假装同步。**双向反向验证**:从 ci.yml 删掉一步 → 当场点名;把已进 CI 的错标成 local only → 也报「标签在骗人」;解析不出任何步骤 → 直接失败而不是报绿(一个什么都没解析到却报绿的元门禁,正是它自己要防的那种假绿)。现在 **9/9 通过,其中 7 步 CI 也会跑**。写测试时还中了一次自己的招:断言「不再写死那句话」时忘了剥注释 —— 而那句话正留在文件抬头讲病根,这恰是 `contracts.ts` 设计原则第 2 条说的「把问题讲清楚不该让门禁报警」;已改为只看代码,并**同时断言注释里必须保留它** —— 删掉就没人知道这句话曾经骗过人。测试 12 条。 |
| **v12.388.0** | 2026-08-30 | `b713d32` | **🕳 一条只认「直接调用」的付费门禁,漏掉了最贵的那个入口**。两条 high 都来自多智能体扫描 + 对抗验证。**① `vision-audit/run` 只验登录、不验归属(IDOR)**:文件抬头写的就是「Auth: 登录用户」—— 是写码时漏的,不是后来删的。任意登录用户拿着**别人的** projectId POST 这里,服务端就对受害者项目的每张分镜图调 GPT-4o Vision,并在响应里原样返回对方的画面描述、台词、情绪标注 —— 既烧 owner 的额度,又把他的私有创作内容读走;而本文件自己的注释还写着「vision 调用**按镜烧钱**」,却连付费守卫都没有。两道闸一起补,并锁住「归属校验必须排在读库之前」——先查库再鉴权,数据已经泄露了。**② `/api/create` 完全没有鉴权**:裸 curl 一个 `{ idea }` 就能在生产模式下跑通整条 AI 制片管线(脚本 LLM → 分镜出图 → 视频生成),单次至少 ¥5–30,无预算上限、无频控。它还是条**遗留死路** —— 全仓搜下来前端零调用者(真入口是 `/api/create-stream`),也就是说它只对外部攻击者可见;没有直接删是因为删除不可逆、也可能有我不知道的外部调用方,加守卫后至少不再敞着。**但真正该追问的是:v12.382 我刚建的付费门禁,为什么没抓到它?** 因为那版的特征全是「直接调付费 SDK」(`dispatchTTSGenerate` / `callLLMWithFallback` / `new MinimaxService`…),而 `/api/create` 走的是 `new AgentOrchestrator()` + `startProduction(idea)` —— **一条都不命中**。一个只认直接调用的付费门禁,会漏掉所有把花钱包了一层的入口,而那恰恰是最贵的那类:编排器一跑就是整条管线。补上间接特征后识别数 **22 → 30**,全部有守卫;反向验证过:把 `/api/create` 的守卫拿掉,门禁当场点名它并说出命中的是哪条特征。**实测**:两个端点无 token 均 401;拿 demo 账号的 projectId 调 vision-audit → **403**,IDOR 确实被挡住了。测试 8 条。 |
| **v12.387.0** | 2026-08-30 | `13b4a66` | **🎚 owner 手动调好的音色,一走重合成就被自动分配顶掉**。主路径 `shot-audio` 的优先级链是 **force > overrides > routing > default**(`lib/voice-routing.effectiveVoice`,v9.7.7 起)—— owner 在 Character Studio 把某个角色的嗓子指定好,存进 `voice-overrides` 资产。而 **recompose 的配音重生只读 `voice-cast`(自动分配),完全不读 `voice-overrides`**:从重合成 / 本地化 / 广告工作台任一入口触发配音重生,他设的音色都会被自动分配顶掉,而且没有任何提示。又一次「主路径修了旁路没修」,而且这三个入口都是高频操作。修法刻意选**最小的那个**:overrides 的优先级本来就在 cast 之上,直接盖进同一张表即可,语义完全一致,不必改 `pickShotVoice` 的签名 —— 改的是内存里的 Map,**不会**把 overrides 写回持久化的 voice-cast(那会让「手动」和「自动」混成一团,以后分不清哪个是人定的)。overrides 取出来仍要过 `isKnownVoiceId`:它同样是持久化数据,可能躺着历史脏值(与 v12.375 同一条理由)。**实测**:给项目 1 设一条「柳如烟 → narrator_female_cn」,接线前选出 `young_female_cn`(青年女声)、接线后选出 `narrator_female_cn`(成熟女声),未被覆盖的李长安不受影响,脏值 `female-zh` 不放行。验证用的那条覆盖已从库里清掉 —— 那是我为测试造的,不是 owner 设的。测试 8 条,其中一条横向锁住「recompose 与 shot-audio 读的是同一份覆盖表」。 |
| **v12.386.0** | 2026-08-30 | `b4e6f2d` | **🗑 整季导出的「兜底 URL」,在响应发出前就被自己的 finally 删掉了**。`POST /api/series/:id/export` 把各集视频 concat 到 `mkdtempSync` 出来的临时目录,再 persistAsset 落到永久存储。原代码写着「失败兜底用 tmp serve-file URL」—— 而那个兜底**自相矛盾**:`try { … return Response.json({ videoUrl: persisted?.url \|\| tmpUrl }) } finally { fs.rmSync(tmpDir) }`,finally 在 Response 交给 Next 发出**之前**就跑完了。客户端拿到的链接指向一个**已经被删掉**的文件,点下载即 404;这条死链还会被 upsertAsset 写进库,重进页面依然打不开 —— 而 ffmpeg 辛苦拼好的整季文件此时已经没了,只能整季重导。**关键是要和别处区分开**:全仓还有九处 `persisted?.url \|\| X`,但那些 X 都是**引擎 CDN 外链** —— 会过期,几天内能用,那是**降级**;兜底到自己马上要删的临时文件,是**兜了个空**。所以只改这一处:落盘失败直接 502(`season_persist_failed`,文案点明「临时文件即将被清理」),**并且不写库** —— 写一条死链进去,以后每次打开都是坏的。测试里专门留了一条断言锁住这个边界(`series/cover` 兜底到远程 imageUrl 是合理的,不在本条禁止之列),免得后人读了本版就把所有 `\|\|` 兜底一律删掉。另:v12.347 的「persistent_url 不得是外链」门禁又拦了我一次 —— 我写的 `persistentUrl: videoUrl` 虽然上一行就是 `const videoUrl = persisted.url`,但门禁只看形态、不看上下文,已改成直接写 `persisted.url`。库里目前 0 条整季资产(这个功能还没被真正用过),所以没有存量死链要清。测试 7 条。 |
| **v12.385.0** | 2026-08-30 | `8e75aba` | **🔁 最高频的重生入口,一直是那条没人修的旁路**。有两个 regenerate-shot 端点:`app/api/projects/[id]/regenerate-shot` 在 v12.343 加了落盘、v12.344 加了降级标记;而**顶层 `app/api/regenerate-shot` 两样都没跟上** —— 偏偏它才是 video-node / editor-node / review-node / pull-sheet-table 点「重生此镜」的唯一调用点。**① 不落盘**:三个落库点(单镜 / 批量补渲 / 阶段重做)全都只写 `mediaUrls: [引擎 CDN 外链]`,`persistent_url` 为 null,而那种链接三五天就 403。owner 花真金白银重生一镜、看到「完成」,几天后视频消失;更糟的是下一次 recompose 的 `fullUrl()` 取的正是 `media_urls[0]` —— 自动重合成静默地拿到一条死链。**② 单镜 SSE 不带 `isAnimatic`**:批量路径(第 199 行)一直带着,单镜路径(第 125 行)漏了。引擎全挂、回落 Ken Burns 占位片时,界面照样显示「镜头 N 重新生成完成!」加绿勾;而且前端 `video-node` 的 `updateAsset` 写的是一个**全新 data 对象**,会把资产上原有的 `isAnimatic:true` 一并抹掉 —— 只补路由是半修,本会话的「降级镜头」计数也会跟着归零,直到刷新页面才从 DB 读回来。`app/projects/[id]/page.tsx` 更是**无条件**打「补渲完成 ✅」。三处全部修好:落盘抽成同文件内共用的 `persistRegenerated()`(不在一个文件里抄三遍,失败不阻塞但留 warn)、单镜事件补标记、两个前端各自透传并区分文案。**守这条的测试当时是假绿**:v12-150 写的是 `toContain('isAnimatic: !!result.isAnimatic')`,而该串在文件里出现**两次** —— 命中批量路径就通过,单镜路径从没被验过。一条命中即绿的 `toContain`,保护的是它碰巧撞上的那一处,不是它想保护的那件事。已把它和本版新测试都改成**遍历式**:每一个 `updateAssetBySelector` 都要写 persistentUrl、每一个 `regenerateComplete` 都要带 isAnimatic —— 新增第三个分支自动纳入,不再点名某一处。反向验证过:撤掉单镜的 isAnimatic 立刻变红(旧断言在同样改动下是绿的)。测试 7 条。 |
| **v12.384.0** | 2026-08-30 | `027adea` | **🔇 解说音轨落盘全失败,接口报「已生成」,而且把上一次的成果删了**。`/api/projects/:id/narration` POST 的流程是「TTS 合成 → 每段 persistAsset 落盘 → `deleteAssetsByType('narration')` → `createAsset`」,两个问题叠在一起。**① 落盘失败是静默的**:`persistAsset` 返回 null 时 audioUrl 留 null、persistedAudio 不增,循环继续;而响应里的 `rendered` 取自 `rendered.rendered` —— 那是 **TTS** 的成功标志,与落盘无关。于是「TTS 全成功、落盘全失败」返回 `ok:true` + `rendered:true`,库里躺着一条 mediaUrls 为空的静音轨。owner 看到「解说已生成」,合成成片,交片时才发现没声音;而 MiniMax 的音频外链几小时就过期,想补只能再花一次额度。这与 v12.374 的配音、v12.379 的 BGM 是同一个病:**有 URL ≠ 有文件**。**② 更要命的是顺序**:原来是**先 delete 再 create**,注释还写着「失败可重跑」—— 可失败时旧的那条已经被删了,重跑一次失败就把上一次成功的成果一起毁掉。「可重跑」的前提是失败不破坏现状,所以两道闸必须挡在 delete 前面:落盘全失败 → 502 + `audio_persist_failed` + 明说「已保留上一次的解说音轨、未做任何改动」;部分失败仍落库(有总比没有强)但 `console.warn` 说清哪几段会是静音。响应另补两个自解释字段 `audioSegments` / `partialAudio` —— 原来的 `rendered` 是 TTS 成功标志、`segments` 是**文本**段总数,两个都不回答「有几段真的有声音」,「10 段文本、TTS 全成、只落盘 3 段」看起来和全成功一模一样。**测试是行为测试而非源码断言**:mock 掉 persistAsset / asset-repo,直接断言「落盘全失败时 `deleteAssetsByType` 根本没被调用」。第一版测试自己就假绿了 —— 请求体缺 `text`,路由 400 早退,那条否定式断言「通过」了却什么都没验;已加自证(必须先 `expect(res.status).toBe(502)` 且 `persistAsset` 被调用过)。测试 6 条。 |
| **v12.383.0** | 2026-08-30 | `8ca79e4` | **📋 上一版立了唯一入口,却只手工接了三个消费方 —— 实际有十一个**。v12.381 修的是「一个项目可以有多条 script 资产(主稿 + `script-<lang>` 多语稿 + `script-original` 备份),而 `ORDER BY shot_number` 在 shot_number 全为 NULL 时等于插入次序」。我当时**用手数了三个消费方**。全仓扫描先挖出三个漏网的:**`pull-sheet/import`** —— 读写**都**落在 `[0]`,CSV 回灌会把 owner 在 Excel 里改好的分镜 merge 进俄语稿、中文主稿一字未改,而 `projects.script_data` 同步成中俄混杂内容,接口还是 200;**`pull-sheet/replicate`** —— 用俄语台词和角色名起一个**全新项目**(伤害最大,不是一处小错而是一整个项目);`pull-sheet/save-template`(实际影响小于初判 —— 模板的 perShot 只存 shotNumber/shotSize/cameraMovement/durationSec,不含台词与角色名,受影响的只有模板标题,这一点由对抗验证纠正)。**接着把这条不变量登记进 `consumer-gate` 的契约表,门禁又扫出八个**:`export-aaf`、`export-edl`、`lipsync`、`publish-readiness`、`render-loop`、`save-template`(与 pull-sheet 下那个同名但不同路由)、`shot-audio`、`lib/film-health-io`。**手工枚举漏了 8/11。** 这正是 `contracts.ts` 开篇那张表在讲的病 —— 只不过这次犯的人是我,而且是在刚立完唯一入口的下一版;所以本版的重点不是「再补几个消费方」,而是**把「不得直接取 script 资产第一条」变成入库门禁**,契约 `allow: []`(这条不该有例外)。写测试时**又一次**锚到了 import 行:`updateAssetBySelector` 的第一处是 import;而 anchor-gate 追不到这里 —— 变量经过 `codeOf(read(...))` 两层转换,它的「变量→源文件」映射断在中间,已在测试里注明这个覆盖缺口。测试 6 条。 |
| **v12.382.0** | 2026-08-30 | `53f7a72` | **💸 五个会花 owner 钱的端点,裸 curl 就能调**。一次多智能体全仓安全扫描找出来的,四条都过了独立对抗验证:`/api/voice-sample`(无鉴权,且 `body.text` **毫无长度上限** —— 传 5 万字进去照样合成,按字符计费全打在 owner 的 MiniMax 账上)、`/api/season/narrate`(无鉴权,且 `episodes` **无数量上限** —— 一次请求就能触发几十集 TTS,外层集级并发最高到 8)、`/api/polish-script`(basic 模式**绕过** checkPlan —— 它只在 `mode==='pro'` 时才调,而且注释明说「未登录用户当 free」:不 401、只降级)、`/api/master-prompt/refine`、`/api/short-video/plan`。而同类端点(narration/synthesize、cameo/preview、character-traits/from-face)早就有 `guardPaidEndpoint` —— 又是「主路径修了旁路没修」。**危害不只是钱**:没有登录态就写不下 cost-log,事后连「谁花的、花在哪」都查不到,只剩「余额怎么没了」。五处全部接上 `guardPaidEndpoint`(鉴权 + 预算),整季按**集数**估预算而不是固定值,并补上 `SAMPLE_TEXT_MAX=200` 与 `MAX_EPISODES_PER_CALL=24` 两个上限。**实测**:无 token 五个端点全部 401 且都在调外部 API 之前返回;带 token 时功能正常(空 episodes → 400),40 集 → 413 被上限挡下。**并把这条固化成门禁** `scripts/paid-endpoint-gate.mjs`(挂进 preflight,8/8):路由若出现付费特征就必须出现守卫调用,走基线只拦新增,豁免须写理由。**门禁自己先栽了一跤**:第一版把守卫的调用删掉、`import` 留着,门禁照样放行 —— **匹配命中了 import 行**,和 indexOf 那个坑同源,只不过这次是我刚写的门禁自己踩的;已改为剥掉 import 行再判。同时加了一条**自检**:识别出 0 个付费路由就直接失败 —— 一个什么都识别不出的门禁比没有门禁更危险,它给人「已经守住了」的错觉。现在它识别出 **22 个付费路由、全部有守卫**,拿掉任何一个守卫立刻报警。测试 15 条。 |
| **v12.381.0** | 2026-08-30 | `f91812f` | **🌍「一键多语」翻译好了,成片却永远用主稿台词**。localize 把翻译稿存成 `script-<lang>` 资产、备份存成 `script-original`,于是一个项目的 script 资产可以有好几条 —— owner 那个汽车广告项目实测就有三条:「剧本」、`script-ru`、`script-en`。但**三个消费方各选各的**:localize 过滤掉 `script-*` 再取第一条(唯一正确的),而 **pull-sheet 和 recompose 都直接取 `[0]`、不过滤**。而 `listAssetsByType` 是 `ORDER BY shot_number`,script 的 shot_number 全是 NULL,顺序实际由插入次序决定 —— 现在碰巧主稿在前,但那不是任何保证:先出过俄语版的项目,pull-sheet 就会拿俄语稿去导一份中文项目的场记表。**recompose 那条最要命**:v12.187 给它加了 `body.language` 让 TTS 按语种发音,**台词来源却没跟着切** —— 传 `language:'en'` 的结果是用英语嗓念主稿台词、烧上去的字幕也还是主稿,多语版做到成片这一步就断了。新增 `lib/script-asset.ts` 作唯一入口,三处全部改走它;判据认「`script-` + 两位语种码」而不是笼统的 `script-` 前缀(`script-original` 是备份、将来的 `script-drafts` 也不该被当成语种稿);请求的语种没有对应稿时回退主稿并**如实回报 `scriptFellBackFrom`** —— 静默回退正是本系列一直在消灭的东西。**实测**:同一组真实数据下 `en`→「828 kilometers…」、`ru`→「828 километров…」,而旧行为无论 language 传什么都只拿「剧本」。**过程中我误判了一次**:发现主稿里存的是**日语**台词,一度怀疑 localize 的 apply 破坏了原稿;查 `created`/`updated` 都是 09:53、从未被改过才确认 —— 那个项目本来就是日语创作的,没有数据丢失。但这暴露了我新代码里的一个假设错误:第一版把 `usedLanguage` 对主稿写死成 `'zh'`,而**主稿是什么语种其实未知**,已改为 null 并写进测试。测试 12 条。 |
| **v12.380.0** | 2026-08-29 | `9b76c63` | **🔁 我建的门禁只防了一半,当天就撞上了它没防的那一半**。v12.378 的锚点门禁只查「锚点出现 >1 次」(有歧义),而 v12.379 撞上的是另一半:v12.374 的断言锚在 `const musicRaw` 上,那版把它改名成 `musicCandidates`/`musicPick`,锚点在源码里变成**出现 0 次** —— `indexOf` 返回 -1,`slice(-1, X)` 切出的东西必然不是想验的那段,断言以完全错误的理由变红(BGM 的行为其实更强了)。**0 次比多次更狠**:多次至少还切到了某段真代码,0 次连位置都是假的。补的时候又挖出门禁自己的精度问题:变量映射是**文件级**的,而 `const src = fs.readFileSync(...)` 常常是函数内的局部变量,同一个名字在一个测试文件里可以指向好几个源文件,后写的覆盖先写的 —— 按原实现扫会报出 7 处「0 次锚点」,**全是误报**(其中还包括我前一版刚写的 v12-377)。解析真作用域太重,取保守解:**同名变量指向过不同文件就整个放弃它** —— 门禁宁可漏报,也不能让人对着一条假线索去改没坏的测试。**另一条线:定时任务纳入版本控制**。每日重跑此前**只存在于一台机器的 `~/Library/LaunchAgents` 里**,仓库既没有生成它的脚本也没有文档 —— 换台机器就没了,想知道「几点跑、日志写哪」也无从查起。新增 `scripts/install-rerun-schedule.sh`(`--print` 只输出 plist 不碰系统,便于测试;安装时先 unload 再 load,幂等)。顺带修两件真正误导过人的事:**① 日志路径不一致** —— plist 的 `StandardOutPath` 是 `wind-comic-rerun.out.log`,而 `rerun-cron.sh` 自己 append 的是 `wind-comic-rerun.log`,查「今天跑了没」时打开 .out.log 看到 0 字节,会直接得出「从没执行过」的错误结论(我就这么误判过一次,实际它 9:00 准时跑了);现在三处统一。**② 一天只跑一次,而配额刷新并不对齐那一刻** —— 日志实测 8/28 两轮分别生成 5 个和 1 个视频,而 8/29 09:00 那轮第一镜就撞 2056;脚本本身幂等(已在盘的一律跳过),改成 **9:00 / 14:00 / 20:00** 三个时段,多几次撞上窗口的机会。`RunAtLoad` 明确置 false —— 装一次就烧一轮额度不合理。测试 9 条,含在临时目录里分别造 0 次锚点、多次锚点、同名歧义变量三种情形喂给门禁。 |
| **v12.379.0** | 2026-08-29 | `6ff6baa` | **🎼 AI 作曲塌了之后,这片子没有任何办法配上背景乐**。v12.376 查明 MiniMax Music API 对本账号已永久停用(410 / 2153),而它是本项目**唯一**的配乐来源。后端其实一直留着另一条口子 —— recompose 认 `body.bgmUrl`(customBgm),优先级还在 music 资产之上 —— 但全仓搜下来**前端零消费方**:能力做好了、入口没接,只因 AI 作曲一直能用,没人发现;现在它成了唯一通路。新增 `POST /api/projects/:id/music/upload`(登录 + 项目归属校验 + 外链过 ssrf-guard + 20MB 上限 + MIME 白名单),落成**同一个 `music` 资产** —— recompose 无需任何参数就能读到,不为「自备」另开一套语义。`MusicGenPanel` 加上传入口,并在 AI 作曲返回 `PROVIDER_DISCONTINUED` 时**改口**:锁掉作曲按钮、指名可行的那条路,而不是让人对着一堵墙反复重试。**补通道时挖出一个更值得修的**:recompose 取 `musicAssets[0]`,而 `listAssetsByType` 是 `ORDER BY shot_number` —— music 资产的 shot_number 全是 NULL,顺序实际由插入次序决定。项目 1 有两条(6 月那条 AI 作曲文件早丢了、刚上传的自备),`[0]` **稳稳取到坏的那条** —— 上传了却没声音,还查不出原因。v12.374 的守卫本来就能判可达性;既然能判,就该拿它来**选**,不只是用来**拒**:候选按时间倒序、用 `filterReachable` 过一遍、取第一条活的。**端到端实测**:上传 1.1MB 测试音频 → `musicDropped` 从 true 变 false → 成片整体响度 -22.2 → **-19.6 dB**,而且**第 2 镜(无台词)也测到 -19.1 dB** —— 那一镜没有配音,响度只可能来自 BGM。测试 6 条,其中 3 条是**真正的组件渲染测试**(testing-library + jsdom,项目里已装但此前几乎只写「读源码断言」):验的是「选中文件真的会调上传端点、body 真的是 FormData」,而不是「源码里有 fetch 这个词」。 |
| **v12.378.0** | 2026-08-29 | `2cdc006` | **🎯 「indexOf 命中第一处」我一天之内栽了三次 —— 固化成门禁**。v12.369 `findCharacterBibleByName` 命中 import(偏移 808)而不是调用点(1988);v12.375 `for (const c of clips)` 在广告净化段先出现过,窗口长度变负、切出空串,断言以**错误的理由**变红;v12.377 `send('complete')` 命中早退分支、`shouldStopForQuota` 命中 import 语句。危害是双向的,而**假绿那一面更隐蔽**:锚点落在 import 或注释上时,`not.toMatch(...)` 会在一段无关代码里**静静通过** —— 被断言保护的那段代码其实从来没被检查过。新增 `scripts/anchor-gate.mjs`:在测试文件里建立「变量 → 它 readFileSync 的源文件」映射,再看 `VAR.indexOf('literal')` 的字面量在那个源文件里出现几次,>1 即歧义。判定刻意保守 —— 解析不了的一律放过(宁可漏报,也不让门禁自己变成噪音源);按 consumer-gate 的模式走基线,只拦新增,豁免须写 `// anchor-gate: ok — <为什么>`,**不写理由的豁免不放行**。全仓扫出 **8 处歧义锚点,逐条核对**:v12-209 的 `STATUS_META`、v12-289 的两处、v12-176 的一处都命中了正确位置(脆弱但不错);**v12-340 那处是真的假绿** —— `publish-preflight` 在 distribution-panel.tsx 出现 3 次,第一处是 line 19 的**注释**,窗口切成 line 19→61,而真正的预检 fetch 在 **line 67,在窗口之外**;那条「不该把预检失败塞进主错误态」的 `not.toMatch(/setErr\(/)` 验的是一段与预检毫无关系的代码。讽刺的是那处上方就写着「断言窗口按**语义**界定……(第一版就是这么取错的)」—— 上次只修了右界,**左界仍是歧义锚点**。修法:先定位带反引号的 URL(真调用点),再 `lastIndexOf('fetch(')` 往回找本次调用的起点,并加一条**窗口自证**(`expect(block).toContain('fetch(')`)—— 这条自证当场就抓出我第一版锚点仍然切歪了。顺手把 v12-337 的 `+320` 固定字符数窗口也换成语义边界(同样是 `not.toMatch`,同样会静默通过)。歧义锚点 8 → 5。门禁挂进 preflight(7/7)。测试 7 条,含**用临时目录造一个歧义锚点让门禁去抓** —— 门禁自己也得被验证。 |
| **v12.377.0** | 2026-08-29 | `a08decd` | **⏱ 一次网络抖动,就能让当天的视频额度一个都用不上**。查每日定时重跑(launchd `ai.qfmanju.rerun`,9:00)时发现:8/29 那轮 **0.3 分钟就结束、真视频生成 0 个**,日志只有一句「引擎全部不可用,产出的是 Ken Burns 占位片」+「判定当日额度已耗尽」。**判据是「产出了占位片」→ 倒推「额度耗尽」** —— 然后停掉本项目剩余镜头,并以退出码 3 让调用方对**后续所有项目**关掉视频步骤。但占位片只说明**这一镜失败了**:欠费、接口下线、网络抖动、敏感词拦截都会走到同一个回落分支。而「为什么降级」这条信息**压根没出过 server**:编排器里每个引擎的报错只 `console.error` 打印、不收集,回落时返回的 `isAnimatic: true` 不带任何原因(v12.344 让 isAnimatic 过了边界,原因却留在了原地)。本版三层接线:编排器**收集** `engineFailures`(截断从 100 字放宽到 300 —— 判据全在 `base_resp.status_msg` 里,100 字会把它切掉)→ route 的 complete 事件带出边界 → 脚本改用 `shouldStopForQuota()` **照真实报文判**。实测(强制走已欠费的可灵)抓到三条以前一条都看不见的报文:`kling 429/1102 Account balance not enough`、`minimax 2056 已达到 Token Plan 用量上限`、`veo 401`。这也顺带回答了 8/29 那轮 —— MiniMax 确实是配额满,**停整轮碰巧是对的,只是当时靠猜、没有证据**。现在:配额/欠费才停,网络抖动与敏感词继续下一镜;**拿不到报文时保守停** —— 没有证据时少花钱比多试错安全(与 v12.374 `media-reachable` 的「只拦确认缺失」方向刻意相反:那里误杀会静悄悄少掉声音,这里误放会真金白银烧额度,代价不对称,默认值也该相反)。退出码 3 改由 `stat.quotaStop` 决定,不再由「出现过占位片」决定。判定抽成 `lib/quota-vocab.mjs` 的纯函数以便测行为而非测源码文本。测试 11 条 —— 写的时候**又踩了一次「indexOf 命中第一处」**:该文件有两处 `send('complete')`,前一处是早退分支,和我此前踩过的是同一个坑,已改为从声明处往后找。 |
| **v12.376.0** | 2026-08-29 | `6c98638` | **⛔ 界面给了一个「重试」按钮,而那个调用永远不可能成功**。想给项目 1 补一首 BGM,接口回 502「AI 作曲失败:fetch failed(**需配置 MiniMax key 且 music-2.6 有额度**)」。照这句去查 key、查额度,一辈子也查不出来 —— 上游给的真话是 **HTTP 410 / status_code 2153**:「This Music API is no longer available to new users」,这个接口对本账号**永久不可用**,key 是好的、额度也不是问题。更要命的是分类器:410 报文含的是 `no longer available`,而规则写的是 `/not available/`,**差一个词就匹配不上** → 落进 `UNKNOWN` → `retryable` 默认 **true**;而 `app/dashboard/create/page.tsx:694` 正是按 `retryable` 决定要不要给「重试」按钮。同样落进 UNKNOWN 的还有 **可灵 1102 欠费**(自 2026-08-11)和 **MiniMax 2056 当日额度满** —— 三种「重试绝不会成功」的错误,恰恰是 owner 当下天天遇到的三种。**而 v12.348 早就建好了配额词表**(`lib/quota-vocab.mjs`:ARREARS_RE / SATURATED_RE),全仓的错误分类器却从没接过 —— 又一次「造好没接线」。本版新增三档终局码:`PROVIDER_DISCONTINUED`(充值和重试都无效)/ `PROVIDER_ARREARS`(要充值)/ `QUOTA_SATURATED`(等次日刷新),全部 `retryable: false`,文案指向**真正能解决问题的动作**。判定顺序照抄 quota-vocab 的既有教训:**欠费必须排在限流之前**(可灵欠费返回 HTTP 429,按 429 判会先被限流吃掉);「已停用」比欠费更终局,排最前。**接进通用分类器就得为新场景负责**:原词表里有裸的 `balance` 和裸的 `已用完|用尽` —— 在 v12.348 的配额告警场景够用,但通用分类器什么报文都会经过,`white balance filter error` 会被判欠费、`重试次数已用完` 会让处置从「换引擎」错成「去充值」。已收紧为「balance 邻近须有不足语义」「用完/用尽前须挨着额度/余额/配额/token/credit/quota」——「次数」刻意不收,那正是最容易误伤的词。v12.348 的全部消费方 41 条断言仍绿。**排查过程中我自己也走了两次弯路**:第一次调用恰好撞上瞬时网络抖动报 `fetch failed`,我据此两度怀疑 v12.357 的全局代理,做了 A/B 才证伪(装不装代理都 HTTP 200)—— 真话一直在上游报文里,只是被那句固定后缀盖住了。测试 11 条。 |
| **v12.375.0** | 2026-08-29 | `95a3716` | **🚺 柳如烟被配成了「霸道男声」**。v12.374 的探针顺手暴露的:`resolveCharacterVoice('柳如烟')` → `domineering_male_cn`。**病根是中英词表不对称** —— v12.346 的性别投票读分镜的 `visualPrompt` + `sceneDescription`,英文侧一直认代词(`his|her|himself|herself`),**中文侧却只认名词性称谓**(男人/少女/母亲…),没有「她/他」。而 v12.346 的实测集 visualPrompt 全是英文,英文代词够用,这个缺口就没露出来。柳如烟那两个单角色镜的英文 prompt 恰好只写了名字和衣饰(`silver bracelet on wrist`、`pale cyan shirt`),中文 `sceneDescription` 里却明明白白写着「残阳勾勒出**她**侧脸轮廓」—— **0 票**,于是散列成了男声。补词表时「他」不能裸着用:**「其他」「吉他」里都含「他」**,而这两个词在剧本里很常见,用负向后顾挡掉;「他们/她们」是复数、归属不清,一并排除 —— 与「只认单角色镜」是同一条克制。**全量 A/B**:47 个有单角色镜的角色,判出率 **57% → 66%**,6 个改判逐条核对 —— 柳如烟→female、丽贵妃→female、沈云棠→female(f=4 m=1)、阿楠→male、侠客→male 均有剧本文本支持;另一个项目的李长安从 **female → 无结论**(m=5 f=4),那是把一个**错判**变成了「不判」,符合 v12.346「判不出就交回原链路」的既有约定。**另一半是我自己在 v12.374 造的债**:那版 `shot-voice` 直接调 `resolveCharacterVoice`,而全片音色本该由 `voice-cast.resolveAndPersistCast` 统一发放(它读剧本性别、把结果持久化)—— 等于在已有两份实现之外**又造了第三份**,重配一镜就可能换嗓,正是 v12.338 花力气防的事。现在 recompose 向唯一入口领表,`pickShotVoice` 只负责「谁在说」;但 cast 是**持久化数据**,取出来仍要过 `isKnownVoiceId` —— 走唯一入口不等于入口里的数据一定干净(历史记录里可能躺着 `female-zh`)。**实测**:项目 1 重生配音后落库 `{柳如烟: young_female_cn, 李长安: audiobook_male1_cn}`,成片 mean_volume -22.1 dB。测试 15 条;写测试时又栽了一次「indexOf 命中第一处」——`for (const c of clips)` 在广告净化段先出现过,窗口长度变负、切出空串、断言以错误的理由红,已改用语义唯一锚点并**显式断言窗口有效**。 |
| **v12.374.0** | 2026-08-29 | `480a1d9` | **🔇 成片是哑的,而接口报的是成功**。项目 1 的 11 个镜头视频全在盘上,合成返回 `200 / voiceover: 5`,ffprobe 也确实有音频流 —— 实测 **mean_volume = -91.0 dB,整条片子没有一点声音**。三层原因叠在一起,每层单看都「不算错」:**① 配音 URL 指向 `/tmp/claude-501/qf-audio/`,文件早被系统清了**。v12.124 修过这个,提交注释把后果都写明了 ——「旧 `os.tmpdir()/qf-audio` 会被 macOS GC → recompose 配音 404」—— 但那版只动了**写入端**,历史数据里的坏 URL 一个没管,而**消费端至今只校验 `vo.audioUrl` 字符串非空**。ffmpeg 拿到不存在的输入不会报错退出,它安静地产出一条静音轨。新增 `lib/media-reachable.ts`:三态判定(ok / missing / unknown),**只拦「确认缺失」**,远程链一律放行 —— 误放的代价是合成时才失败,误杀的代价是悄悄少了声音,后者正是要消灭的东西。BGM 走同一条防线(项目 1 的 music 资产同样只剩库里一条记录)。**② 重生配音的音色是写死的 `'female-zh'`,MiniMax 回 `2054 voice id not exist`** —— 这条路径**一次都没成功过**。`female-zh` 是主管线里无角色名时的兜底占位,editor-agent 早在 v12.288/v12.296 就改成按角色名走 VOICE_CATALOG(22 档,每档映射真实 minimax 音色),那条注释甚至明说这两个写死 id「**不在 VOICE_CATALOG 内**」;但 recompose 是另一个入口,没跟着改 —— 同一语义两份实现,主路径修好、旁路继续坏。新增 `lib/shot-voice.ts`,规矩只有一条:**返回值必须是目录里真实存在的 id,兜底那一支也不例外** —— 兜底最容易被写成一个「看起来合理」的字符串,而这正是本 bug 的成因。多角色镜**不猜**说话人(猜错就是给角色换嗓,比统一旁白更糟),判据与 v12.346「只让单角色镜投票」一致。**③ `dispatchTTSGenerate` 失败不抛错**,它返回 `{result: null, tried: [...]}`,而调用点只有 `if (result) push` —— 每次失败都被无声吞掉,连一行日志都没有,排查时唯一能看见的只有「重生了但没声音」。现在失败带着**镜号 + 音色 + 各 provider 原因**浮到响应里。**另修一条我自己刚写错的提示**:警告文案让人用 `regenerateVoiceover` 重生,而真实参数名是 `regenVoiceover` —— 指向一个不存在的参数,传了也静默无效。**实测闭环**:修复后重合成 `voiceover: 5`、耗时 15s→33.7s(TTS 真的跑了)、**成片 mean_volume 从 -91.0 dB 变成 -22.2 dB / max -1.3 dB** —— 项目 1 的成片第一次真正有声音。测试 18 条,并做了反向验证(撤回修复 → 对应断言变红)。 |
| **v12.373.0** | 2026-08-29 | `b977ec3` | **🎭 把 bible 补齐之后,「主角」和「伙伴」也一起命中了 —— 那比找不到更糟**。
...
[/\b(血腥|鲜血|流血)\b/g, '红色液体']
```
**JS 的 `\b` 是「`[A-Za-z0-9_]` 与非词字符之间的边界」,而中文字符不是词字符** —— `\b血腥\b` 在纯中文里**永远匹配不上**。实测 5 条典型 prompt:**0 条被改动**。

所以那个「重试一次」拿的是**逐字节相同的原文**:白白多打一次上游、多等一轮,结果必然还是 1026。**代码写得很完整,却一次都没生效过** —— 这比没写更糟,因为它让人以为这条路已经保上了。

修的时候刻意避开刚清理过的两个坑(v12.358~v12.366):
- **不用单字** —— 裸 `枪` 会命中「水枪 / 枪法 / 长枪短炮」;
- **长的备选排前面** —— JS 交替是 leftmost-**first** 不是 longest,`枪|手枪` 会把「手枪」切成「手 + 能量装置」。

**然后我自己的测试抓到了我自己的过度匹配**:「**大麻**烦来了」→「神秘物质烦来了」。**多字词也不保险 —— 它可以是更长词的子串**,已加否定环视 `大麻(?!烦)`。这条教训写进了代码注释。

**双向实测**:血腥/尸体/手枪/半裸/爆炸/冲锋枪/海洛因**全部正确改写**且「手枪」不被切碎;「水枪玩具」「枪法很准」「血压偏高」「大麻烦」**全不受影响**。

验证:**vitest +16 · tsc 0**。修的是净化器,四条 1026 重试路径原样保留(有断言锁)。 |
| **v12.371.0** | 2026-08-29 | `8cbd782` | **🎯 场景图每次都白试一次必然失败的引擎;失败后又给了错的建议**。
...
```ts
const realImages = fs.readdirSync('data/storage/assets').filter(f => /\.png$/.test(f))
```

**它读的是别的测试会写的目录。** 本地那里有 76 张真图,所以绿;而 `data/storage/assets` 在 git 里**一个文件都没跟踪**,CI 里的内容是**同批其它测试写进去的 mock 产物**(26 字节的假 png)—— ffmpeg 解不了 → 签名为 null → 断言失败。

`it.runIf(realImages.length > 0)` 让我以为「没图就会跳过」,**但 CI 里恰好有图,只是假的** —— 这个守卫防住的是「空目录」,防不住「有内容但不是真图」。

修:**测试自带夹具** —— 用 ffmpeg 现造两张确定性纯色图放临时目录,与环境完全无关。顺便补了一条更有意义的断言:**不同颜色 → 签名必须不同**(否则它没在测量任何东西)。

**验证方式配得上这个教训**:把本地那 76 张图临时移走再跑,14/14 仍全绿 —— 证明依赖真的断干净了,而不是「我觉得应该好了」。

**这次是我的流程问题,不是运气问题**:我每版都跑了本地全量并确认零失败批次,但**本地全绿推不出 CI 全绿** —— 这条早就写在记忆里(`ci-green-not-implied-by-local-gate`),我却连推三版才回头看 CI。以后每版推完立刻查 CI,不攒。

**顺带修出一个真 bug**:`sync-doc-stats` 写头部版本用的是 `version.replace(/\.0$/, '')` —— **只去掉结尾的 `.0`**。`12.370.0 → v12.370` ✓,但 `12.370.1 → v12.370.1` ✗,而门禁(`tests/v12-291`)要求 major.minor 短号 —— **意味着任何补丁版发布都必红**,而且红在「文档没同步」这种看起来像流程问题、实则是脚本 bug 的地方。改成一律取前两段,与门禁口径对齐。

**这里我还判断错了一次**:先以为是「断言过紧」,动手放宽了 `v12-234` 的正则;但另一条断言(`v12-291`)明确要求短号 —— **短号才是既定约定,是写入端错了**。已撤回对断言的改动,改修写入端。**遇到「测试红」先分清是断言错还是实现错,别默认改断言。**

验证:**vitest 4849 全绿(零失败批次)· tsc 0 · preflight 6/6**;空目录复跑同样全绿;版本号门禁 7/7。 |
| **v12.370.0** | 2026-08-29 | `6c27c72` | **🔧 修好我自己的测量工具 —— 它把接好线的端点误报成了孤儿**。

本轮我临时写过一版「造好没接线」扫描,靠它接线了 `reframe` / `covers-from-frames` / `hook-ideas` / 剪映草稿导出四条真功能 —— **那部分成果是实的**。

**但那一版只剥离了 `[id]`,没剥离其它动态段。** 于是 `characters/bible/[name]` 的比对键成了字面量 `[name]`,前端源码里当然搜不到 —— **被误报成孤儿,而实际上前端接得好好的**(debounce 查询 + AbortController + dismiss 状态,与端点文档描述完全一致)。

**我据此在 v12.369 的版本日志和提交信息里写了「前端零引用」—— 那句话是错的**,已在 v12.369 条目下加了更正说明。该版的实质(bible 覆盖率 2/73、回填 49 条、端点从全部未找到变成全部命中)**不受影响**,但结论必须收回。

**教训不是「扫描不可靠」,而是:一次性脚本没人复核、也没法复现。** 做成 `scripts/audit-orphan-endpoints.mjs` 并配测试,错了至少能被下一次跑出来。脚本里三条纪律:剥掉**全部**动态段;「设计上就没有前端调用方」的端点单列并**写明理由**(Stripe 回调 / 测试夹具 / cron 入口);**过短的段跳过 —— 宁可漏报也别谎报**。输出末尾明写「这是启发式结果,逐条人工确认后再动手」。

**修正后的真实结果:疑似孤儿从 8 个降到 4 个** —— `admin/plugin-stats`、`cameo-ip/grants`、`tools/remove-bg`、`tools/video-compare`。`characters/bible/[name]` 与 `cameo-ip/[tokenId]` 都是动态段 bug 造成的假阳性。

测试里专门锁了**旧实现的具体错误**:`buggy('characters/bible/[name]') === '[name]'` vs `matchKey(...) === 'bible'` —— 把错误本身写成断言,比只写「已修复」有用。

**本轮 10 版收官(v12.361 → v12.370)**:5 处**实测有害**的单字正则已修并固化门禁;情感 TTS 覆盖率 37%→60%;每日重跑分离视频/图像额度(当场解封 53 张图);画风漂移**首次产出可验证正确的真实数字**;角色 bible 从 3% 覆盖到全部命中。另**收回一条自己说错的结论**。

验证:**vitest +15 · tsc 0**。 |
| **v12.369.0** | 2026-08-29 | `c5698c2` | **📖 跨项目角色复用「设计写得很清楚,数据覆盖率 3%」**。
...
**但它从来没工作过 —— 因为数据不在**:owner 的 `global_assets` 有 **73 条**角色,**只有 2 条**带 `metadata.bible`。端点要求 `bible.imageUrl` 非空,于是查任何角色都返回 `{found:false}` —— **覆盖率 3%,等于不存在**。原因是 bible 只在「锁定角色」这条路径写,普通角色不写。

> **⚠️ 更正(v12.370)**:本条原写「前端**零引用**(接线扫描里的孤儿之一)」——**那句话是错的**。前端 `character-lock-section` 其实接得好好的(debounce 查询 + AbortController + dismiss 状态)。误判来自我那版临时扫描脚本**只剥离了 `[id]`、没剥离 `[name]`**,比对键成了字面量 `[name]`。数据覆盖率 3% 与回填的实质**不受影响**,但「零引用」这个结论必须收回。详见 v12.370。
...
```
而本项目的分镜图是 `/api/serve-file?key=…`(相对路径)——**即使配了嵌入模型,也会 0 张合格**,直接报「可探测分镜图不足 2 张」。看起来像「素材不够」,其实是**过滤条件写错了**。两个 bug 叠在一起,任何一个单独修都不会有数字出来。

**修法**:① 新增 `lib/image-signature` —— 用**已在依赖里的 ffmpeg**(不引新依赖)把图缩到 8×8 取 RGB 得 192 维签名,喂给既有的 `detectDriftOutliers`(那函数只要 `number[]`,**数学是通用的**,不在乎向量来自神经网络还是像素)。**零 API、零额度**,正好切合 owner 当前的额度处境。② 过滤条件放开到「有 url 即可」,远端嵌入才限 http(s)——那是它的真实约束。

**能力边界随结果一起交付,不是写在注释里**:返回体带 `method` + `methodNote`,面板渲染成「测量方式:本地色彩/构图签名 —— 能发现调色与构图跑偏,**发现不了同色调下的人物走形**」。不说清楚,用户会把「没报警」当成「人物一致性没问题」。配了 `IMAGE_EMBED_MODEL` 仍优先走语义嵌入。

**实测(《月挂不下来》)—— 首次产出真实数字**:比对 11/12 镜,平均漂移 0.427,指认**第 3 镜偏离最大(0.65)**。**人工核对该镜确实是近乎全黑的门缝逆光剪影**,与其余均匀照明的院落场景差异极大 —— **指认正确**。

验证:**vitest +13 · tsc 0**。 |
| **v12.367.0** | 2026-08-29 | `527f766` | **⏭ 视频额度尽了就整轮停 —— 白白放掉当天的图像额度(实测卡住 53 张)**。

前两轮审计(「给结论套兜底默认值」「静默 catch」)**都没挖到真问题**:7 处默认值里 6 处是配置项或保守取 0;177 处 catch 多数带合理说明,`polish-json` 那 8 处空 catch 是**四级降级解析器的既定机制**。**没找到就是没找到,不硬造版本** —— 转去看 owner 当下实际在用的东西。

**每日重跑一遇到占位片就 `exit 0` 退出整轮。但两套额度是分开的** —— 实测:视频耗尽当天,角色/场景/分镜**仍在正常出图**。代价已量化:

```
项目 1 完成 · 项目 2 卡在视频阶段 → 项目 3/4/5 一张图都没有
合计 53 张图像资产被卡住,当天的出图配额就这么放掉了
```

更要紧的是**分镜图正是明天视频的 i2v 首帧** —— 先备好,明天的视频额度才花得到刀刃上;否则明天还得先出图再出视频,**两天只推进一天的量**。

修:视频额度耗尽 → **只跳过视频步骤**,后续项目改跑 `--only=chars,scenes,boards`,并在标题与收尾都标明「仅图像」。退出码 3 的语义从「停整轮」改成「关视频、继续跑」。

**实跑验证**(就在视频额度已尽的今天):项目 2 判定耗尽 → 置标志 → **项目 3 切成「仅图像」并正常出图**(4 个角色 + 场景陆续完成)—— 这些图在改动前是拿不到的。

**顺带修一个我自己埋的坑**:`rerun-daily.sh` 是我让 owner 手动跑的那条命令,但**只有 `rerun-cron.sh` 有服务探活,它没有**。今天实测时 dev server 恰好挂了,结果每个项目喷一大段 `ECONNREFUSED` 调用栈,人得读到最后才知道原因。已加探活,并给出**两条可执行的下一步**(自己 `npm run dev`,或改用带自动启动的 `rerun-cron.sh`)。

**推翻了 v12.344 自己立的一条决定,并把理由写进那组测试**:那一版锁的是「出现占位片 → 停整轮」,当时的理由是「换个项目只会再撞同一堵墙」——**这句话只对视频成立**,图像步骤撞不到那堵墙。断言已随设计修订更新,保留的核心不变:**占位片必须触发停止,不能继续空烧视频额度**。

验证:**vitest +10 · tsc 0**。 |
| **v12.366.0** | 2026-08-29 | `baf1056` | **🚧 「单字正则」固化成门禁 —— 并把「不修」的理由逐条记账**。

本轮从 v12.358 起,这一个模式造成的**实测损失**:

| 版本 | 误命中 | 后果 |
|---|---|---|
| v12.358 | `ai` ∈ `hair` | **61 个角色 21 个被写成赛博朋克**,含年代剧女主 |
| v12.361 | `性` ∈ 「性格」 | **「性格温柔的小学老师」被判 CSAM** |
| v12.362 | `清`/`明` ∈「清新明亮」 | **电商广告锁成古装**,写进 prompt 还标「用户已指定」 |
| v12.364 | `老` ∈「老陈/老师」 | **中年角色配老年音色** |
| v12.365 | `冷` ∈「冷静/冷笑」 | **80% 镜头误报色调冲突** |

**为什么是「记账」而不是「全部修掉」。** 清剩余站点时,我先拿构造样例吓自己(「灯火」→愤怒、「音乐」→喜悦、「特别」→否定),**但逐个用真实输入实测后发现它们不误判** —— 这些函数收的是**短结构化字段**(`shot.emotion`、角色名)或**本就是单字动词**(劈/砸/踹),不是自由文本。

其中 `edit-intent` 我一度断定「分**别**改成特写」会被误当否定,**实测基准句「第3镜改成特写」返回同样结果 —— 假设是错的**,那是解析器动词集的边界,与单字无关。

「构造样例能命中」≠「实际会出错」。这一课在 v12.363 已经付过学费:那次我准备修的误判在真实数据里根本不会发生,真问题是完全不同的**漏判**。

所以:**已证实有害的五处已修;其余 13 个文件逐条复核并写明理由**(输入是短字段 / 本就是单字动词 / 需双词同时命中 / 误判后果仅为不追加默认值…),**新增的一律拦下**。门禁另锁三件事:清单里不得有已消失的条目(清单要跟着代码走)、每条必须写理由(不许空占位)、本轮已修的五处不得回退成单字。

**反向验证过**:故意在 `lib/video-aspect.ts` 加一条 `/古|清|明|朝/` → 门禁精确抓出并点名文件;还原后复绿。

验证:**vitest +4(门禁)· tsc 0**。 |
| **v12.365.0** | 2026-08-29 | `ea239fc` | **⚠️ 80% 触发的「可能冲突」警告,等于没有警告**。

`shot-edit-merge` 的冲突提示**设计是对的** —— 只报不判、交用户确认,而且要求 note 与原描述**同时**命中。实测一句与属性无关的指令「让她转过身来」在 120 条真实分镜上 **0% 误报** —— 这个设计本身就抑制了大部分误报,不该动。

**但 note 侧用了裸字,误报可达且严重**:
```
「让他冷静下来」 → 96/120(80%)报「色调」冲突   —— 说的是神态
「他冷笑一声」   → 80% 同上
「给她加一把雨伞」→ 25% 报「天气」冲突          —— 是道具
```
**危害不在于警告本身难看,而在于 80% 触发的警告会被无视 —— 真冲突跟着一起被无视。**

**修法只收紧 note 侧**,original 侧保持宽松:冲突要求两边同时命中,note 侧收紧就够了,original 宽一点不会产生误报、收紧它反而会漏掉真冲突。色调改到调色语义(色调/色温/暖调/冷光/饱和度/黑白…),天气改到天气语义(下雨/雨天/大雾/晴天…)。时间与景别两类**实测没有误报,原样不动** —— 不为了整齐划一去改没坏的东西。

**双向实测 7/7**:「冷静」「冷笑」「雨伞」「温暖的气氛」→ 0% 误报;「调成暖色调」「改成冷光」「改成下雨的夜晚」「改成特写」→ 仍然全部报出。

验证:**vitest +20 · tsc 0**。 |
| **v12.364.0** | 2026-08-29 | `f9986d3` | **👴 `老X` 是对任意年龄成年人的通称,却被判成「老年」**。

词表里用裸字 `老` 判年龄档。实测:
```
老陈 / 老王 / 老李 / 老师 / 老板 / 老公 → 全部判成「老年」
```
owner 的出租车故事里正是「司机**老陈**」(中年司机)—— 会被配上老年音色。`婆` 同理:「老婆」是妻子,不是老人。

这条与 v12.346 是同一族病:**姓名启发式在中文里极易误命中**。那一版的解法是「从剧本读事实」;这一版能做的是**把启发式收紧到不说谎的程度** —— 必须出现明确表示年长的词(老年/年迈/苍老/老者/大爷/白发/花甲…)才判老年,**判不出就不给年龄档**。下游有 v12.296 立的「知性别不知年龄默认青年」兜底,比瞎猜一个老声/童声稳得多。

顺带收口:年龄档原本在两处各写一份**不一样**的正则(一份有 `大爷`、一份没有),现在合成 `ELDERLY_HINT` / `CHILD_HINT` 两个常量。

**收紧时漏了一种真正的老年称谓**:「老X头」(老张头/老王头)——「老X」是通称,「老X头」专指老年男性,两者必须区别对待。**是 v12.288 的既有测试拦住的**,已补 `老[一-龥]?头`。教训:收紧词表时先跑既有测试,再下结论「收紧完成」。

**双向验证**:老陈/老师/老板/老公不再判老年;老奶奶/老爷爷/大爷/白发老者/老张头**仍然判得出**,且性别一并判对。普通人名(李长安/柳如烟)不给年龄档。

验证:**vitest +27 · tsc 0**。 |
| **v12.363.0** | 2026-08-29 | `4179e69` | **🎭 情感 TTS 只对 37% 的镜头生效 —— 而且失效是静默的**。

全仓扫单字正则扫到 `lib/emotion-tag`。**但真实数据推翻了我的假设。**

我本来准备修「`火` 命中灯火、`乐` 命中音乐」这类误判。可这个函数的输入是 `shot.emotion`(短情绪词)**而不是自由文本**,那些误判在实际数据里根本不会发生 —— **拿构造样例吓自己没有意义**。

拿 owner 的 **223 个有情绪标注的真实镜头**跑一遍,真问题是**漏判**:
```
neutral 171 镜(77%),其中 141 镜(63%)是「本有情绪、却退化成 neutral」
落空的:毛骨悚然 / 寒意彻骨 / 释然 / 挣扎 / 暗涌 / 碾压式的胜利 / 决绝 …
```
模块立意写着「让 speech-2.8-hd 的情感表达真正接通」,**实际只接通了 37%**,而且**一条日志都没有**。

**两处改动**:
① 词表按真实取值补全 —— 编剧写的是「毛骨悚然」不是「恐惧」,是「释然」不是「开心」。复合情绪(「坚定与恐惧交织」)按**强度优先**匹配,不被中性词吞掉。
② **区分「判定为中性」与「判不出」**。旧实现两者都返回 `neutral`,调用方无从区分,于是 63% 的镜头静默退化、没人知道。新增 `classifyEmotion` 返回 `matched`。

**实测(同一批 223 镜)**:判不出从 **63% → 40%**;fearful 10%→16%、happy 1%→14%。

**剩下的 40% 如实承认覆盖不了**:「暗涌」「毁灭与献祭交织的崇高感」「精密计算的狂热」—— 这是文学化复合描述,关键词表做不到。既然做不到,就**让它看得见**:两个调用点(`minimax.service` / `tts.service`)在判不出时告警并带上原词。**只加字段不接线就是花架子** —— 这一步才是让「静默退化」变成「可排查」的关键。

**顺带修一条锁错东西的旧断言**:v12.211 的测试写的是 `expect(tts).toContain('emotionToMinimaxEmotion(options.emotion)')` —— **锁的是函数名字面量**。换成 `_resolveEmotion` 后行为一字未变,断言却红了。已改成验行为:允许两种实现,但**断言「不许把中文原样透传」**(`emotion: options.emotion`)—— 那才是 v12.211 真正要守的东西。本会话第 N 次遇到「锁写法不锁行为」。

验证:**vitest +30 · tsc 0**。旧接口 `emotionToMinimaxEmotion` 行为不变(零回归,有断言锁)。 |
| **v12.362.0** | 2026-08-29 | `e63408a` | **📚 同一个「是不是古装」的判断,在四个文件里各写一遍 —— 四份都用单字**。

接着 v12.361 的全仓扫描往下清。这四处是同一族:

| 文件 | 规则 |
|---|---|
| `prompt-templates:39` | `/古装\|宫\|侠\|剑\|秦\|唐\|宋\|明\|清/` |
| `style-bible:39` | `/古装\|秦\|唐\|宋\|明\|清\|朝\|宫\|侠\|武\|仙\|修\|汉服\|…/` |
| `idea-normalizer:123` | `/古装\|宫\|侠\|剑\|秦\|唐\|宋\|明\|清\|武侠/` |
| `screenwriter-enhance:166` | `/古\|侠\|将军\|皇\|帝\|仙/` |

**实测误命中,全是正当的现代广告文案**:「现代都市广告,画面**清**新**明**亮」→ 古装(**三处同时命中**);「运动品牌广告,热**血**沸腾的比赛」→ 惊悚。

`prompt-templates` 那处后果最远:结果被写成「4. 题材锁定:古装(**用户已指定**,严格遵守)」塞进剧本 prompt —— **一句用户从没说过的话**。owner 的电商广告、汽车广告就是这样被锁成古装的(v12.358 修数据时已确认)。

**修法是收口成一份共享词表 `lib/genre-vocab`,不是把四个正则各改一遍** —— 后者下次还会漂移成四份不一样的。词表两条规矩:**一律双字及以上实词**(英文加 `\b`),**判不出返回 null**、由调用方决定默认值(词表不替调用方猜)。科幻词表特别注明**没有裸 `ai`** —— 它是 `hair`/`waist`/`fair` 的子串,正是 v12.358 那个 bug 的元凶。

**端到端双向验证**(不只测词表,测到真实产物):
...
测试另锁一条:四个消费方的**代码行**里不得再出现旧的单字正则(排除注释,免得被自己引用旧代码的说明绊倒 —— 这个坑本会话已踩过三次)。

验证:**vitest +27 · tsc 0**。 |
| **v12.361.0** | 2026-08-29 | `b311165` | **🛡 内容安全闸门把「性格温柔的小学老师」判成了 CSAM —— 顺带补上英文侧的既有漏洞**。

v12.358 那个「单字正则匹配自由文本」的病造成了 21/61 角色数据损坏,我只修了两处。这一版**先做全仓扫描**:`lib`/`app`/`services`/`components`/`scripts` 下**41 处**正则含 ≥2 个单字备选项,分布在 16 个文件。按影响面排,`lib/prompt-guardrails` 排第一 —— **安全代码误判代价最高**。

**实测误伤(改动前)**:
```
「小学老师李梅,性格温柔,总是耐心听学生说话」→ 拦截,理由「未成年人性化」
「初中生小杰,个性内向,喜欢一个人画画」      → 拦截,理由「未成年人性化」
```
正常角色设定被判成 CSAM,**而且理由把用户描述成那样** —— 比单纯拦截更糟。根因是规则里用了裸字 `性`(性格/个性/理性/男性/女性)、`肉`(肌肉/红烧肉)、`床`(起床/床头)。

**修法的取向先写清楚:安全代码宁可误伤、不可漏判。** 所以不是「删掉敏感字」(那会削弱拦截),而是**保留敏感字 + 排除已知无害复合词**(否定环视),排除项逐条确认过在任何组合下都构不成性相关表述,因此**不产生绕过路径**。性相关表述收口成 `SEX_TERM` 一份,CSAM 与真人色情两条规则共用。

**写测试时抓到一个既有漏洞**:`underage nude photo` **直接放行**。核对改动前的版本确认 —— **英文侧原本几乎是空的**,整条规则只有 `H\b` 和 `R-18`,没有 nude/naked/porn/hentai/sexual 任何一个。这不是本次改动引入的,是一直就在。补上英文词只会**加强**:规则已要求与 underage/child/loli/minor 邻近,不存在因此误伤正常内容的路径。

**双向验证(这组测试的重点不是「不再误伤」,而是「不再误伤且一条都没放过」)**:误伤 **0/10**(含英文正当用例)、漏判 **0/9**(含 `underage nude photo` / `child porn` / `minor sexual content`)。另有断言锁住「排除项本身不构成绕过」——「小学生性格,以及性行为描写」仍然拦得住。

验证:**vitest +24 · tsc 0**。 |
| **v12.360.0** | 2026-08-29 | `d4bcade` | **🤐 MiniMax 的「假成功」:HTTP 200 + status_code 0,却什么都没生成**。

重生 28 个角色图的收尾撞到:墨七连失两次,报的是

```
Minimax: no task_id in response: {"id":"…","data":{},
  "metadata":{"failed_count":"1","success_count":"0"},
  "base_resp":{"status_code":0,"status_msg":"success"}}
```

接口自称 **success**、`status_code` 是 **0**,却 `failed_count: 1 / success_count: 0` 且不给 task_id。原来一律报「no task_id」加一整坨 JSON —— **看的人无从判断该改 prompt 还是重试**。现在识别这个形态并给出**下一步动作**;两条通路(视频/图像)同一模式,都覆盖;不是这个形态的仍按原样报,不把所有 no-task_id 都归成过滤。

**诚实记一笔:我对这次失败的归因是错的。** 看到该角色描述里有「无实体服饰,身体由数据流光纹缠绕」,我判断「几乎肯定触发了内容过滤」,还这么跟 owner 说了。但**第三次重试一字未改就成功了** —— 说明它是非确定性的。改进的报错仍有价值(真触发过滤时能说清怎么办),**但不是它修好了这次**。教训:非确定性失败别硬扣因果,连失两次也不等于确定性复现。

**本轮素材修正收官(v12.358 → v12.360)**:年代判定修根因、负向词对齐、**28 个角色图全部按新 prompt 重生完毕**(26 次一遍过、1 次跳过、墨七第三次成功),全库**年代/负向词不自洽 = 0**。

验证:**vitest 4661 全绿(零失败批次)+5 · tsc 0**。 |
| **v12.359.0** | 2026-08-29 | `b0f246f` | **⚖️ 年代改了、负向词没改 —— 自相矛盾的 prompt,而我把它误判成了「模型的锅」**。

上一版修完年代,owner 让我重生其余角色图。**动手前先从备份库对比出「到底哪些真被改过」**(不凭日志回忆),结果这一比,比出了自己上一版的漏洞。

**v12.358 只换了年代片段,配套负向词原样留着**:
- 柳如烟 改成 `ancient Chinese hanfu era`,负向词却仍是 `--no historical --no ancient --no hanfu` → **要古风又禁古风**
- 苏砚青 改成 `modern contemporary setting`,负向词却仍是 `--no hoodie --no sneakers --no modern` → **要现代又禁现代**

实测 **28 个角色全部不匹配**。年代和负向词本来就是同一个决定的两面,分开处理必然出这种事。

**最该记的一笔:上一版我说错了话。** 柳如烟第一次重生出来穿古装配现代高跟鞋,我在版本日志和汇报里都写了「这是生成模型的残留,不是设定错了」——**那个判断是错的**。根因就是我自己留下的这条矛盾负向词:prompt 一边要 hanfu、一边 `--no hanfu`。改对后重生,**高跟鞋消失,变成完整的古风衣裙**。
教训写进代码注释:**看到「模型没听话」,先回头查自己的 prompt 有没有自相矛盾,别急着甩锅给模型。**

修:`scripts/fix-character-era.mjs` 把年代与负向词改成**成对结构**(`ERA_PAIRS`),换年代时同步换负向词、原来没有而新年代需要就补上;新增 `scripts/fix-era-negatives.mjs` 按当前年代把负向词强制对齐,**不重新判定年代**(那已在 v12.358 定好,重判只会引入新变数),三种情形分别处理:换 / 删 / 补。

`scripts/regen-fixed-characters.mjs` 重生这批图 —— **改了 prompt 不重生等于没改**。清单来自与备份库的 diff;单个失败不中断整批,但**额度类错误立即停止**,不继续空烧;支持断点续跑。

测试(+16)锁行为:**负向词不得禁掉自己要的东西**(`hanfu` 的负向里不能有 `--no hanfu`)、判不出年代时正负都为空(不能只留一个负向词)、成对替换、以及把「误判成模型残留」这条教训写进注释这件事本身也被断言锁住。

验证:**vitest +16 · tsc 0**。两次数据修改前均已备份 `data/qfmj.db`。 |
| **v12.358.0** | 2026-08-29 | `0b14e3e` | **🏯 单字匹配把 61 个角色里 21 个写成了赛博朋克 —— 同一个错误在三处代码各犯一遍**。

owner 让我「修掉柳如烟那个 prompt 并重生她的角色图」。**结果不是一个 prompt 的事。**

**病根:拿单个常用汉字 / 无词界的英文片段,去 match 自由文本。**

`lib/mckee-skill` 判年代:`/…|修|清|明|朝|…|ai|…/`
- **`ai` 命中 `hair`**(还有 `waist` / `fair` / `straight`)→ **任何提到头发的角色都变赛博朋克**
- `修` 命中「身姿**修**长」、`清` 命中「**清**澈」、`明` 命中「聪**明**」、`朝` 命中「**朝**气」→ 现代角色变古装

实测 owner 的 61 个角色:**21 个被写成 cyberpunk**,分配与题材完全无关 —— 「以未来科技为主题的汽车宣传片」拿到 hanfu,新能源汽车广告拿到 cyberpunk,**同一部年代剧里李长安是 hanfu、柳如烟是 cyberpunk**。

`lib/script-parser` 判题材:`/古|朝|宫|侠|武|…/`、`/爱|恋|…/`、`/杀|死|案|凶|…/`
- `死` 命中「**死死**盯着」(分镜里极常见)→ 任何片子都成「悬疑」
- `古` 命中「复**古**」→ 新能源汽车广告成「古装」

**第二处的后果更远**:结果被 `prompt-templates` 写成「4. 题材锁定:古装(**用户已指定**,严格遵守)」塞进剧本 prompt —— **一句用户从没说过的话,却标着「用户已指定」**,后续所有环节严格遵守它。

**三条修法**:① **权威来源优先** —— genre/style 是显式声明,description/appearance 是自由文本;原实现把它们拼成一个串,导致描述里一个偶然字符能盖过真正的题材。② **词界与多字词** —— 英文一律 `\b…\b`,中文只用双字及以上实词。③ **判不出就不加约束**,而不是默认塞 `modern contemporary setting` —— 强行给一个年代等于替用户做了他没做的决定。

**数据修复找对依据踩了两次**:先用 `script_data.synopsis` —— 那是 prompt 脚手架,含被污染的「题材锁定:古装」;换成 `projects.description` —— **它也不是描述,同样是脚手架**,同一句话又在里面。**唯一干净的是 `projects.title`**(用户自己写的创意原文)。前两版干跑都把新能源汽车广告判成了 hanfu,是逐条打印命中内容才找出来的。最终修正 **28 个**、本就正确 11、**判不出题材的 25 个一律不动**(章节名标题看不出题材,宁可留给人)。

**柳如烟已按 owner 要求重生**:prompt 里 cyberpunk 清除、hanfu 就位;新图是三视图古风丝质衣裙,与李长安的设定一致。**如实说一处瑕疵:鞋子仍是现代高跟鞋**,`--no modern` 没完全拦住 —— 这是生成模型的残留,不是设定错了。

**测试抓到我自己引入的一个真 bug**:用 python heredoc 写 TS 时 `\b` 被解释成**退格符**(U+0008),源码里成了 `/…|<BS>AI<BS>/` —— **一个永远匹配不上的正则,而 tsc 完全不报错**。是「英文 AI 带词界」那条断言把它抓出来的。已固化成**控制字符门禁**(C0 控制符白名单三处:AAF 的 NUL 终止符、剔除控制符的正则本身、Map 复合键分隔符)—— 这类字符肉眼不可见、review 也看不出。

验证:**vitest +25 · tsc 0**。数据修复前已备份 `data/qfmj.db`。 |
| **v12.357.0** | 2026-08-29 | `18773fb` | **🔌 服务端 fetch 不认系统代理 —— 一整类网关在服务端不可达,而它伪装成「导演功能坏了」**。
...
返回形态上做了一处防御:端点的 `hint` 说它可直接作 `hookVariants:[{title}]`,但当前实际返回的是字符串数组。**两种都兼容**(`typeof h === 'string' ? h : h.title`)—— 端点将来改形态不会把界面打空。空列表明说「返回为空 —— 换个角度重试或手写」,不留一个空白区让人以为还在加载。

复用面板已有的复制助手(`copy(text, key)` + `copiedKey`),不另写一套。

**实测(《月挂不下来》)**:`deepseek-chat-v3.1` 产出 5 条 —— 「嫂子深夜敲门所为何事?」「压抑的情感如何宣泄?」等,确实贴合剧本。

**同批扫出的 `director-review` 本版未接**,如实说明原因:它是 **SSE 流式**端点,交互复杂度比按钮式高一档(要处理进度、中断、分段渲染),硬塞进这一版会做成半吊子。记为后续项,**不假装一起做了**。

---

**本轮 10 版收官汇总(v12.346 → v12.355)**:全部来自**先核实再动手** —— 迭代方案里的候选逐条验证过,其中两条按原方案做会是错的(`withLLMPlugin` 缺前置注册表、`withLipSyncPlugin` 是花架子),已在对应版本里写明原因并换成真问题。另有一次**全仓接线扫描**(184 端点 → 12 个前端零引用),接掉了其中 3 个最有价值的。

验证:**vitest +10 · tsc 0**。 |
| **v12.354.0** | 2026-08-28 | `8326a9f` | **🖼 成片抽帧做封面接线 —— 端点从 v12.113 就在,前端零引用**。

接线扫描里的第二条。**为什么它值得先接:不消耗任何 T2I 额度** —— 直接从已合成的成片里抽帧 + VLM 打分排序。owner 眼下的处境正是图像/视频额度都紧张(可灵欠费、青云top 欠费、MiniMax 当日已耗尽),这时候它是**唯一还能出封面的路**。

**按钮位置是有意的:放在「生成封面候选」左边。** 额度紧张时它才是该先点的那个,藏在主按钮后面等于没接。文案直接写「从成片抽帧(不耗额度)」——**它存在的理由就该写在按钮上**。

诚实的一处细节:端点在 VLM 全挂时会**按采样序返回**并标 `scored:false`。界面据此分别显示「已按画面质量排序」或「评分引擎不可用,按时间顺序排列」—— 不让用户把一组未经排序的帧当成精选结果。

复用既有的候选/安全区渲染,不另起一套(同一语义两份实现是本仓老毛病)。两个按钮互斥禁用 —— 同时点会互相覆盖候选集。

**⚠️ happy path 在本机跑不了,如实说明**:实测返回 `没有可用的成片(final_video),先合成再精选封面` —— 因为成片文件也在 v12.342 那次误删里丢了。**接线闭环是验证过的**(请求 → 端点 → 可执行提示 → 界面原样转达),但「抽出 6 帧」这一步要等有完整成片时才能跑通。

验证:**vitest +9 · tsc 0**。 |
| **v12.353.0** | 2026-08-28 | `397ee5d` | **📐 改画幅接线 —— 端点从 v12.16 就在,前端零引用**。

**先做了一次全仓扫描**:184 个 API 端点里,**12 个前端一次都没被调用过**。剔掉设计如此的(`stripe/webhook` 是外部回调、`mock-assets` 是测试夹具),**6 个是真正「造好没接线」的用户功能**:`reframe`、`covers/from-frames`、`hook-ideas`、`director-review`、`remove-bg`、`video-compare`。

这一版先接最该接的 `reframe`:竖屏短剧要投 B站/YouTube 就得有横屏版,而它**不重新生成每一镜**、直接把已合成的成片重构图 —— 对用户是「一次成片、两个平台都能投」,增量成本几乎为零。

放在**分发面板**而不是监看台:平台画幅要求是分发时才关心的事(抖音 9:16、B站 16:9)。两个方向各一个按钮,模式可选并把代价写在选项里 —— 「模糊填边(画面完整)」vs「裁切填满(会切掉边缘)」,**让用户在点之前就知道会失去什么**。界面上明写「不重新生成每一镜」,这是决定要不要点的关键信息。

**实测时撞到并顺手修了一件事。** 拿真项目跑 9:16,端点返回:
```
重构图失败: serve-file path not found: /api/serve-file?key=af7e6a4e…
```
成片的库记录还在、磁盘文件没了(v12.342 那次误删的后遗症),但这句话是内部术语,用户看了不知道该干什么。改成 **「成片的本地文件已丢失(库里有记录、磁盘上没有),需要重新合成后再改画幅」** + `code: 'source_missing'`,识别条件覆盖 `path not found` / `ENOENT` / `no such file` 三种原文;非此类错误仍原样透出,不被一刀切吞掉。**报错的价值在于告诉人下一步做什么。**

另:测试里 `toMatch(/模糊填边(画面完整)/)` 恒不成立 —— 括号在正则里是捕获组,实际匹配的是「模糊填边画面完整」。字面量断言改用 `toContain`,并把原因写在注释里。

验证:**vitest +11 · tsc 0**。实测:同比例请求正确拒绝(400「项目主比例已是 16:9」),反方向请求走到真实重构图路径。 |
| **v12.352.0** | 2026-08-28 | `5ebeab9` | **📊 口型这一路一条遥测都没有 —— 而迭代方案里那条如果照做,是花架子**。

迭代方案写的是「G5 `withLipSyncPlugin`」。**照着做解决不了问题**,这一版把原因写清楚:

`withXxxPlugin(input, fallback)` 的语义是「插件链当 primary、orchestrator 老路径当 fallback」。但 **lipsync 本来就是走注册表调度的**(`dispatchLipSyncGenerate` 从 v12.213 就在),**没有另一条老路径** —— plugin 与 fallback 会是同一个函数,模式(off/shadow/primary)因此毫无意义。更要命的是 `getPluginChainMode()` 默认返回 `'off'`,而 `runWithPlugin` 在 off 时直接 `return fallback()`、**连遥测都不落**。**套上 wrapper,admin 面板还是空的。**

**真正缺的是在真实调用点落账**,与 plugin mode 无关。admin 的 plugin-stats 面板上 image/video/tts 三条曲线都在,lipsync 是空的 —— 运维看不到它的成功率与耗时,也就无从判断自托管的 wav2lip/SadTalker 到底跑得怎么样。

修:`lipsync/render` 的 `dispatchLipSyncGenerate` 调用点前后记时,**成败都落**(只记成功的话成功率恒 100%),失败时把链上各 provider 的原因一并带上;`void` 调用,遥测永远不能拖垮渲染。面板侧不用改 —— 它本来就是按 kind 动态分组的,落了就出现。

`withLipSyncPlugin` **仍然加了**(补齐四路对称,给将来真有自定义 provider 时用),`PluginEventKind` 补 `'lipsync'`,文件头「三个 wrapper」改成「四个」—— 注释也是要维护的。但明确记一笔:**wrapper 不是这一版的实质**,并把「为什么不套它」写进代码注释,免得后人再照方案加一遍。

验证:**vitest +10 · tsc 0**。 |
| **v12.351.0** | 2026-08-28 | `8d9e14e` | **🥁 MV 的 BPM 自动检测 —— 检测函数从 v12.246 就在,页面却一直让用户自己数拍子**。

`lib/beat-detect.detectBeats`(ffmpeg silencedetect)早就能拿到拍点序列,而 `app/dashboard/mv` 里是 `useState(120)` + 一个手填数字框。**造好了没接线**,而且这次的"没接线"很具体:缺的是「拍点序列 → BPM」这一段,没有它,检测结果没法变成页面上那个数字。

**补的那段纯函数里有两个坑,不是「取平均」能绕过的**:
- **必须取中位数。** silencedetect 会漏拍(弱拍被噪声底吞掉),漏一拍就产生一个双倍长的间隔;平均值被这类离群值拉偏,中位数不受影响。
- **半速/倍速歧义。** 每小节只在重拍触发时,测出来是真实 BPM 的一半;鼓点密时又可能是两倍。所以把结果反复 ×2 / ÷2 折算进常用区间(70–170)—— 这是音乐软件的通用做法,不是拍脑袋定的。

另外给出**齐整度**(间隔落在中位数 ±15% 内的占比):齐整=检测可信,散=多半漏拍。低于 0.6 时端点显式提示「仅供参考,建议试听后手动校正」——**给个数字了事是不负责任的**。

**正确性用已知真值验过,不是自说自话**:ffmpeg 造一条每 0.5s 一拍(=120 BPM)的 12s 节拍轨 → 检测到 **24 个拍点** → 推算 **120 BPM**(真值 120)→ 齐整度 **100%**。

安全上有一处要紧的:端点必须**只接受本仓存储内的文件**。否则给它 `/etc/passwd`,ffmpeg 的报错方式能反推出文件在不在 —— 这个端点就成了任意文件探测器。走 `resolveByKey` + `isServeFilePathAllowed` 双闸。

界面上**手填框保留** —— 自动检测是补充不是替代;测不出就返回 409 + 明确原因并提示手动填写,**不编一个数出来**。

验证:**vitest +17 · tsc 0**。 |
| **v12.350.0** | 2026-08-28 | `7037b8c` | **📉 画风漂移检测接线 —— 端点从 v12.2.4 就在,前端一次都没调过**。

**放在「技术监看台」而不是评分面板,是想清楚了才放的**:漂移是**客观可量化**的(逐镜视觉 embedding 的余弦距离),和示波器同类;而既有的一致性评分是 LLM 对画面的**主观描述**。两者混在一处,用户分不清哪个是测量、哪个是判断。

它抓的是**渐进漂移**:每一镜单看都还行,但第 1 镜和第 11 镜已经不像同一部片 —— 这正是逐镜 LLM 评分抓不到的那类问题。

**按需触发,不自动加载** —— 它要对每张分镜图做一次 embedding,开销和费用都不小,挂在页面加载上是在替用户花钱。

界面上把「有偏离」和「没偏离」都说清楚:偏离最大的镜在柱状图上标成琥珀色并列出镜号,没有偏离时明写「未发现明显偏离」而不是留空白让人猜。另附一句「漂移是相对值,只在同一部片内可比」—— 防止用户拿两部片的数字互相比。

**⚠️ 这一版交付的是「接线 + 正确的降级显示」,不是「能看到漂移数字」。** 本机未配 `IMAGE_EMBED_MODEL`,端点如实返回 `{available:false, reason:'未配置图像嵌入…,漂移检测退回逐镜 LLM 一致性评分'}`,面板照实显示「暂不可用 —— <原因>」。**实测走的就是这条降级分支**。配上该模型后才有数值 —— 说明白比装作能用重要。

验证:**vitest +12 · tsc 0**。 |
| **v12.349.0** | 2026-08-28 | `47d460f` | **🎬 剪映草稿导出接线 —— 端点造好两个多月,前端一次都没调过**。

迭代方案里这条被标成「NOT small」,标得对,但难的地方不是我以为的那个。

**不接线不是因为忘了。** POST 版要调用方自己拼 `clips[{path, durationSec}]`,而**前端手里只有 `/api/serve-file?key=…`** —— 那是 HTTP URL,剪映打不开。前端**根本没有能力**提供「剪映打得开的路径」。所以这不是「加个按钮」的活,加了也是废的。

**破局点是想清楚这个应用是本机跑的**:素材就在 `data/storage/assets/` 下,剪映和本应用在同一台机器上,所以**绝对路径直接可用** —— 而把 key 解析成绝对路径这一步,只有服务端做得了(`resolveByKey().absPath`)。

新增 `GET ?file=content|meta`:服务端从项目资产(video / shot-audio / music)直接组装草稿,两次下载 —— 与既有 EDL/AAF 的 `Content-Disposition` 模式一致,**不引入打包依赖**(仓库里没有 zip 库,为两个 json 加一个依赖不划算)。

**诚实做了三件事**:
- 解析不出本地路径的素材**跳过并记账**,不塞一个打不开的路径进去装完整;
- 一个片段都没有时返回 **409 + 明确原因**,不返回空草稿骗人;
- 把限制**随文件一起交出去**(`X-JianYing-Notes` 响应头 → 界面显示),而不是只写在代码注释里:剪映 ≤5.9 可用(6+ 草稿加密)、schema 为社区逆向、导入前请在真剪映验证。

前端按钮单独占一行,不混进上面那排 EDL/AAF —— **它产出两个文件、且有版本限制,行为与「一键下载一个文件」不同**,混在一起会让人以为也是单文件。

**实测(《月挂不下来》)**:HTTP 200,导出 **11 镜**,正确跳过 1 个缺本地文件的,草稿名取自项目标题,时长 98s;**11/11 路径都是真实存在的绝对路径**;`?file=meta` 同样正常返回 `draft_id/draft_name/draft_fold_path/tm_duration`。

验证:**vitest +13 · tsc 0**。 |
| **v12.348.0** | 2026-08-28 | `6b0cb79` | **🔌 两个「系统知道、但没说」的引擎问题 —— 可灵没钱 17 天没人看出来**。

都是实测重跑素材时撞出来的,不是翻代码翻出来的。

**① 显式指定 `happyhorse` 被静默忽略。** 我给单镜重生传 `videoProvider: 'happyhorse'`,日志里每一镜**仍然先撞可灵的余额墙**。根因:v12.272 接入 HappyHorse 后,主管线会把它 push 进 `availableEngines`,**但单镜重生那条路径的 `availForRegen` 只列了 veo/minimax/kling** —— `resolveEngineOrder` 里 `has('happyhorse')` 为假,显式选择被丢掉,回落 env 链序。而日志一直打着「HappyHorse: ON」。两条路不对称,不对称的那条没人看。修:`availForRegen` 补上(沿用主管线同一条 `happyHorseAspectSupported` 判断,不是随手加)、`genByEngine` 补实现;**并且显式引擎不可用时要出声告警** —— 静默换一个跑是这次最难排查的那部分。

**② 可灵欠费被记成「限流」,巡检一路报 OK。** 可灵欠费原文是 `Account balance not enough`(HTTP **429**),而 `api-usage-tracker` 的规则只认 `credit|余额|insufficient` —— 三个词一个不沾,落到兜底的 `rate_limited`。**而巡检脚本那份词表本来是全的**(认 `balance` / `not enough`),两边各写一套,**好的那份没被复用** —— 又是「同一语义两份实现」。

后果不是标签难看:**「限流」的处置是等一会再试,「欠费」的处置是充值**,建议完全相反。查告警表:owner 的可灵从 **2026-08-11** 起就没钱了,累计 ×18 次,而 `npm run audit:api` 一路报 `[OK] 可灵 Kling`,**17 天没人看出来**。

修法三层:
- 措辞词表收口成 `lib/quota-vocab.mjs`(**全仓唯一一份**;用 .mjs 是为了 TS 和纯 node 脚本都能 import 同一份)。收录时写明两条教训:青云top 额度耗尽返回 **401** + "quota exhausted"(既不是 402 也不含 insufficient);可灵返回 **429**,所以**欠费判定必须排在限流之前**。
- 巡检对「查任务状态」这类**只验得了鉴权**的探测,如实标成 `仅鉴权` 而不是 `OK`,并计入「需处理」。可灵只在**创建任务**时才回 1102,查任务永远看不到钱。
- **巡检读运行时告警表** —— 系统早就知道,只是从没告诉过巡检。且**不信存下来的 `alert_type`**(历史行正是被错分的),直接拿 `error_message` 判。拿不到本地库(CI / 别人的机器)就跳过,不让巡检失败。

**顺带把已经错记的历史行改正**:新增 `scripts/reclassify-quota-alerts.mjs` 按新词表重判,实测改正 **3 条** kling 行 `rate_limited → exhausted`。改前已备份 `data/qfmj.db`。

**验证是端到端的**:巡检输出从 `✅[OK] 可灵 Kling` 变成 `💸[欠费] 可灵 Kling`,汇总从「欠费 1 / OK 9」变成「欠费 2 / OK 8」。

验证:**vitest +21 · tsc 0**。 |
| **v12.347.0** | 2026-08-28 | `3089081` | **🏷 `persistent_url` 里存着会过期的外链 —— 这一列的名字就是它的承诺**。

v12.343 修完三个重生端点后,审计留了个尾巴:「22 个写资产的端点不调 `persistAsset`,其中 11 个确实在写媒体 URL」。逐个追来源后发现两族,而**第二族比第一族阴**。

**① 压根不落盘**(3 处):
- `heal-shots` —— 补渲视频。注释白纸黑字写着「持久化:更新既有 video 资产」,做的却只是写 DB 行。补渲的意义是「把坏掉的镜治好」,治完几天又 403 等于没治。
- `candidates/pick` —— 九宫格选定的那张。用户「选定」意味着这张要当视频首帧进成片,**恰恰最该落盘的一张,却是唯一没落的**。
- `regenerate-shot-4k` —— 4K 重渲。收费的高成本操作,产物反而是最不该几天后失效的那一份。

**② 假持久**(3 处,更阴):
```ts
persistentUrl: url.startsWith('http') ? url : null
```
把引擎外链直接塞进 `persistent_url`。消费方 —— 包括 **v12.342 那个「清理前查引用」的扫描** —— 看到这一列有值就认为「有本地持久副本」,而它几天后就 403。出现在 `shot-sketch`、`anytext-cover`、`create-pipeline`。

**不只是修代码,还把已经坏掉的救回来。** 库里查出 **9 条**这样的假持久(全是 storyboard-sketch,来自 `imagine-cdn.youchuan.cn`)。逐条探活:**8 条仍存活、1 条已经 403**。新增 `scripts/rescue-fake-persistent.ts`(`npx tsx`,**复用仓库自己的 `persistAsset`**,不另造一套落盘逻辑),把存活的抓回本地存储并改写 `persistent_url` 与 `media_urls`,已死的只报告不动 —— 留着让人知道丢了什么。实测抢救 8 条约 10MB;那 1 条死的正好证明这不是假想风险。

**测试设计上做了一次让步并写明原因**:`persistentUrl: <变量>` 的变量是不是外链,静态分析判不了。所以严格门禁只锁**那个具体谎言**(`startsWith('http') ? url : null`),其余走**已复核清单** —— 8 个站点逐个追过来源并把结论写进注释(`serveFilePathUrl` 本地 / `persistFirstValid` 落盘结果 / 复制既有资产),**新增站点必须复核后加进白名单,否则红**,不给静默新增留口子。

**又踩了一次 `[^}]*`**:断言 `createAsset({[^}]*persistentUrl:` 被 `data: {…}` 里的第一个 `}` 截断,恒为假。本会话第三次栽在括号类上,已改成语义窗口(`indexOf('if (changes === 0)')` → `indexOf('healed.push(')`)。

验证:**vitest +8 · tsc 0**。抢救前已备份 `data/qfmj.db`。 |
| **v12.346.0** | 2026-08-28 | `b7acaa5` | **🗣 角色性别从剧本读,不再靠猜名字(8% → 48%)**。

迭代方案里这条标着「越晚修影响越大」,原因是 v12.338 的 voice-cast 会把分配结果**持久化** —— 错一次就永久锁死。

**先量化,再动手。** 音色选路最终落到 `inferTraitsFromName`,而那份词表只认**称谓词**(叔/爷/姐/妹/夫人/公子),不认中文人名。拿 owner 真实的 50 个角色实测:**只判得出 4 个(8%)**,其余 46 个落到「全目录确定性散列」——**性别是随机的**。柳如烟一旦散列到男声,voice-cast 就把它当成片事实锁住。

**没有去扩姓名词典 —— 那是猜。** 剧本里其实写着答案:分镜的 `visualPrompt` 是给图像引擎的英文描述,必然点明人物性别(「Li Chang'an, a young Chinese man with bronze tan complexion」)。新增纯函数模块 `lib/character-gender.ts`,拿**单角色镜**投票:

- 多角色镜**不投票** —— 性别词归不到具体人身上
- 一边有票另一边为 0 → 直接定;两边都有票需 **2 倍以上优势**(镜里常有第二个未具名人物污染少量票)
- 判不出返回 undefined,交回原链路 —— 延续既有的「不瞎猜」约定
- 中文 visualPrompt 也认

**实测(owner 真实数据,50 个角色)**:姓名词表 4 个(8%)→ 剧本投票 21 个(42%)→ **合并 24 个(48%)**;两种方法**冲突 0 个**(互相印证);仍判不出的 26 个退回散列,与之前完全一致。

接线走**唯一入口**(v12.296 立的规矩):`resolveCastVoices` 新增可选 `hints` 参数,`_preferredVoice` 里**剧本证据覆盖姓名启发,但只覆盖它确实给出的字段** —— 「李大爷」剧本判 female、姓名词表判老年,两者互补而不是互相清空。`resolveAndPersistCast` 拿 projectId 读本项目剧本造线索,读不到就返回 undefined(线索是增强项,拿不到不该让配音整条链失败)。

**零回归是硬约束,并写成断言**:不传 hints 时 `resolveCastVoices` 的输出必须与改动前**逐字节一致**;已在定妆表里的角色,线索再强也不动 —— 那是成片事实。

验证:**vitest +14 · tsc 0**。 |
| **v12.345.0** | 2026-08-28 | `b09d056` | **📚 两个「库」模块对真实用户都是空的 —— 各有各的病根 + 每日重跑定时化**。
