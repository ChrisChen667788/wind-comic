...
| **v12.442.0** | 2026-09-18 | `待填` | **🔒 构建期不许几十个进程抢同一个 sqlite 文件 —— 修掉一个会周期性红掉 Docker 的竞争**。v12.441 推上去后 `build (linux/arm64)` 红了:`Failed to collect page data for /api/auth/login` ← `SqliteError: database is locked`。根因:`next build` 会起多个 worker 并行收集页面数据,**每个 worker 导入 `lib/db` 都会对同一个 `data/qfmj.db` 建表 + 跑迁移**(全是写操作);arm64 镜像跑在 QEMU 下更慢,`busy_timeout = 5000` 等不到锁就抛。**这是竞争不是必现** —— 所以「重跑一次绿了」不能当修好了(v12.440 的 Docker 就是侥幸绿的,代码路径完全一样)。**修法**:构建期给每个进程一个临时目录里的独占库(`NEXT_PHASE=phase-production-build` 或显式 `QFMJ_EPHEMERAL_DB=1`),进程退出即清理;Dockerfile 只在 builder 阶段置位,runner 阶段绝不带 —— 运行时必须用真库。构建期本来就不该读生产数据:页面全是 `force-dynamic`,库只是被 import 带起来的副作用。**证据**:本机跑一次生产构建,`data/qfmj.db` 的 mtime/大小**全程未变**,同时 `/tmp` 下出现 **32 个**构建期临时库目录 —— 那 32 路写竞争正是 arm64 上锁死的东西。**验证**:新增 4 条测试(运行时仍是 `cwd/data/qfmj.db`;三个进程三个独占库且都不在数据目录下;子进程退出后临时目录自己消失;Dockerfile builder 置位/runner 不带且在 `npm run build` 之前);6 个变异逐一注入全部抓红;**顺带被自家门禁抓到一次**:Dockerfile 那条断言原本对 runner 段只有「不包含」,窗口切错(切成空串)会静默通过 —— `fake-green` 门禁当场报出来,补了正向自证(builder 段含构建命令、runner 段含运行入口)。测试刻意不赌「共享时一定锁死」(快盘上可能侥幸不锁),而是锁住那个必要条件:共享模式 N 个进程指向同一文件,构建模式各自一个。**过程中自己踩的坑**:用 `git checkout -- lib/db.ts` 回滚变异时,把本版刚写的改动一起还原了,重写了一遍 —— 这个坑在 2026-09-08 犯过同样一次。 |
