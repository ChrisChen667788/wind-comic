...
export async function POST(request: Request) {
  ...
  if (anchorProjectId) {
    const ap = db.prepare('SELECT id, user_id, aspect, style_id, primary_character_ref, locked_characters FROM projects WHERE id = ?').get(anchorProjectId) as any;
    ...
    startEpisode = 2; // 锚点占第 1 集,新建集从第 2 集起
