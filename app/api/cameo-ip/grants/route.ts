...
import { listPendingGrantsForOwner, decideGrant } from '@/lib/cameo-ip';
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ pending: listPendingGrantsForOwner(payload.sub) });
}
...
export async function PATCH(request: Request) {
  ...
  try {
    const grant = decideGrant(grantId, payload.sub, !!body?.approve);
