...
import { loadTeam, saveTeam } from '../../lib';
...
export async function POST(request: NextRequest) {
  ...
  const r = db.prepare('SELECT * FROM team_invites WHERE token = ?').get(token) as any;
  ...
  const chk = canAcceptInvite(invite, now());
  if (!chk.ok) return NextResponse.json({ message: chk.reason }, { status: 400 });

  const user = db.prepare('SELECT id, name, email FROM users WHERE id = ?').get(payload.sub) as
    | { id: string; name?: string; email?: string } | undefined;
  const accepter = user || { id: payload.sub };

  const { pool, members } = loadTeam(invite.ownerUserId);
  ...
  const apply = db.transaction(() => {
    saveTeam(invite.ownerUserId, pool, nextMembers);
    db.prepare('UPDATE team_invites SET status = ?, accepted_by = ?, accepted_at = ? WHERE token = ?')
      .run('accepted', accepter.id, now(), token);
  });
  apply();
