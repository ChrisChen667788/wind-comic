...
export async function GET(request: NextRequest) {
  ...
  const rows = db.prepare('SELECT * FROM team_invites WHERE owner_user_id = ? ORDER BY created_at DESC').all(owner) as any[];
  const nowIso = now();
...
export async function POST(request: NextRequest) {
  ...
  const invite = buildInvite({ token, ownerUserId: owner, email, role, allocated, nowIso: now() });
  db.prepare(
    `INSERT INTO team_invites (token, owner_user_id, email, role, allocated, status, created_at, expires_at, accepted_by, accepted_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL)`,
  ).run(invite.token, invite.ownerUserId, invite.email, invite.role, invite.allocated, invite.status, invite.createdAt, invite.expiresAt);
