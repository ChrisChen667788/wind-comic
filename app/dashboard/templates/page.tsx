...
export default function TemplatesMarketPage() {
  ...
  const load = useCallback(async (query: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/templates${query ? `?q=${encodeURIComponent(query)}` : ''}`);
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      <div className="flex items-center gap-2 mb-5 max-w-md">
        ...
        <button onClick={() => load(q)} className="cinema-btn !px-3 !py-2 !text-xs">搜索</button>
      </div>
          ...
          {templates.map((t) => (
            <div key={t.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-4 flex flex-col gap-3 hover:border-white/20 transition-colors">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={`text-[11px] px-2 py-0.5 rounded-full border shrink-0 ${qColor(t.quality)}`}>质量 {t.quality}</span>
              </div>
