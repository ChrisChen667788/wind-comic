...
| **v9 · Postgres 平台 + 变现** | SQLite↔Postgres **双驱动**全量切换(17 核心表/簇迁 async repo,PG 端到端验证 + 事务提交/回滚原子性;默认仍 SQLite,`DB_DRIVER=pg` 可选)· 多平台**分发包**生成 · **真二进制 AAF 导出**(MS-CFB)+ EDL/FCP7 XML(Premiere、达芬奇可导入)· 质量与一致性深化(发布门禁 / 重生回环 / 一致性报告)+ 可灵式**多参 + 一键成片**融合。 |
...
### 10. **6095 个单测全过, TypeScript 严格模式, 没有"敬请期待"**
...
| 开源 | ❌ | ❌ | ❌ | ❌ | ⚠️ 权重部分开放 | ✅ MIT | **✅ MIT** |
| 单镜改 prompt 重生 | ⚠️ | ✅ | ✅ 区域编辑(选时间点 + 圈画面,2.5) | ✅ motion brush | ✅ 对话式迭代编辑(招牌能力) | ✅ video-edit 端点 | **✅ + reference image upload** |
| 节奏 / 冲突审计 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | **✅ v2 (v12.275): shot score + reversal detection **plus** conflict-curve shape (escalating / flat / front-loaded / no-climax), drag-segment localisation to exact shot ranges, opening-density check, and duration-rhythm analysis — every finding names the shots to fix** · optional hard gate `PACING_GATE=block` (v12.455; default only warns) |
| 可剪辑时间线导出(EDL / FCP7 XML) | ❌ | ❌ | ⚠️ 即梦可把素材送进剪映,但不是时间线 | ❌ | ❌ | ❌ | **✅ EDL (CMX 3600) + FCP7 XML (Premiere / DaVinci) + built-in multi-track timeline** |
...
| 按剧生成免版税 AI 配乐 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | **✅ ElevenLabs Music → self-hosted ACE-Step / YuE → MiniMax fallback chain (v12.410; needs one of them configured — MiniMax music-2.6 is closed to new accounts)** |
...
| **音乐 BGM** | ElevenLabs Music(`ELEVENLABS_API_KEY`) | 自托管 ACE-Step / YuE(`MUSIC_SELFHOST_URL`) | MiniMax music-2.6(新账号已停服)|
...
然后打开 <http://localhost:3100>。**不需要 clone、不需要 npm install、不需要本地构建。**

想连着 compose 一起跑(带演示工程):
...
- `MINIMAX_API_KEY` — image-01 / Hailuo-2.3 视频 / speech-2.8-hd TTS / music-2.6 BGM(新账号已停服,配乐请配 `ELEVENLABS_API_KEY` 或 `MUSIC_SELFHOST_URL`)
...
2. **测试是底线.** Vitest 6095/6095 必须保持绿. 新加 lib/service 必须配测试.
