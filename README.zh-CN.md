...
- **🎙️ 配音口型全链** —— 角色音色路由(按名自动 + 手动挑/试听)→ TTS → **viseme 关键帧轨** → 浏览器实测**嘴-声对齐分** → **漂移自动校正** → 可插拔引擎渲染(wav2lip / SadTalker / MuseTalk,自带 `LIPSYNC_API_URL`)→ 写回时间线。**一键全片口型** + Vision **质检自愈回环**(弱镜自动重渲)。
