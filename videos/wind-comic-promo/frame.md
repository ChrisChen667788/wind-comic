...
description: >
  ...
  as bloom / panel / tile underprint, Plus Jakarta Sans display + Plus Jakarta Sans sans + JetBrains Mono data,
...
> **Brand-mapping correction (manual, one key).** `build-frame` picked `#827C76` for `ink` from
> colorStats. That hex is the site's *secondary* text tone (24 occurrences); the site's real ink is
> near-black — its largest background area is `#000000` with `#F5F1EA` text over it, i.e. the product
> is dark-ground / light-type. Mapped onto this light-paper preset, `#827C76` gave 3.90:1 on `paper`
> — under the 4.5:1 body-text floor. Set to `#141210` (17.69:1 measured). Only this one key was touched.
...
typography:
  ...
  # — display ramp (Plus Jakarta Sans 400, tight, negative tracking) —
  ledger-title:{ fontFamily: "Plus Jakarta Sans", cqw: 1.55, weight: 400, lineHeight: 1.15 }
  strand-title:{ fontFamily: "Plus Jakarta Sans", cqw: 1.7, weight: 400, lineHeight: 1.1 }
  headline-sm:{ fontFamily: "Plus Jakarta Sans", cqw: 2.9, weight: 400, lineHeight: 1.0 }
  headline:{ fontFamily: "Plus Jakarta Sans", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.005em" }
  date-rail:{ fontFamily: "Plus Jakarta Sans", cqw: 5.0, weight: 400, lineHeight: 0.96, tracking: "-0.005em" }
  display-it:{ fontFamily: "Plus Jakarta Sans", cqw: 7.0, weight: 400, italic: true, lineHeight: 1.04, tracking: "-0.005em" }
  numeral-md:{ fontFamily: "Plus Jakarta Sans", cqw: 7.5, weight: 400, lineHeight: 0.92, tracking: "-0.01em" }
  display:{ fontFamily: "Plus Jakarta Sans", cqw: 14.6, weight: 400, lineHeight: 0.86, tracking: "-0.018em" }
  numeral-jumbo:{ fontFamily: "Plus Jakarta Sans", cqw: 28.0, weight: 400, lineHeight: 0.84, tracking: "-0.04em" }
...
- **Fonts** — already set to **Plus Jakarta Sans** (display) / **JetBrains Mono** (body); ignore any preset font name lingering in prose.
...
The voice is three faces in rigid roles: **Plus Jakarta Sans** (weight 400, tight line-height,
negative tracking) carries every display, numeral, and quote from 40px to 720px+; **Plus Jakarta Sans** carries
...
- **Plus Jakarta Sans 400** display (tight, negative-tracked); **Plus Jakarta Sans** body + micro-labels; **JetBrains Mono** data.
...
- **Squint** — one Plus Jakarta Sans moment dominates at 3–6× its neighbor; the sun bloom centers the eye.
...
Two ramps. The **reading/data ramp** (Plus Jakarta Sans body 0.85cqw ink, micro-labels in px, JetBrains Mono
data) carries copy + chrome; the **display ramp** (Plus Jakarta Sans `ledger-title` 1.55cqw →
...
- **Plus Jakarta Sans is weight 400 only** (its contrast is the weight signal), tight line-height (0.84–1.06), negative tracking; **micro-labels uppercase Plus Jakarta Sans 600, ≥0.16em**; **mono for all numerals/dates**. No bold serif, no second text color, no mono in body/display.
...
headline-sm. **Focal** a single huge Plus Jakarta Sans `numeral-jumbo` (≈28cqw) dominating, with a
...
**Focal** a 2-line Plus Jakarta Sans **italic** `display-it` quote in ink, centered, under an
...
yellow-panel, micro-label, headline. **Focal** an Plus Jakarta Sans `headline` in ink sitting **on
...
- Set every line in **ink**; use **Plus Jakarta Sans 400** (tight, negative-tracked) for display, **Plus Jakarta Sans** body, **JetBrains Mono** for all numerals/dates.
- Make every separator a **1px ink hairline** (soft variant for dense rows); flood a **yellow panel** for poster moments (ink on top).
- Keep micro-labels **uppercase Plus Jakarta Sans 600, 0.16–0.32em**; pin the **pagenum** bottom-right.
...
- No second text color; no bold Plus Jakarta Sans; no inverted ink grounds.
...
- **Type** — Plus Jakarta Sans 400 tight negative-tracked, fit-to-measure; micro-labels uppercase 0.16em+; mono numerals; ≥1.4cqw floor.
...
- **Plus Jakarta Sans + Plus Jakarta Sans + JetBrains Mono via Google Fonts.** CJK: Smiley Sans (display) / Noto Serif SC (body) / Noto Sans SC (labels); italic serif and mono tabular figures have no exact Hanzi equal — keep ledger dates Latin.
