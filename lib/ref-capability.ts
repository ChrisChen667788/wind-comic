...
export function refUsageFor(engine: RefEngine, refCounts: number[], env?: Record<string, string | undefined>): RefUsage {
  ...
  const used = refCounts.reduce((n, c) => n + Math.min(1 + Math.max(0, c), cap), 0);
  const dropped = offered - used;
  const reason = dropped === 0
    ? `${engine} 接收全部 ${offered} 张参考图`
    : engine === 'kling' && cap === 1
      ? `可灵 Elements 未开启(KLING_ELEMENTS=1 且需对应套餐)—— 每角色只发 1 张正面图,已忽略 ${dropped} 张角度图`
      : engine === 'minimax'
        ? `MiniMax v1 S2V-01 每个角色只收 1 张参考图 —— 已忽略 ${dropped} 张角度图(H3 可收 9 张,但只能按量付费,Token Plan 订阅调不动)`
        : `${engine} 每角色只收 ${cap} 张 —— 已忽略 ${dropped} 张角度图`;
  return { engine, perCharacter: cap, used, dropped, reason };
}
