...
export function planLipSyncQc(input: LipSyncQcInput): LipSyncQcVerdict {
  ...
  if (input.onlyShots && input.onlyShots.length) {
    const set = new Set(input.onlyShots);
    audits = audits.filter((a) => set.has(a.shotNumber));
  }

  const plan = buildRebirthPlan(audits, { threshold, maxShots: 100 });
  const weakShots = plan.shots.map((s) => s.shotNumber);

  if (weakShots.length === 0) {
    return { decision: 'done', weakShots: [], round, threshold, message: `口型质检通过:全部 ≥ ${threshold} 分 ✓` };
  }
  if (round < maxRounds) {
    return { decision: 'rerender', weakShots, round, threshold, message: `${weakShots.length} 镜口型偏弱(<${threshold})— 第 ${round}/${maxRounds} 轮自动重渲` };
  }
  return { decision: 'stop', weakShots, round, threshold, message: `仍有 ${weakShots.length} 镜口型偏弱,已达 ${maxRounds} 轮上限 — 转人工复核` };
}
