...
/**
 * 性相关表述(v12.361)——**全仓唯一一份**,CSAM 与真人色情两条规则共用。
 *
 * 设计原则:**宁可误伤,不可漏判**,所以这里不是「把敏感字删掉」,而是
 * 「保留敏感字,但排除已知无害的复合词」。排除项都经过逐条确认:
 * 它们在任何组合下都构不成性相关表述,因此不产生绕过路径。
 *
 * · `性`:排除 个性/理性/感性/male-female 的男性女性/属性/弹性/记性/急性/慢性/
 *        药性/毒性/血性/人性/本性/天性/习性/索性/任性/野性/磁性/酸性/碱性/
 *        阴性/阳性/中性/性格/性别/性命/性子/性质/性能/性状/性价比
 *        —— 仍然拦得住 性行为/性侵/性爱/性感/性暗示/性器 等
 * · `肉`:排除 肌肉/红烧肉/牛肉/猪肉/鸡肉/鱼肉/肉眼/果肉/肉体(非性语境难分,保留拦截)
 *        —— 仍然拦得住 肉欲/黄文里的用法
 * · `裸` 保持裸字:它没有无害的常见复合词(裸眼/裸辞极少出现在角色设定里,
 *        且即便误伤,代价也远小于漏判)。
 */
const 性_INNOCUOUS_PREFIX = '个|理|感|男|女|属|弹|记|急|慢|药|毒|血|人|本|天|习|索|任|野|磁|酸|碱|阴|阳|中|惰|韧|粘|溶|挥发';
const 性_INNOCUOUS_SUFFIX = '格|别|命|子|质|能|状|价';
const 肉_INNOCUOUS_PREFIX = '肌|红烧|牛|猪|鸡|鱼|羊|果|鸭|腊|瘦|肥';
const 肉_INNOCUOUS_SUFFIX = '眼|类|丝|片|馅|松|铺';

/** 拼成一个可复用的备选组(不含外层括号)。 */
const SEX_TERM = [
  `(?<!${性_INNOCUOUS_PREFIX})性(?!${性_INNOCUOUS_SUFFIX})`,
  `(?<!${肉_INNOCUOUS_PREFIX})肉(?!${肉_INNOCUOUS_SUFFIX})`,
  '裸', 'H\\b', 'R-?18', '色情', '涩', '黄文', '做爱', '床戏',
  '性行为', '性侵', '性爱', '性暗示', '性器',
  // v12.361:**英文侧原本几乎是空的** —— 改动前整条规则只有 `H\b` 和 `R-18`,
  // `underage nude photo` 直接放行。这是**既有漏洞**,不是本次改动引入的
  // (已核对改动前的版本确认)。
  // 补英文词只会**加强**拦截:这两条规则已经要求与 underage/child/loli/minor 邻近,
  // 不存在「因为加了 nude 就误伤正常内容」的路径。
  'nude', 'naked', 'nsfw', 'porn', 'hentai', 'lewd', 'erotic', 'explicit',
  'sexual', 'sex\\b', 'fondl', 'molest',
].join('|');
...
const HARMFUL_PATTERNS: Array<{ re: RegExp; reason: string }> = [
  ...
  //
  // v12.361:原来用**裸字** `性` / `肉` / `床`,在正当创作文本上会误伤。实测:
  //   「小学老师李梅,**性格**温柔」→ 被拦,理由「未成年人性化」
  //   「初中生小杰,**个性**内向」→ 同上
  // 把正常人物设定判成 CSAM,而且**理由还把用户描述成那样** —— 比单纯拦截更糟。
  //
  // 修法是**排除已知无害复合词**,不是删掉这个字 —— 后者会削弱拦截。
  // `SEX_TERM` 保留了所有真正的性相关表述;下方测试锁住「有害的仍然拦得住」。
  { re: new RegExp(`(未成年|小学|初中|幼儿|loli|child|underage|minor)[\\s\\S]{0,20}?(${SEX_TERM})`, 'i'), reason: '未成年人性化' },
  { re: new RegExp(`(${SEX_TERM})[\\s\\S]{0,20}?(未成年|小学|初中|幼儿|loli|child)`, 'i'), reason: '未成年人性化' },

  // 真人色情/诽谤
  // v12.361:`床` 同样是裸字(起床/床头/病床),改用 `床戏`;其余保持不变。
  { re: /(.{2,8})\s*(裸|H\b|color|做爱|床戏|乳|阴|肛)\s*(图|视频|文)/, reason: '真人色情' },
