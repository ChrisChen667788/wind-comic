...
/**
 * v12.377:一组引擎失败报文 → 「是否该停掉当轮视频」。
 *
 * 每日重跑脚本原来的判据是「产出了 Ken Burns 占位片」→ 推断「当日额度耗尽」→
 * 停掉本项目剩余镜头,并对后续项目关掉视频步骤。但占位片只说明**这一镜失败了**:
 * 欠费、接口下线、网络抖动、敏感词拦截都会走到同一个回落分支,
 * 其中只有配额类值得停整轮 —— 一次网络抖动就能让当天的视频额度一个都用不上。
 *
 * 拿不到报文时**保守停**:没有证据的时候,少花钱比多试错安全。
 * (这与 media-reachable 的「只拦确认缺失」方向相反 —— 那里误杀会静悄悄少掉声音,
 *  这里误放会真金白银地烧额度,代价不对称,所以默认值也该相反。)
 */
export function shouldStopForQuota(failures) {
  const blob = (Array.isArray(failures) ? failures : [])
    .map((f) => `${f?.engine ?? ''} ${f?.error ?? ''}`)
    .join(' | ')
    .trim();
  if (!blob) return { stop: true, reason: 'no-evidence' };
  if (ARREARS_RE.test(blob)) return { stop: true, reason: 'arrears' };
  if (SATURATED_RE.test(blob)) return { stop: true, reason: 'saturated' };
  return { stop: false, reason: 'transient' };
}
